...
```js
// sidebars.js — fully autogenerated from the bundle tree
...
3. **Docusaurus CommonMark mode is experimental.** `markdown.format: 'detect'`
   ...
   The Docusaurus scaffolder should be smoke-tested with a real `docusaurus
   build` against the live bundle before each release.
