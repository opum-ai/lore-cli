...
`0.6.2` is **RELEASED**. Published 2026-09-14 from tag `v0.6.2` at
...
sha256-verified before any registry write. All seven packages are present at
`0.6.2` with `latest` moved on each, checked package by package against the
registry. A clean-room install — a fresh temp dir, nothing from local caches —
resolved `@opum-ai/lore@0.6.2`, returned `0.6.2` from the launcher's own
`--version`, and pinned all six platform packages at `0.6.2`.

**The digest check was independent for this release, not merely self-consistent.**
`SHA256SUMS.txt` is generated by the operator from the same tarballs it then
verifies, so on its own it proves only that the download has not changed since
it was sealed. For `0.6.2` the six PLATFORM tarballs were additionally
recomputed and matched against `package.platformTarballSha256` in each
`ladybug-package-qualification-<name>-34801853367-3` report — the value
`.github/workflows/release.yml`'s `package` job asserts — 6/6, every report
carrying `repository.commit = ea3813ae…`. The ROOT LAUNCHER has no CI-recorded
digest, because it is `npm pack`'d inside that same job, so for that one tarball
the manifest remains a local seal. Do not round this up to "all seven
independently verified" (LCLI-489).

**`0.6.2` carries NO provenance attestation, and neither did `0.6.1`.** Verified
directly rather than inferred: `npm view @opum-ai/lore@0.6.2 dist.attestations`
returns empty, where `0.6.0`'s returns a SLSA provenance link. This is a real
regression in supply-chain posture and is not an oversight — provenance is
produced by the CI OIDC path, that path is broken (below), and **a manual
publish cannot mint an attestation**. State this plainly rather than letting a
reader infer provenance from `0.6.0`'s presence.
