...
**Staleness is detectable; recovery is not — the two halves of a run id do not
degrade the same way.** Run METADATA is attempt-scoped and answers correctly
forever: `.../actions/runs/34968280818/attempts/1` will keep saying `success`
with its real job list no matter how many later attempts run. Run ARTIFACTS
are not attempt-scoped at all — `.../attempts/1/artifacts` is a hard 404 — so
`attempts/1` tells a reader that attempt 1 succeeded, but cannot hand back the
bytes it produced. The only thing that can is a copy taken at citation time
(this is why opum-cli-e2e could catch the `win32-x64` divergence at all: five
of its six re-fetched reports matched an earlier retained copy byte-for-byte,
and the sixth didn't). So: cite `.../attempts/1` to make a claim VERIFIABLE,
but don't read it as a recovery route — this record's own six-tarball digest
match is not independently reproducible from GitHub anymore, only trustable
because it was performed and recorded before the re-run. Same rule this repo
already set in [Release publishing](../runbooks/release-publishing.md) for
`0.6.0`'s run `34393976910` (there, a 404 from a history rewrite; here, a
...
`34968280818`'s own `npm-packages` artifact, every tarball sha256-verified
before any registry write. All seven packages are present at `0.7.0` with
`latest` moved on each, checked package by package against the registry. A
...
**The tag was qualified before the publish, and the publish was conditional on
it.** Release run `34968280818` was dispatched on the tag with `publish: false`
and finished `success`, with all six matching-host package qualifications green —
`linux-x64`, `linux-arm64`, `darwin-x64`, `darwin-arm64`, `win32-x64`,
`win32-arm64`. This matters because `release.yml` is `workflow_dispatch`-only:
the six-host native execution matrix runs **on** the tag, not before it, so
`main` being green is not evidence about the platform packages and must never be
reported as if it were.

**The digest check was independent for this release, not merely self-consistent.**
The six PLATFORM tarballs were recomputed and matched against
`package.platformTarballSha256` in each
`ladybug-package-qualification-<name>-34968280818` report — the value
`.github/workflows/release.yml`'s `package` job asserts — 6/6, every report
carrying `repository.commit = 5c437eb5…`, the same commit the tag peels to. The
ROOT LAUNCHER has no CI-recorded digest, because it is `npm pack`'d inside that
same job, so for that one tarball `SHA256SUMS.txt` remains a local seal
generated by the operator from the same bytes it then verifies. **6 independently
verified and 1 locally sealed — do not round this up to "all seven
independently verified"** (LCLI-489).

**`0.7.0` carries NO provenance attestation, and neither did `0.6.2` or `0.6.1`.**
Verified directly rather than inferred: `npm view @opum-ai/lore@0.7.0
dist.attestations` returns empty, where `0.6.0`'s returns a SLSA provenance
link. This is not an oversight and not tampering — provenance is produced by the
CI OIDC path, that path is broken (below), and **a manual publish cannot mint an
attestation**. `release.yml`'s `publish (npm, OIDC trusted publishing)` job shows
`skipped` on run `34968280818` for exactly this reason. State this plainly
rather than letting a reader infer provenance from `0.6.0`'s presence.

**npm propagation lagged the writes badly, and the intermediate state looked
exactly like a partial failure.** Four of the seven packages read as ABSENT from
the registry immediately after their own publishes had already succeeded.
Visibility landed at 0s, 0s, 1s, 79s, 79s, 140s and 200s across the set — so the
lag is **per-package and bimodal**, not a single delay a fixed sleep would cover.
**Publish order is not propagation order.** Poll each package by name, and never
re-publish or unpublish on a transient 404: an unpublish cannot be taken back
inside 72 hours, which makes misreading this state the one genuinely destructive
...
`scripts/publish-release.sh 0.6.2 34801853367`** against Release run
`34801853367`'s own `npm-packages` artifact (attempt 3), every tarball
sha256-verified before any registry write. All seven packages reached `0.6.2`
with `latest` moved on each, and a clean-room install pinned all six platform
packages at `0.6.2`. Its six platform tarballs were likewise verified against
`package.platformTarballSha256` in the `-34801853367-3` reports, 6/6, every
report carrying `repository.commit = ea3813ae…`; the root launcher was a local
seal, as it is for every manual publish. It carries no provenance attestation,
for the same reason `0.7.0` does not.

`0.6.1` was **RELEASED**. Published 2026-09-13 from tag `v0.6.1` at
