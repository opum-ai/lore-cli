...
| Quest CLI package, migration, implementation, and release | `opum-ai/quest-cli` | Treat its evidence as external owner state; Lore's LCLI-315.4 work is explicitly authorized to qualify the locally installed `0.2.0` RC, without asserting public availability, and every later build or migration behavior still requires fresh owner evidence |
