...
| Quest CLI package, migration, implementation, and release | `opum-ai/quest-cli` | Treat its evidence as external owner state; LCLI-315.4 qualifies the locally installed Quest `0.2.7` artifact and its migration contract; this local qualification does not assert public availability, and every later artifact requires fresh owner evidence |
