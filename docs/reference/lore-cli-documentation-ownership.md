...
| Quest CLI package, migration, implementation, and release | `opum-ai/quest-cli` | Treat its evidence as external owner state. `@opum-ai/quest@0.1.0` is publicly published; Lore's current tracker default remains Backlog.md until LCLI-315.4 implements the approved Quest-default behavior and LCLI-333 releases it. Do not imply that registry availability alone changes existing configurations or current Lore behavior. |
