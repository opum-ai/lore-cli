...
tags:
  ...
  - roadmap
  - local-graph
summary: Orders post-MVP local graph work as LadybugDB persistent indexing, graph explorer, and then LadybugDB-enabled capabilities, with local MCP on hold.
...
The post-MVP local graph roadmap is deliberately sequential. Lore first adopts a persistent LadybugDB projection to improve repeated-query performance and establish measurable scale, packaging, recovery, and concurrency behavior. Only after that foundation passes does Lore ship a local graph explorer. New multi-repository and deeper graph capabilities follow the explorer. The local stdio MCP design remains retained but on hold and is not part of this delivery chain.

Parent task `LCLI-283` owns the roadmap. Milestones and feature gates are:

1. M6 / `LCLI-283.1` — LadybugDB persistent local indexing.
...
`LCLI-283.1` introduces the database foundation before any new UI or graph operation.
...
- `LCLI-283.1.2` implements deterministic projection construction, reconciliation, transactional replacement, invalidation, corruption recovery, and disposal.
- `LCLI-283.1.3` routes graph, lexical query, and context through indexed retrieval while retaining the in-memory implementation as a conformance oracle and documented fallback.
...
`LCLI-283.2` depends on all M6 acceptance gates and consumes the stable indexed projection through a Lore-owned read contract.

- `LCLI-283.2.1` freezes the explorer snapshot, data, provenance, interaction, accessibility, graph-health, and bounded-rendering contracts.
- `LCLI-283.2.2` builds the deterministic static explorer and CLI entrypoint with search, filters, detail inspection, inbound and outbound focus, depth focus, dangling references, and supersession chains.
