...
- Hosted AuraDB and the private hosted graph API remain separate. This ADR changes the local CLI projection only and does not move hosted graph ownership into `lore-cli`.

### Redaction note (2026-09-09): repository identifier removed for public-repo hygiene

This ADR originally named the specific private repository behind the hosted
graph API mentioned above. Once this repository became public, the
identifier was replaced with a general description — the decision, its
date, its rationale, and its consequences are unchanged; only the specific
repository name is removed. This is a redaction, not a revision: nothing
above was re-decided. See `CLAUDE.md`'s "Product portfolio and
cross-repository routing" section for the same class of redaction applied
to a portfolio-wide reference list.
