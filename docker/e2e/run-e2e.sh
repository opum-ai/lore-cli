...
step_json "LCLI-299 AC3: schema export --out writes the full profile outside the default directory" \
  '.kind == "schema.result"
   ...
   and (([.data.files[].path] | sort) == [
     ...
     "'"$LCLI299_CUSTOM_OUT"'/constants.schema.json",
...
# default nine-type vocabulary wholesale, and a full `lore schema export` prunes a
...
# `lore scaffold mkdocs` adds a real OKF concept, docs/tags.md (LCLI-357), alongside the
# non-lore mkdocs.yml -- but the scaffold command itself never syncs, so the bundle's existing
# managed index (built by every `lore sync` since Phase 1) goes stale the moment this new file
# lands. This is the third, previously-unlocated source of the drift LCLI-378 traced between
# Phase 17a and Phase 24c; heal it the same way Phase 16 and Phase 17b already do.
step_json "lore sync: heal the index left stale by docs/tags.md from lore scaffold mkdocs" \
  '.kind == "sync.result"' -- lore sync --json
...
# passes before this phase ever runs. CI once found this step failing anyway, from index drift
# that accumulated between the two phases across three sources: Phase 16's unsynced ADR successor,
# Phase 17b's unsynced custom-type doc, and (LCLI-378, harder to trace because it needed real
# bisection rather than reading) Phase 18's `lore scaffold mkdocs` leaving docs/tags.md unindexed.
# All three are now fixed at their source. A defensive `lore sync` used to sit here masking the
# third one while it was still unlocated -- exactly the "auto-repairable drift silently made
# invisible again" failure LCLI-377 itself warns about -- so it is deliberately removed now that
# the source is fixed, restoring this step as a genuine regression guard for the whole 17a-24c
# window rather than a rubber stamp over it.
