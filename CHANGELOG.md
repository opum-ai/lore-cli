...
- **Every plain `lore agent context` pack is query-augmented** (LCLI-575; opum-doc ADR "Make lore
  agent context always query-augmented", ODOC-265, and its Amendment 1). A `## Bundle-wide query
  hits` section lists up to
  ...
  `--workspace --repository`, hits come from the selected members only. The opum-agent-workflow/v1
  projection (`lore agent project`, `lore agent context --contract`) does not carry the section:
  its embedded pack, `packDigest` and `inputRevisions` are byte-identical to 0.9.2's, so a
  document outside the profile's catalog cannot move a pinned digest (Amendment 1).
- **`lore help --json` lists `kindSchemaVersions`**, the per-`kind` `schemaVersion` overrides
  (additive; cli-contract §7.1). Today it is `{"agent.context.export": 2}`.
...
- **`lore agent context <unknown profile>` exits `0`, not `3`.** It degrades to the query section
  ...
  amendment §5.6 there, citing ODOC-265 decision 2. `agent show`, `agent project` and `agent
  context --contract` are unchanged on an unknown profile.
- **BREAKING (`--json`): the `agent.context.export` envelope's `schemaVersion` is `2`** (was `1`),
  for the exit `3` → `0` remap above — settled by the ADR's Amendment 1 (opum-doc `main` a8bb596)
  and recorded in cli-contract §5.6. The bump is scoped to that one `kind`: every other envelope,
...
- **Fixes a reported bug: `lore init --migrate-backlog` could fail with an unrecoverable `Alias
  collision` error (exit 5)** when a Backlog.md project had a dotted subtask (`task-1.1`) alongside
  an unrelated plain task whose id Quest's positional renumbering happened to land on. Quest's own
  fix for this already existed (`--preserve-source-ids --source-family <PREFIX>`) but `lore init`
  never exposed a way to reach it — an operator hitting the collision had no recourse short of
  abandoning the flow and driving Quest's CLI by hand. Both flags are now accepted and threaded
  through to Quest exactly as its own contract requires (LCLI-465).

- **A new `--antigravity` agent bridge writes project-local `GEMINI.md` context** for Google
  Antigravity and Gemini CLI, which both read the same filename at the workspace root (confirmed
  against each tool's own official docs). Offered in the interactive wizard's multi-select
  whenever the `gemini` binary is on `PATH`, mirroring how `--claude`/`--codex`/`--hermes` are
  each gated on their own binary (LCLI-464).

### Changed

- **The interactive wizard's up-to-three sequential yes/no agent-bridge questions (Claude, Codex,
  Hermes) are now one multi-select** ("Select instruction files to write (space toggles, enter
  accepts)"), listing only the bridges actually detected on `PATH`. **Nothing starts pre-checked**
  — a bare Enter writes no bridge files at all, the same "decline every bridge" outcome this
  wizard already supported, now the free default rather than something to opt back into (the
  user's own original complaint was tools writing files nobody asked for, so pre-checking
  anything would have reproduced it). Coordinated directly with quest-cli so both CLIs answer the
  fleet's "which instruction files should this write?" question the same way — same library
  (`@clack/prompts`), same wording, same empty-selection-legal/cancel-cleanly semantics
  (LCLI-462). The non-interactive flag path (`--agents`/`--codex`/`--hermes`/`--antigravity`,
  `--yes`) is unchanged; this replaces only the interactive prompt.
- **The `AGENTS.md` option's label now names pi and OpenCode**, both confirmed to read `AGENTS.md`
  as their primary convention (`CLAUDE.md` only as a fallback) — no dedicated file needed for
  either, unlike Claude Code (whose `CLAUDE.md` has no native `AGENTS.md` fallback at all) or
  Antigravity/Gemini CLI (whose `GEMINI.md` is a genuinely separate, equally-official convention
  per Google's own docs, hence the new dedicated `--antigravity` bridge above) (LCLI-464).

## [0.5.0] - 2026-09-08

**Lore and Quest move in lockstep from this release on.** Quest ships 0.5.0 at the same time, and
every stacked release from here forward names the pair with one shared version number rather than
two independently-numbered CLIs that happen to be tested together.

### Removed

- **BREAKING: `InitResult.backlog` and the internal `legacyBacklogCheck` helper
  are removed.** `trackerCheck` (added in 0.3.5) is now the only tracker-capability field `lore
  init --json` emits — it names whichever backend was actually probed, where `backlog` could only
  ever describe a Backlog.md bundle and silently omitted a Quest or Jira probe's result under the
  same shape. A `--json` consumer reading the deprecated field must move to `trackerCheck` before
  upgrading past 0.4.x (LCLI-359).

### Added

- **A new `lore instructions agents` topic documents how `lore agents`/`lore agents --check`
  decide which agent bridge (Claude, Codex, or both) to check or write.** The underlying
  presence-based scoping (LCLI-442) was already correct — a Codex-only repository's `--check`
  already stayed green without a Claude bridge nobody asked for — but the behavior itself was
  undocumented outside source comments, which is exactly the class of confusion LCLI-437 first
  reported. Also adds regression coverage for a Codex-only repository's OWN bridge drift, which
  every existing drift test exercised only with both bridges present (LCLI-437).

### Fixed

- **Configuration error diagnostics stay stable and actionable across the whole declared-supported
  Bun range (1.3.14 through the newest release), instead of leaking a specific runtime's exact TOML
  parser wording.** Bun 1.4.x changed both malformed-TOML error text and how an imprecise bare
  integer literal is handled (Bun's own parser now rejects it at parse time, rather than silently
  rounding it for lore's own precision check to catch) — both are stable, actionable `validation`
  errors either way; only the exact code path producing them differs by runtime. CI now also runs
  the configuration test suite against the newest available Bun, not only the pinned floor, so a
  future wording change is caught immediately (LCLI-436).

- **`lore export` and the Ladybug indexed source builder no longer reject a tracker-none bundle.**
  A docs-only bundle (`lore init --tracker none`) already passes `lore validate --strict`, but
  `lore export` and workspace/indexed retrieval unconditionally constructed a real tracker adapter
  to list tasks, throwing "this bundle has issue-tracker coupling disabled" even though a read-only
  projection just needs an empty task set. A genuine tracker write (`lore link`, etc.) still rejects
  disabled coupling exactly as before (LCLI-435).

- **`lore check` no longer walks into a git-ignored vendor/cache directory when its bundle root
  contains one.** A large gitignored non-bundle tree (a vendored toolchain, `node_modules/`, a
  build cache) sitting inside a checked directory used to be read like any other content, turning
  every stray `.md`-shaped file in it into real findings — a docs gate failing on files the project
  never wrote. `lore check` (with or without an explicit path argument — both share the same walk)
  now skips anything `git` reports as ignored, falling back to the original unfiltered walk outside
  a git repository so this stays advisory, never a hard requirement (LCLI-379).

- **BREAKING: Lore's Quest writes no longer declare every actor `human`, and now
  require one to be declared at all.** `src/adapters/quest.ts` hardcoded `--actor lore --actor-kind
  human` on every task edit, task create, and Backlog migration apply/rollback, so a
  delegated-agent session's identity and accountable human were lost the moment the write reached
  Quest — and any caller reached Quest at all, since the hardcoded identity always satisfied
  Quest's own actor requirement. The adapter now accepts an explicit actor context (`actor` option,
  or the `LORE_QUEST_ACTOR`/`LORE_QUEST_ACTOR_KIND`/`LORE_QUEST_ACCOUNTABLE_HUMAN` environment
  variables), validated the same way Quest validates its own `--actor`/`--actor-kind`/
  `--accountable-human` flags. **A `lore link`/`lore unlink`/`lore init --migrate-backlog` invocation
  against a Quest-backed bundle that previously declared no actor at all now fails closed instead of
  silently succeeding under a fabricated human identity, uniformly reporting `error_type: "validation"`
  at the top level (exit 6) with the fix named in the hint** — set the three environment variables
  above before upgrading past 0.4.x if your invocation doesn't already. `lore link`/`lore unlink`
  previously would have folded this into LORE-58's generic per-task `drift` wrapper instead, the same
  as any other per-task edit failure; a missing actor declaration is identical for every task in a
  batch and a caller configuration error, not per-task drift, so it now bypasses that wrapper and
  surfaces directly, matching the shape `lore init --migrate-backlog`'s Quest apply/rollback always
  had (LCLI-434, unified in LCLI-459; see `lore instructions linking` for the exact shape).

## [0.4.6] - 2026-09-07

### Fixed

- **`lore sync`/`lore check` no longer throw when a Story links a task Quest has paused to its new
  `Blocked` status.** Quest 0.4.0 adds a non-terminal `Blocked` status deliberately excluded from
  `task status-flow`'s ladder; the tracker adapter now reads Quest's `pausedStatus` field and
  classifies a matching task status as active (`in-progress`) instead of failing the reconciliation
  with "not in the project's configured status flow" (LCLI-455).
...
- **`lore agent context <profile> --workspace <manifest> --repository <member-id>` compiles a
  profile-bounded evidence pack across an explicit workspace manifest** (PLAN.md §4.6; LCLI-432).
  A profile's `sources`/`pinned` entries stay unqualified and bundle-relative, so the same profile
  works bare and cross-repo: in workspace mode an unqualified reference fans out to every selected
  `--repository` member, qualified as `<member-id>::<source-id>`, while an explicitly qualified
  reference (`opum-doc::reference/opum-fleet-operating-model`) pins exactly that one member without
  fanning out. `pinned` stays strict (a missing pinned reference is still a hard failure); `sources`
  relaxes — a reference missing from one loaded member is reported `missing-in-member` in the
  catalog rather than failing the whole pack. A manifest member that cannot be loaded at all (a
  dormant fleet member) is skipped rather than rejecting the compile: it is named once at the top
  of the pack under `## Skipped workspace members`, and every source that would have targeted it is
...
- **`lore init`'s interactive wizard now defaults to one agent harness, not both.** With both
  Claude Code and Codex detected, pressing Enter through both agent-bridge questions used to
...
- Matching-host package qualification now gives its fixture Backlog process the
  logical repository root through `BACKLOG_CWD`, while keeping the physical
  subprocess cwd isolated from a protected environment-file sentinel
  (LCLI-337).
