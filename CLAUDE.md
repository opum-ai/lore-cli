...
Authoring standards for this file's own prose (the "can a reader satisfy this sentence literally
and still violate the rule" test, and the gate-defect shapes it catches) moved to
[Lore CLI repository notes](docs/reference/lore-cli-repository-notes.md) — read it before editing
this file's repo-profile or routing prose, not before ordinary task work.
...
Cross-session lessons from coordinating this pairing on 2026-08-29/30 — verify a peer's claim
rather than accept it, recompute instead of comparing two descriptions, state a falsifier before a
run, and four more — moved to
