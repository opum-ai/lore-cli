 ...
 *
 * Each target's config is generated **additively** — every scaffold run only ADDS files, and never
 * moves, renames, restructures, or rewrites authored bundle content — so the OKF bundle stays the
 * single source of truth (ADR-0010 §2).
 *
 * Additive is the invariant; "outside `docs/`" is NOT, and saying so was a documented falsehood two
 * of the three implemented targets contradicted (LCLI-357). Two targets add new files *inside*
 * `docs/`, each because its consumer requires the file to live there:
 * {@link buildMkdocsScaffold} writes `docs/tags.md` (Material's `tags` plugin renders the index from
 * a page inside the docs tree, consumer-compatibility.md §3.3), and {@link buildObsidianScaffold}
 * writes `docs/.obsidian/app.json` and `docs/.obsidian/.gitignore` (Obsidian's vault-scoping needs
 * `docs/` itself to be the vault root, §3.2). Only {@link buildDocusaurusScaffold} writes entirely
 * outside. What holds for all three is that nothing already in the bundle is mutated to satisfy a
 * consumer, and every added file is itself OKF-legal.
 ...
 *
 * Provenance goes through the shared {@link versionedProvenance} — the same helper the root
 * `docs/index.md` and every `lore new` write use — rather than a hardcoded `timestamp` key
 * (LCLI-357). That hardcoded key was the legacy OKF 0.1 spelling, so under a 0.2 bundle the
 * validator warned "legacy key" on it and `lore validate --strict` exited 6: scaffold generated
 * content lore's own gate then rejected. Routing through the helper means the emitted shape
 * follows the profile's `okfVersion` and can never disagree with what the validator expects.
 */
 ...
 *   without copying or mutating it (ADR-0010 §2: scaffolding is additive). Docusaurus is the one
 *   target that writes entirely outside `docs/`; see this module's docstring.
