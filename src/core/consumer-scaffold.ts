 ...
 * resolve mismatched minors). `react`/`react-dom` are pinned to the verified peer range.
 * Deliberately no `"type": "module"` — see the module docstring for why the config files
 * must load as CommonJS.
 */
...
function docusaurusConfigJs(siteName: string): string {
  return `// Generated once by \`lore scaffold docusaurus\` — user-owned; re-run with --force to regenerate.
// See docs/reference/consumer-compatibility.md §3.4 and ADR-0010 for the rationale.
//
