...
/** Input for {@link indexDriftFindings}. */
export interface IndexDriftInput {
  /** Each live `index.md`'s current on-disk bytes, keyed the same way `existing` is for {@link
   * import("./indexes").generateIndexes} — bundle-relative path, e.g. `"index.md"`, `"adr/index.md"`. */
  readonly existing: ReadonlyMap<string, string>;
  /** The same paths' freshly regenerated bytes — {@link import("./indexes").generateIndexes}'s own
   * return value, given `existing` as its seam input. */
  readonly regenerated: ReadonlyMap<string, string>;
  /** Same meaning as {@link ReconcileDriftInput.fixable}: whether `lore sync` would actually
   * regenerate this bundle root if run, so the message can offer that hint. */
  readonly fixable: boolean;
}

/**
 * The **index-drift** findings: one `error` per live `index.md` whose committed
 * `<!-- lore:index -->` block no longer matches what {@link import("./indexes").generateIndexes}
 * would regenerate for it (LCLI-377) — the same condition `lore sync` would fix by writing.
 *
 * Deliberately narrower than "every mismatch": a path is only judged when its `existing` bytes
 * already carry the `<!-- lore:index:begin -->` marker, i.e. some earlier `lore sync` already
 * wrote it. `generateIndexes` *appends* a fresh block to an index that has none yet (documented
 * scaffold-then-write behavior — a brand-new `lore init` + `lore new`, before the first `lore
 * sync`, is not stale, it is simply never-synced), so comparing bytes with no marker present
 * would flag that ordinary, unexceptional state as drift on every such bundle's very first
 * `check`. An index missing entirely from `existing` is the same "never synced" case and is
 * skipped the same way; {@link import("./indexes").orphanedIndexPaths} is the separate query for
 * an on-disk index whose directory no longer needs one at all — out of scope here.
 */
export function indexDriftFindings(input: IndexDriftInput): CheckFinding[] {
  const findings: CheckFinding[] = [];
  const hint = input.fixable ? " — run `lore sync` to regenerate it" : "";
  for (const [path, regenerated] of input.regenerated) {
    const current = input.existing.get(path);
    if (current === undefined || !current.includes(INDEX_BLOCK_BEGIN)) {
      continue;
    }
