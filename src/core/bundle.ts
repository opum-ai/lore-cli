 ...
 *
 * @param skipDir an optional pruning predicate, checked against each subdirectory's own
 *   bundle-root-relative path before recursing into it (LCLI-379: `lore check`'s explicit-path
 *   walk uses this to skip a wholly git-ignored directory — a vendored `.herdr/`-shaped tree —
 *   without ever reading its contents). Defaults to never pruning, the walk's original behavior;
 *   {@link walkMarkdown} never passes one, so every other caller is unaffected.
 */
