 ...
 * consumes the resolved list. Must carry **at least two** entries: a single-entry flow cannot
 * distinguish "not started" from "terminal" ({@link reconcileStatus} rejects it).
 *
 * Deliberately does NOT include a backend's non-terminal "paused"/side status (Quest 0.4.0's
 * `Blocked`, LCLI-455): that status is excluded from the ladder by the backend itself (Quest's own
 * `LifecyclePolicy` rejects a `pausedStatus` that collides with `statuses`), so a status flow
 * example naming it here would misdescribe the real adapter contract. See
 * {@link reconcileStatus}'s `pausedStatus` parameter for how it is classified instead.
 */
 ...
 * @param pausedStatus a backend's non-terminal "paused"/side status, deliberately excluded from
 *   `statusFlow`'s ladder (Quest 0.4.0's `Blocked`, LCLI-455 — `TrackerAdapter.pausedStatus()`).
 *   `undefined` by default. A task status exactly matching `pausedStatus` classifies as active
 ...
 * Quest 0.4.0's `Blocked`, LCLI-455), it classifies as `"active"` instead of falling through to the
...
function classify(
  ...
  if (index === -1) {
    if (pausedStatus !== undefined && status === pausedStatus) {
      return "active";
    }
    throw new LoreError(
      "validation",
