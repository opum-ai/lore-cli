...
<style>
:root{color-scheme:light dark;font:16px/1.45 ui-sans-serif,system-ui,sans-serif;--bg:#f6f3ec;--panel:#fffdf7;--ink:#20231f;--muted:#686d64;--line:#c9c7bc;--accent:#12615b;--in:#7857a7;--out:#b35b16;--warn:#9b3a25}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--ink)}header{padding:1rem 1.25rem;border-bottom:1px solid var(--line);background:var(--panel)}h1,h2,h3,p{margin:.25rem 0 .75rem}.layout{display:grid;grid-template-columns:minmax(16rem,25rem) minmax(20rem,1fr) minmax(18rem,28rem);gap:1rem;padding:1rem}.panel{background:var(--panel);border:1px solid var(--line);border-radius:.65rem;padding:1rem;min-width:0}.controls{display:grid;gap:.7rem}.controls label{display:grid;gap:.2rem;font-weight:600}.checks{display:flex;flex-wrap:wrap;gap:.5rem}.checks label{display:flex;align-items:center;gap:.25rem;font-weight:400}input,select,button{font:inherit}input,select{width:100%;padding:.55rem;border:1px solid var(--line);border-radius:.35rem;background:var(--panel);color:var(--ink)}button{width:100%;text-align:left;padding:.65rem;border:1px solid var(--line);border-radius:.4rem;background:transparent;color:inherit;cursor:pointer}.node-list,.relation-list{list-style:none;margin:0;padding:0;display:grid;gap:.45rem}.node button[aria-current=true]{outline:3px solid var(--accent)}.node.inbound button{border-left:6px solid var(--in)}.node.outbound button{border-left:6px solid var(--out)}.node.both button{border-left:6px solid var(--accent)}.badge{display:inline-block;margin-right:.35rem;padding:.05rem .4rem;border:1px solid currentColor;border-radius:999px;font-size:.78rem}.muted{color:var(--muted)}.warning{color:var(--warn)}dl{display:grid;grid-template-columns:max-content minmax(0,1fr);gap:.35rem .75rem}dt{font-weight:700}dd{margin:0;overflow-wrap:anywhere}.relation-list button{padding:.35rem}.sr-status{min-height:1.5em}.legend{display:flex;gap:.75rem;flex-wrap:wrap}.legend span:before{content:"";display:inline-block;width:.8rem;height:.8rem;margin-right:.25rem;border-radius:.15rem;background:var(--accent)}.legend .in:before{background:var(--in)}.legend .out:before{background:var(--out)}@media(max-width:950px){.layout{grid-template-columns:1fr 1fr}.details{grid-column:1/-1}}@media(max-width:640px){.layout{grid-template-columns:1fr;padding:.5rem}.details{grid-column:auto}}@media(prefers-reduced-motion:reduce){*{scroll-behavior:auto!important;transition:none!important}}@media(prefers-color-scheme:dark){:root{--bg:#171a18;--panel:#202521;--ink:#f2f0e8;--muted:#b6bbb2;--line:#4b514b;--accent:#60c9bc;--in:#bd9ee7;--out:#ef9a55;--warn:#ff9d85}}
</style>
</head>
<body>
<header><h1>Lore graph explorer</h1><p id="provenance" class="muted"></p><p id="health" class="sr-status" role="status" aria-live="polite"></p></header>
<main class="layout">
<section class="panel controls" aria-labelledby="controls-title"><h2 id="controls-title">Find evidence</h2><label>Search<input id="search" type="search" autocomplete="off"></label><fieldset><legend>Record kind</legend><div class="checks" id="kind-filters"></div></fieldset><label>Type<select id="type-filter"><option value="">All types</option></select></label><label>Status<select id="status"><option value="">All statuses</option></select></label><label>Focus depth<select id="depth"><option>0</option><option selected>1</option><option>2</option><option>3</option><option>4</option></select></label><button id="clear-focus" type="button">Clear focus</button><div class="legend" aria-label="Relationship legend"><span class="in">Inbound</span><span class="out">Outbound</span></div></section>
<section class="panel" aria-labelledby="records-title"><h2 id="records-title">Records</h2><p id="counts" class="muted"></p><ul id="nodes" class="node-list"></ul></section>
...
for(const kind of ["repository","concept","task"]){const label=make("label");const input=document.createElement("input");input.type="checkbox";input.value=kind;input.addEventListener("change",()=>{input.checked?state.kinds.add(kind):state.kinds.delete(kind);render()});label.append(input,document.createTextNode(kind));$("kind-filters").append(label)}
...
function relation(edge,key){if(!key)return"none";const out=edge.fromRecordKey===key,inbound=edge.toRecordKey===key;return out&&inbound?"both":out?"outbound":inbound?"inbound":"none"}
...
function render(){const scope=state.focus?neighbors(state.focus,state.depth):null;const matched=nodes.filter(n=>matches(n,scope));const nodeLimit=state.focus?LIMITS.maximumVisibleNodes:LIMITS.initialNodeLimit;const visible=matched.slice(0,nodeLimit);const visibleKeys=new Set(visible.map(n=>n.recordKey));const relations=new Map();for(const edge of SNAPSHOT.facts.authoredEdges){const rel=relation(edge,state.selected);if(rel!=="none"){for(const key of [edge.fromRecordKey,edge.toRecordKey])if(key){const old=relations.get(key);relations.set(key,old&&old!==rel?"both":rel)}}}
const list=$("nodes");list.replaceChildren();for(const node of visible){const li=make("li",undefined,"node "+(relations.get(node.recordKey)||"none"));const button=make("button");button.type="button";button.setAttribute("aria-current",String(node.recordKey===state.selected));const badge=make("span",node.kind,"badge");button.append(badge,document.createTextNode(node.title||node.id));button.addEventListener("click",()=>{state.selected=node.recordKey;render()});button.addEventListener("dblclick",()=>{state.focus=node.recordKey;state.selected=node.recordKey;render()});li.append(button);list.append(li)}
$("counts").textContent=visible.length+" of "+matched.length+" matching records"+(visible.length<matched.length?" (bounded initial view)":"");$("health").textContent=SNAPSHOT.health.state+": "+SNAPSHOT.health.counts.concepts+" concepts, "+SNAPSHOT.health.counts.tasks+" tasks, "+SNAPSHOT.health.counts.danglingEdges+" dangling edges";renderDetails(visibleKeys)}
function row(dl,key,value){const dt=make("dt",key),dd=make("dd",value??"—");dl.append(dt,dd)}
function renderDetails(visibleKeys){const root=$("details");root.replaceChildren();const node=byKey.get(state.selected);if(!node){root.append(make("p","Select a record to inspect provenance and relationships.","muted"));return}root.append(make("h3",node.title||node.id));const dl=make("dl");row(dl,"ID",node.id);row(dl,"Type",node.type);row(dl,"Status",node.status);row(dl,"Source",node.sourcePath);row(dl,"Commit",node.gitCommit);row(dl,"Export",node.exportDigest);root.append(dl);const focus=make("button","Focus this record to depth "+state.depth);focus.type="button";focus.addEventListener("click",()=>{state.focus=node.recordKey;render()});root.append(focus);
const relationshipLimit=state.focus?LIMITS.maximumVisibleEdges:LIMITS.initialEdgeLimit;const allRelated=SNAPSHOT.facts.authoredEdges.filter(e=>e.fromRecordKey===node.recordKey||e.toRecordKey===node.recordKey);const related=allRelated.slice(0,relationshipLimit);root.append(make("h3","Relationships"));const ul=make("ul",undefined,"relation-list");for(const edge of related){const outbound=edge.fromRecordKey===node.recordKey;const other=outbound?edge.toRecordKey:edge.fromRecordKey;const target=other?byKey.get(other):null;const li=make("li");const label=(outbound?"outbound ":"inbound ")+edge.edgeKind+" → "+(target?.title||edge.target)+(edge.dangling?" (dangling)":"");if(target){const button=make("button",label);button.type="button";button.addEventListener("click",()=>{state.selected=target.recordKey;render()});li.append(button)}else li.append(make("span",label,"warning"));ul.append(li)}if(!related.length)ul.append(make("li","No authored relationships.","muted"));if(related.length<allRelated.length)ul.append(make("li",related.length+" of "+allRelated.length+" relationships shown","muted"));root.append(ul);
const supersession=new Set([node.recordKey]),queue=[node.recordKey];while(queue.length){const key=queue.shift();for(const edge of SNAPSHOT.facts.authoredEdges){if(!["supersedes","superseded_by"].includes(edge.edgeKind)||!edge.toRecordKey)continue;let next=null;if(edge.fromRecordKey===key)next=edge.toRecordKey;else if(edge.toRecordKey===key)next=edge.fromRecordKey;if(next&&!supersession.has(next)){supersession.add(next);queue.push(next)}}}if(supersession.size>1){root.append(make("h3","Supersession chain"));const chain=make("ul");for(const key of [...supersession].sort()){const item=byKey.get(key);chain.append(make("li",item?.title||key))}root.append(chain)}}
$("provenance").textContent="Snapshot "+SNAPSHOT.source.snapshotKey+" · commit "+(SNAPSHOT.source.gitCommit||"uncommitted")+" · schema "+SNAPSHOT.schemaVersion;window.__LORE_EXPLORER__={snapshot:SNAPSHOT,state,render};render();
</script>
</body>
</html>
`;
