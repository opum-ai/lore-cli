...
export async function runLink(options: LinkOptions): Promise<number> {
  ...
  // check it for an actor-context failure (LCLI-459) alongside `anyBackRefFailed`.
  let backRefOutcomes: readonly PromiseSettledResult<"already-present" | "added">[] = [];
  ...
  if (anyBackRefFailed || backlogCommit.error !== undefined) {
    throw actorContextFailure(backRefOutcomes) ?? backRefFailure("link", report);
  }
...
export async function runUnlink(options: LinkOptions): Promise<number> {
  ...
  // See runLink's `backRefOutcomes` — kept outside the block below so the throw site can check it
  // for an actor-context failure (LCLI-459) alongside `anyBackRefFailed`.
  let backRefOutcomes: readonly PromiseSettledResult<"removed" | "already-absent" | "skipped">[] = [];
  if (!noBackRef) {
    const removal = await removeBackRefs(adapter, taskIds, label, docPath, backend);
    refs = removal.refs;
    backRefOutcomes = removal.outcomes;
  ...
  // ErrorEnvelope as a failed edit (LORE-58), never the success envelope on a nonzero exit.
  if (anyBackRefFailed || backlogCommit.error !== undefined) {
    throw actorContextFailure(backRefOutcomes) ?? backRefFailure("unlink", report);
  }
...
/**
 * The first outcome that failed because of a missing/invalid Quest actor declaration (LCLI-459),
 * or `undefined` if every failure was genuine per-task drift. `resolveActor()`'s rejection is
 * identical for every task in the batch — it depends only on the caller's global actor/env
 * context, never on which task is being edited — so `runLink`/`runUnlink` re-throw this directly
 * instead of folding it into {@link backRefFailure}'s generic `drift` wrapper: a caller
 * pattern-matching on `error_type` sees `validation` for this class of failure, the same shape
 * `lore init --migrate-backlog`'s Quest apply/rollback already had.
 */
function actorContextFailure(outcomes: readonly PromiseSettledResult<unknown>[]): LoreError | undefined {
  for (const outcome of outcomes) {
    if (outcome.status === "rejected" && isQuestActorContextFailure(outcome.reason)) {
      return outcome.reason as LoreError;
    }
  }
  return undefined;
}
 ...
 * `backlogCommit`), so a caller loses no diagnostic granularity, just its location. Never called
 * when {@link actorContextFailure} found a match — that error is re-thrown directly instead.
 */
