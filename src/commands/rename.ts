...
import { removeFile, writeFileOverwriting } from "./fswrite";
...
export function runRename(options: RenameOptions): number {
  ...
  // Plan the move + inbound rewrite (throws not_found / conflict), then regenerate the index hubs
  // against the post-rename graph and merge them over the plan's writes (index regen wins).
  ...
  if (!parsed.dryRun) {
    ...
    // The moved file's new bytes were just written at its new path; delete the old path last, so a
    // crash mid-run never destroys the source before its replacement exists.
    if (plan.rename !== null) {
      removeFile(join(docsRoot, plan.rename.from), `${DOCS_DIR}/${plan.rename.from}`);
    }
  }
