...
async function expectUnlinkError(
  ...
  const stdout = capture();
  try {
    await runUnlink({ ...opts(args, adapter, gitSpawn, backend), stdout });
  } catch (err) {
    expect(err).toBeInstanceOf(LoreError);
    // AC#2 (LORE-58): stdout parses or stays silent — empty on every link/unlink failure, partial or otherwise.
    expect(stdout.text()).toBe("");
...
describe("lore link/unlink under a non-backlog tracker: a reported file never trips the drift refusal", () => {
  ...
  // A missing Quest actor declaration (LCLI-434) is a caller configuration error identical for
  // every task in the batch, not genuine per-task drift — unlike the poisoned-edit test above,
  // `lore link`/`lore unlink` must surface it as `validation` at the top level (LCLI-459),
  // matching the shape `lore init --migrate-backlog`'s Quest apply/rollback already had
  // (quest-adapter.test.ts's "no actor context anywhere fails migration apply and rollback...").
  const actorContextError = new LoreError(
    "validation",
    "Quest write requires an explicit actor declaration",
    "set LORE_QUEST_ACTOR and LORE_QUEST_ACTOR_KIND before this command",
  ...
  test("a missing Quest actor declaration on lore unlink surfaces as validation, not folded into drift (LCLI-459)", async () => {
    writeDoc(
      "stories/quest-actor-unlink.md",
      "---\ntype: Story\ntitle: Quest actor unlink\ntasks:\n  - q-1\n---\nBody.\n",
    );
    // Must already carry the back-ref (label + doc) so removeBackRefs actually reaches editTask
    // instead of taking its no-op "already-absent" branch — see the "lore unlink succeeds..." test
    // above for the same setup shape.
    const adapter = fakeAdapter(
      [
        questTask("Q-1", {
          labels: ["doc:stories/quest-actor-unlink"],
          documentation: ["docs/stories/quest-actor-unlink.md"],
        }),
      ],
