...
describe("lore scaffold mkdocs — never-silent-clobber (AC: user-owned, never re-overwritten)", () => {
  test("a re-run over a HAND-MODIFIED config still refuses with a conflict (exit 5) and touches nothing", () => {
    ...
    // Re-run with the SAME fixed clock as the initial scaffold() above, so docs/tags.md's
    // regenerated bytes (which embed the timestamp) match what's already on disk exactly — the
    // only real difference in this run is the hand-edit to mkdocs.yml.
