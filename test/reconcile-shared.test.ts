...
describe("gatherReconciliation", () => {
  ...
  test("threads the adapter's pausedStatus into ReconcileConfig when it implements one (LCLI-455)", async () => {
    mkdirSync(join(root, "backlog"), { recursive: true });
    writeFileSync(join(root, "backlog", "config.yml"), "statuses: not-a-list\n");
    const adapter = {
      ...fakeAdapter([]),
  ...
  test("an adapter with no pausedStatus method (Backlog, Jira) resolves without one — never a crash", async () => {
    mkdirSync(join(root, "backlog"), { recursive: true });
    writeFileSync(join(root, "backlog", "config.yml"), "statuses: not-a-list\n");
    const adapter = { ...fakeAdapter([]), statusFlow: async () => ["To Do", "In Progress", "Done"] };
    expect(adapter.pausedStatus).toBeUndefined();
  ...
  test("a linked task paused to the adapter's pausedStatus reconciles as in-progress instead of throwing (LCLI-455)", async () => {
    ...
    expect(target?.newTaskStatus).toBe("in-progress");
    expect(target?.rows).toEqual([
      { id: "LORE-1", title: "Title for LORE-1", status: "Blocked", file: "backlog/tasks/lore-1 - title.md" },
    ]);
  });
