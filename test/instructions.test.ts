...
describe("runInstructions — topic resolution", () => {
  ...
  test("agents topic explains the presence-based bridge scoping (LCLI-437)", () => {
    const { data } = instructions(["agents"]);
    const normalized = data.body.replace(/\s+/g, " ");
    expect(normalized).toContain("presence-based");
    expect(normalized).toContain("lore init --codex");
    expect(normalized).toContain("a repository that selected Codex alone");
    expect(normalized).toContain("An EXPLICIT, scoped request always means exactly what it asks for");
    expect(normalized).toContain('skill_source = "plugin"');
  });
