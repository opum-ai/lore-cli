...
describe("runCheck — exit codes and discovery", () => {
  ...
  test("an explicit path argument covering a git-ignored vendor directory never walks it (LCLI-379)", () => {
    // Reproduces opum-doc's symptom shape: a repo root containing a clean docs/ bundle beside a
    // large git-ignored non-bundle directory (that repo's real .herdr/, here a stand-in "vendor/").
    // `lore check .` (an explicit path covering BOTH) must not turn vendor/'s content into findings.
    Bun.spawnSync(["git", "init", "-q"], { cwd: root });
    writeFileSync(join(root, ".gitignore"), "vendor/\n");
    mkdirSync(join(root, "vendor", "nested"), { recursive: true });
    // A file that would register real errors/warnings if it were ever read as a bundle doc: a
    // broken internal link (error) inside a non-.md-extension-agnostic path lore would otherwise
    // walk unconditionally, plus a nested nonexistent nesting to prove directory-level pruning
    // (not just a same-name file-level filter) actually skips descending at all.
    writeFileSync(join(root, "vendor", "junk.md"), ref("Junk", "[ghost](./nope.md)."));
    writeFileSync(join(root, "vendor", "nested", "deep.md"), ref("Deep", "[ghost](./nope.md)."));

    const explicit = opts(["."], JSON_CTX);
    expect(runCheck(explicit)).toBe(EXIT_OK);
    const explicitReport = JSON.parse((explicit.stdout as ReturnType<typeof capture>).text()).data;
    expect(explicitReport).toMatchObject({ errorCount: 0, warningCount: 0 });
    expect(explicitReport.findings).toEqual([]);

    // AC4: the no-arg default (docs/ alone) and the explicit path covering the whole root (docs/
    // plus the now-excluded vendor/) report the identical skip count for the identical bundle,
    // rather than silently diverging depending on whether the invocation named a path.
    const bare = opts([], JSON_CTX);
    expect(runCheck(bare)).toBe(EXIT_OK);
    const bareReport = JSON.parse((bare.stdout as ReturnType<typeof capture>).text()).data;
    expect(explicitReport.skippedOutOfBundleLinkCount).toBe(bareReport.skippedOutOfBundleLinkCount);
    expect(explicitReport.fileCount).toBe(bareReport.fileCount);
  });

  test("gitignore-awareness also excludes an individually-ignored file inside an otherwise-tracked directory (LCLI-379)", () => {
    Bun.spawnSync(["git", "init", "-q"], { cwd: root });
    writeFileSync(join(root, "docs", ".gitignore"), "scratch.md\n");
    // Would register as a broken-link error if lore ever read it.
    writeFileSync(join(root, "docs", "scratch.md"), ref("Scratch", "[ghost](./nope.md)."));

    const explicit = opts(["."], JSON_CTX);
    expect(runCheck(explicit)).toBe(EXIT_OK);
    expect(JSON.parse((explicit.stdout as ReturnType<typeof capture>).text()).data).toMatchObject({
      errorCount: 0,
      warningCount: 0,
    });
  });
  ...
  test("falls back to the original unfiltered walk outside a git repository (never fails `lore check` for lacking one)", () => {
    // No `git init` in this root at all — gitIgnoredEntries must fail open, not throw.
    mkdirSync(join(root, "vendor"), { recursive: true });
    writeFileSync(join(root, "vendor", "notes.md"), ref("Notes", "Plain text, no links."));
    expect(runCheck(opts(["."]))).toBe(EXIT_OK);
  });
  ...
  test("LCLI-306: an unknown concept type is advisory normally and gates under --strict", () => {
    ...
    expect(ordinaryReport.data.findings).toContainEqual({
      ...
      file: "badtype/x.md",
      message:
        'unknown type "badtype" in badtype/x.md; validated on `type` only (known types: Epic, Arc, Spec, ADR, Runbook, Reference, Constitution)',
    });
...
describe("runCheck — status + managed-block drift (LORE-27)", () => {
  ...
  test("a linked task paused to the tracker's pausedStatus is status-drift (todo -> in-progress), not a thrown error (Quest 0.4.0's Blocked, LCLI-455)", async () => {
    writeDoc("stories/x.md", storyDoc("X", ["lore-1"], "todo"));
    const adapter = {
      ...fakeAdapter([makeTask("LORE-1", { status: "Blocked" })]),
      pausedStatus: async () => "Blocked",
    };
    const o = opts([], adapter);
    const code = await runCheck(o);
    const parsed = JSON.parse((o.stdout as ReturnType<typeof capture>).text());
