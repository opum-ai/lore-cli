...
const REJECTED_CONFIG_CONFORMANCE_V1: ReadonlyArray<{
  ...
  // "imprecise numeric page id retains the quote-it diagnostic" deliberately does NOT live in
  // this byte-compatible conformance table (LCLI-436): Bun 1.3.14 silently rounds this literal
  // and lore's own asPageId check reports the exact diagnostic above, but Bun 1.4.x's parser
  // rejects the literal at parse time before lore ever runs, with a different message entirely.
  // That is a real cross-runtime divergence, not something this table's toEqual byte-compatibility
  // promise can hold — see its own version-tolerant test below instead.
];
...
describe("loadConfig — malformed input and out-of-contract values", () => {
  test("malformed TOML surfaces the parser's reason without leaking its class name", () => {
    // Bun's own TOML parser message text is NOT lore's contract and has already changed once
    // across supported runtimes (LCLI-436: "Unexpected =" on 1.3.14 vs a differently-worded
    // "TOML Parse error: ..." on 1.4.x) -- only lore's OWN stable parts are pinned here: the
    // config-scoped prefix, a genuinely non-empty reason appended after it, and no leaked
    // internal class name (Bun's `BuildMessage`).
    const err = expectValidationError(() => loadConfig({ root: repoRoot("a = = 1"), env: {} }));
    const prefix = `${CONFIG_REL_PATH} is not valid TOML: `;
    expect(err.message.startsWith(prefix)).toBe(true);
    expect(err.message.length).toBeGreaterThan(prefix.length);
    expect(err.message).not.toContain("BuildMessage"); // never leaks Bun's internal class-name prefix
  });

  test("a multi-error TOML file flattens the parser's sub-messages (aggregate path)", () => {
    // Same version-tolerance as above (LCLI-436): whether Bun reports this as an aggregate of
    // several sub-messages (older Bun) or a single clear message (newer Bun), lore's own
    // contract is only the stable prefix plus a non-empty, single-line reason.
    const err = expectValidationError(() => loadConfig({ root: repoRoot("this is = = not ["), env: {} }));
    const prefix = `${CONFIG_REL_PATH} is not valid TOML: `;
    expect(err.message.startsWith(prefix)).toBe(true);
    expect(err.message.length).toBeGreaterThan(prefix.length);
    expect(err.message).not.toMatch(/[\r\n]/); // flattened to one line regardless of sub-message count
  });
...
describe("loadConfig — confluence.parent_page_id accepts integers", () => {
  ...
  test("an id beyond MAX_SAFE_INTEGER is rejected — by lore's own quote-it hint, or by Bun's own parser (LCLI-436)", () => {
    // Two observed behaviors across supported Bun versions for a bare, unquoted, imprecise TOML
    // integer literal: Bun 1.3.14's native parser silently rounds it to the nearest representable
    // double and hands lore a lossy `number`, which asPageId's own Number.isSafeInteger check
    // catches with a "quote it" hint; Bun 1.4.x's parser instead REJECTS the literal at parse
    // time, before lore's own validation ever runs, with its own precision message. Both are
    // stable, actionable validation errors; neither is a lore bug, so both are accepted here
    // rather than pinning one specific runtime's behavior.
    const err = expectValidationError(() =>
      loadConfig({ root: repoRoot("[confluence]\nparent_page_id = 9007199254740993\n"), env: {} }),
    );
    const lorePrecisionCheck = err.hint?.includes("quote") === true;
    const bunParseRejection = err.message.startsWith(`${CONFIG_REL_PATH} is not valid TOML: `);
    expect(lorePrecisionCheck || bunParseRejection).toBe(true);
  });
  ...
  test("a negative non-safe integer reports the sign, not the magnitude — or Bun's own parser rejects it first (LCLI-436)", () => {
    // Same version split as the positive case above: Bun 1.3.14 rounds this to a lossy negative
    // number, which asPageId's sign check (checked before magnitude) reports as "must be a
    // positive integer page id"; Bun 1.4.x's parser rejects the literal before lore ever sees a
    // value at all. Both are accepted; only lore's own would-be "too large" wording (which would
    // be WRONG here on 1.3.14 -- the sign check must win, not the magnitude one) stays forbidden
    // when lore's own validation is what actually ran.
    const err = expectValidationError(() =>
      loadConfig({ root: repoRoot("[confluence]\nparent_page_id = -9007199254740993\n"), env: {} }),
    );
    const bunParseRejection = err.message.startsWith(`${CONFIG_REL_PATH} is not valid TOML: `);
    if (!bunParseRejection) {
      expect(err.message).toContain("positive integer page id");
