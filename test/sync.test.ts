...
describe("lore sync — config is validated before spending any tracker task round-trip", () => {
  ...
  test("a linked task paused to the tracker's pausedStatus reconciles rather than throwing (Quest 0.4.0's Blocked, LCLI-455)", async () => {
    writeDoc("stories/x.md", storyDoc("X", ["lore-1"], "todo"));
    const adapter = {
      ...fakeAdapter([makeTask("LORE-1", { status: "Blocked", title: "Paused work" })]),
      pausedStatus: async () => "Blocked",
    };

    const result = await syncCmd([], adapter);
    expect(result.code).toBe(EXIT_OK);
    const updated = readDoc("stories/x.md");
    expect(updated).toContain("status: in-progress");
    expect(updated).toContain("Blocked");
  });
