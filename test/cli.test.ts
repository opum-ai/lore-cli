...
describe("cli — init dispatch", () => {
  ...
  test("the router wires stdinIsTTY + stderrIsTTY + a fake prompter through to the wizard end-to-end (LORE-260)", async () => {
    ...
    const prompter: InitPrompter = {
      ...
      // Unlike confirm/choose/ask above, the multiselect's own default is empty (LCLI-462 review
      // round 2) — a bare Enter there writes nothing, which would leave this test with no
      // filesystem side effect to prove the wizard actually ran. Explicitly select every offered
      // option instead, the deliberate choice this test needs, not "whatever the default was".
      multiselect: async (_q, options) => options.map((option) => option.value),
      close: () => {},
    };
    ...
    // Every confirm/choose call above resolves to its own default, and the multiselect above
    // explicitly selected Claude — the bridge was actually applied, the strongest proof the wizard ran.
...
describe("cli — schema dispatch", () => {
  ...
  test("`lore schema export` emits the schema.result envelope and writes schemas", () => {
    ...
    // 8 files for 7 types: Arc's deprecated `Story` alias owns one of its own (LCLI-553).
