...
describe("planBridge (pure) — write disciplines", () => {
  test("a fresh repo plans to create both files with their generated bytes", () => {
    const plan = planBridge({ skillOnDisk: null, claudeOnDisk: null, force: false, check: false, skillSource: "repo" });
  ...
  test("applying the plan then re-planning yields `unchanged` for both (idempotent)", () => {
    ...
    const second = planBridge({
      skillOnDisk: skillBytes,
      claudeOnDisk: claudeBytes,
      force: false,
      check: false,
      skillSource: "repo",
    });
  ...
  test("LORE-129: `--check --force` still reports `protected`, never `updated` — a check never writes", () => {
    const edited = `${buildSkillDoc()}\nhand edit`;
    const checkedAndForced = planBridge({
      skillOnDisk: edited,
      claudeOnDisk: null,
      force: true,
      check: true,
...
describe("lore agents — pretty-mode colour is pinned per BridgeAction (LORE-267, AC#1-3)", () => {
  test("bridgeActionColor maps every BridgeAction exactly: created/updated/removed green, unchanged dim, protected/orphaned yellow", () => {
    // Exhaustive over agent-bridge.ts's BridgeAction union (created | updated | unchanged |
    // protected | orphaned | removed, the last two added by LCLI-442). This test only pins today's
    // six values; the guard against a *future* action added to the union without a mapping update
    // is a compile-time one — `bridgeActionColor` (src/commands/agents.ts) is backed by a
    // `Record<BridgeAction, string>`, so `bun run typecheck` fails (TS2741, missing property) the
    // moment the union grows and this object literal doesn't, before any test ever runs. `lore
    // init`'s renderer (init.ts) imports this exact function, so pinning it here also pins init's
    // colour — the two commands cannot diverge again (AC#3).
    expect(bridgeActionColor("created")).toBe(ANSI.green);
    expect(bridgeActionColor("updated")).toBe(ANSI.green);
    expect(bridgeActionColor("removed")).toBe(ANSI.green);
    expect(bridgeActionColor("unchanged")).toBe(ANSI.dim);
    expect(bridgeActionColor("protected")).toBe(ANSI.yellow);
    expect(bridgeActionColor("orphaned")).toBe(ANSI.yellow);
  });
...
describe("lore agents — the reverse of LCLI-364: a Codex-only repository must not be handed an uninvited Claude bridge (LCLI-437)", () => {
  ...
  test("a Codex-only repository's OWN bridge drift still fails the gate — the check isn't just disarmed wholesale", () => {
    // Every drift test in the sibling "codex bridge" describe block above lays down BOTH bridges
    // first (agents() then withCodexBridge()), so none of them actually prove drift detection
    // survives on a repository that only ever selected Codex — the exact shape LCLI-437 is about.
    // hasClaudeBridge disarming the Claude half must not accidentally disarm codex drift detection
    // too.
    withCodexOnlyBridge();
    const codexSkillAbs = join(root, CODEX_SKILL_REL_PATH);
    writeFileSync(codexSkillAbs, `${buildCodexSkillDoc()}\ndrifted`);
    ...
    expect(code).toBe(6);
    expect(actionFor(result, CODEX_SKILL_REL_PATH)).not.toBe("unchanged");
    // Still no uninvited Claude bridge reported alongside the real codex drift.
    expect(result.files.some((f) => f.path === SKILL_REL_PATH)).toBe(false);
  });
});

describe("lore agents — skill_source = plugin opts SKILL.md out of repo generation (LCLI-442)", () => {
  /**
   * The opt-in: `.lore/config.toml`'s `[agents].skill_source = "plugin"` means the `opum-lore`
   * marketplace plugin owns `.claude/skills/lore/SKILL.md`, not this repository. A bare `lore
   * agents` (`includeCodex: true`) reads that config; a SCOPED `lore init --claude`/`--agents`
   * (`includeCodex: false`) never does — see the "explicit beats config" test below.
   */
  function withSkillSourcePlugin(): void {
  ...
  test("a leftover SKILL.md is reported `orphaned`, exits 6 under --check, and is left in place", () => {
    ...
    const { code, result } = agents(["--check"]);
    expect(code).toBe(6);
    expect(actionFor(result, SKILL_REL_PATH)).toBe("orphaned");
    expect(existsSync(skillAbs())).toBe(true);
  });

  test("orphaned drift names the file and the exact remedy, without relying on colour", () => {
    withSkillSourcePlugin();
    mkdirSync(dirname(skillAbs()), { recursive: true });
    writeFileSync(skillAbs(), buildSkillDoc());
    ...
    const plainOut = capture();
    runAgents({ root, output: PLAIN_CTX, args: ["--check"], stdout: plainOut });
    expect(plainOut.lines()).toContain(`orphaned ${SKILL_REL_PATH}`);

    const prettyOut = capture();
    runAgents({ root, output: { mode: "pretty", color: false }, args: ["--check"], stdout: prettyOut });
    expect(prettyOut.lines()).toContain(
      `  orphaned (config says the skill comes from the plugin; delete this file, or run lore agents --force to remove it) ${SKILL_REL_PATH}`,
    );
    expect(prettyOut.text()).toContain(
      "bridge carries a file config says should not exist — delete it, or run `lore agents --force` to remove it (exit 6)",
    );
  });

  test("a plain (non-check) run reports the drift but leaves the file untouched without --force", () => {
    withSkillSourcePlugin();
    mkdirSync(dirname(skillAbs()), { recursive: true });
    writeFileSync(skillAbs(), buildSkillDoc());
    const { code, result } = agents();
    expect(code).toBe(0); // a plain run is never a gate — --check is (mirrors `protected`'s discipline).
    expect(actionFor(result, SKILL_REL_PATH)).toBe("orphaned");
    expect(existsSync(skillAbs())).toBe(true);

    const stdout = capture();
    runAgents({ root, output: PLAIN_CTX, args: [], stdout });
    expect(stdout.text()).toContain('should not exist per agents.skill_source = "plugin"');
    expect(stdout.text()).toContain("lore agents --force");
  });

  test("--force removes an exact-byte-match leftover SKILL.md", () => {
    withSkillSourcePlugin();
    mkdirSync(dirname(skillAbs()), { recursive: true });
    writeFileSync(skillAbs(), buildSkillDoc());
    const { code, result } = agents(["--force"]);
    expect(code).toBe(0);
    expect(actionFor(result, SKILL_REL_PATH)).toBe("removed");
    expect(existsSync(skillAbs())).toBe(false);
    // Idempotent: a second run sees nothing left to remove.
    expect(actionFor(agents(["--check"]).result, SKILL_REL_PATH)).toBe("unchanged");
  });

  test("--force refuses to remove a hand-edited (differing) leftover SKILL.md", () => {
    withSkillSourcePlugin();
    mkdirSync(dirname(skillAbs()), { recursive: true });
    writeFileSync(skillAbs(), `${buildSkillDoc()}\nhand edit`);
    const { code, result } = agents(["--force"]);
  ...
  test("`--check --force` together still report `orphaned`, never `removed` — a check never writes", () => {
    withSkillSourcePlugin();
    mkdirSync(dirname(skillAbs()), { recursive: true });
    writeFileSync(skillAbs(), buildSkillDoc());
    const { code, result } = agents(["--check", "--force"]);
  ...
  test("explicit beats config: applyAgentsBridge's scoped (includeCodex: false) path ignores skill_source entirely", () => {
    // This is `lore init --claude`/`--agents`'s own call shape (see test/init.test.ts's end-to-end
    // equivalent). An explicit ask for the Claude bridge must never be silently disarmed by a config
    // value only the bare `lore agents` call consults.
    withSkillSourcePlugin();
    const result = applyAgentsBridge({ root, force: false, check: false });
    expect(actionFor(result, SKILL_REL_PATH)).toBe("created");
    expect(existsSync(skillAbs())).toBe(true);
  });

  test("with no skill_source configured, the pre-existing repo-owned default is unchanged", () => {
    const { result } = agents();
    expect(actionFor(result, SKILL_REL_PATH)).toBe("created");
    expect(existsSync(skillAbs())).toBe(true);
  });
