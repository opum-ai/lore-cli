...
- Published on npm as **`@opum-ai/lore@0.4.4`** (bin `lore`) with six
...
> **Status: 0.4.4 released.** Tag `v0.4.4`, the qualified workflow artifacts,
> all seven public `@opum-ai/lore*` npm packages, and a clean registry install
> agree on `0.4.4`. Trusted Publishing is configured for every package; the
