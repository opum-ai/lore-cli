...
- Published on npm as<!--lore-version:published-bullet:begin--> **`@opum-ai/lore@0.9.0`** (bin `lore`) with six
...
> **Status:<!--lore-version:status:begin--> 0.9.0 released.** Tag `v0.9.0`, the qualified workflow artifacts,
> all seven public `@opum-ai/lore*` npm packages with `latest` moved on each,
> and a clean-registry install agree on `0.9.0`.<!--lore-version:status:end-->
...
> measured by `opum-cli-e2e`, not implied by either version string. "The pair"
> means the two CLIs are released and qualified TOGETHER, not that
