# Handover — third backlog campaign, cursor at LCLI-77 (LCLI-69..87)
...
Cursor: LCLI-77 — lore init follows pre-existing symlinks at scaffold paths,
escaping the repo root (security-labeled, sixth security task this campaign,
the SIBLING of just-resolved LCLI-76). Queue order confirmed by user on
...
CRITICAL: read doc-1's LCLI-76 campaign conventions before implementing —
LCLI-76 just added `assertNoSymlinkInPath` to src/commands/fswrite.ts for
EXACTLY this same vulnerability class in `lore scaffold`; LCLI-77 is very
...
| Item | Status |
| --- | --- |
| Tracker doc | doc-1, updated this session (Cursor → LCLI-77, Queue = 8 items, LCLI-76 moved to Resolved with its clean review documented, three new campaign conventions recorded, one new Not-queued follow-up) |
| Review doc | doc-2, full Codex second-opinion review (201 confirmed findings, 25/25 clusters) — source of all queued tasks |
| Queue | 8 tasks remaining (LCLI-77, 73, 74, 75, 80, 79, 78, 81), all `To Do`, `bug`, `High` priority, each with AC + a `--ref` to doc-2 |
| Branch | `dev`, clean (0 unpushed after this session's final push) |
| Leftover branches/PRs | none — `feature/LCLI-76` fully merged (PR #75, rebase-merged) and pruned (local + remote, confirmed via `git fetch --prune`) |
| Not queued | LCLI-42/43/44/45 (deferred) plus six unfiled follow-up candidates: two from LCLI-84, one from LCLI-69, two from LCLI-72, one from LCLI-76 (a narrow, explicitly non-blocking TOCTOU gap in `writeAllOrRollback`'s `--force` branch) |

## Next steps

1. Run the per-issue lifecycle on **LCLI-77** (`lore init` follows
   pre-existing symlinks, security-labeled): branch `feature/LCLI-77` off
   ...
   - `src/commands/init.ts:60-61` — `for (const dir of plan.dirs) {
     ...
     NOT routed through `writeAllOrRollback` (confirmed by LCLI-76's own
   ...
   - `src/commands/fswrite.ts` — **LCLI-76 just added `assertNoSymlinkInPath`
     ...
     fix for LCLI-77 is almost certainly: export `assertNoSymlinkInPath` from
...
2. **AC2** needs a test in whatever file covers `init.ts` (likely
   ...
   add the missing directory-symlink case (mirroring LCLI-76's own two new
...
3. Update doc-1's Cursor/Queue/Resolved/Session-log sections on the feature
   ...
   **LCLI-73** (item #2 of the remaining queue).
4. Archive this handover to `archive/handovers/` and write the next one for
   LCLI-73. Note: today's date (`2026-07-21`) already has ELEVEN prior
...
- **This is the SIXTH security-labeled task this campaign**, and the direct
  sibling of LCLI-76 (just resolved) — same vulnerability class, different
  command. LCLI-76's independent review was the FIRST review this campaign
  ...
  round on LCLI-77 rather than assuming a rubber stamp, since it's a
...
- **`.repro-scratch/` keeps accumulating scratch files from every security
  review** (LCLI-85, LCLI-69, LCLI-72, LCLI-71, LCLI-76 all left files
...
- **A reviewer-suggested "nearly free" improvement using an existing helper
  ...
  already-tested invariants first (LCLI-76's own TOCTOU-follow-up decision).
  Applies in reverse here too: reusing `assertNoSymlinkInPath` for LCLI-77
...
- Don't assume a fix that passes self-review and its own tests is done on a
  ...
  LCLI-76's review found nothing, that was still real work well spent, not a
...
- Don't write a near-duplicate symlink-guard function when LCLI-76 already
