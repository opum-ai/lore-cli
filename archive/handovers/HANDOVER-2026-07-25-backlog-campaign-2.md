# Handover — round 5 init: round-4 review residue & release hardening (waves: 0, issues: LCLI-265/267/268/266 queued, none started)
...
KNOWN CONFLICT: LCLI-266 and LCLI-267 both touch src/commands/agents.ts AND
test/agents.test.ts — they cannot share a wave. LCLI-265 (docs-only) and
LCLI-268 (release.yml + its test + runbook) are disjoint from everything.
Likely shape: wave 1 = {265, 267, 268}, wave 2 = {266}. RECOMPUTE, don't trust.

LCLI-268 has a repo-admin half the agent must NOT self-authorize (creating the
...
Locked facts: LCLI-253 stays blocked on the upstream Backlog.md --json tag
(latest is still v1.48.0; LCLI-254's daily watcher opens a GitHub issue when it
lands — check `gh issue list` each restore). LCLI-257 is needs-human (repo-admin
ruleset). LCLI-42/43/44/45 are deferred-v2. The `backlog` on PATH is a locally
...
| Queued | 4: LCLI-265, LCLI-267, LCLI-268, LCLI-266 |
...
| New task filed at init | **LCLI-268** (publish-job hardening), from round 4's unfixed LCLI-255 exposure |
...
2. Recompute the ready/conflict graph live. Expect wave 1 = {LCLI-265, LCLI-267, LCLI-268}; verify the 266↔267 edge still holds.
3. Dispatch wave 1. **Put the `[Unreleased]` CHANGELOG requirement in every worker prompt** — see traps.
4. Designate exactly **one** worker per wave to run the docker e2e harness. For wave 1 that is most likely LCLI-268 (it is the only one touching a CI/release contract); LCLI-265 is docs-only and LCLI-267 is a colour change, so neither needs it.
5. Wave 2 = LCLI-266 alone. Its AC#3 may widen into `src/commands/rename.ts` / `src/commands/sync.ts` — both call `assertNoSymlinkInAnyPath` (verified at init).
...
- **LCLI-268's scope split is load-bearing**: the workflow-side change is agent-doable; creating the GitHub Environment and its required reviewers is repo-admin and must not be self-authorized (same boundary as LCLI-196/LCLI-257). An in-workflow `if: github.ref == …` guard is **not** a mitigation — it lives in the same file an attacker replaces.
...
- **Don't rely on a task's cluster label as proof two tasks are safe to parallelize.** Different cluster is not sufficient — do the file-citation read. (LCLI-266/267 are a live example: different-sounding concerns, same two files.)
