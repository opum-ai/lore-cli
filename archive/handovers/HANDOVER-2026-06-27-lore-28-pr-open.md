# Handover — LCLI-28 links.ts delivered as open PR #19 (awaiting Jeremy's review/merge)

**Date**: 2026-06-27 | **Grounded against**: branch `feat/lore-28-links`=`c38448e` (==origin, clean tree); `dev`=`ad8c5fe` (==origin, unchanged) | **Backlog**: LCLI-28 **In Progress** (both ACs checked)

## Paste-ready prompt for the next session

```
LCLI-28 (core/links.ts — portable cross-link normalize + validate) is COMPLETE and shipped as
...
dev = ad8c5fe (unchanged — PR not merged). LCLI-28 is In Progress with BOTH ACs checked; it stays
...
A) IF PR #19 IS MERGED (squash → one feat(LCLI-28) commit on dev): do the post-merge chores,
   mirroring the LCLI-47 pattern (commits 3f5423a + 987e0df):
   1. ff dev locally (ssh is DOWN — use gh-token route, see traps).
   2. `backlog task edit LCLI-28 -s Done` and add a "delivered via #19" note.
   3. Archive consumed handovers on dev (separate `docs:`/`chore:` commit, NOT on a feature branch):
      ...
      is consumed by LCLI-28) AND this file into archive/handovers/, then commit + push via gh-token.
...
B) IF PR #19 IS NOT MERGED: do not nag. Either address any review comments Jeremy left on #19, or
   ...
   frontmatter grep which mis-parses): LCLI-26 lore-sync is STILL BLOCKED — its chain is
   LCLI-26 → {22,23,24} → 21 → 5 → the Backlog --json fork (LCLI-1). Genuinely-unblocked HIGH work:
   - LCLI-30 lore check (drift gate + link/anchor + portability lint) — the NATURAL next: it
     ...
     scan. Best continuation of LCLI-28.
   - LCLI-29 index.md/log.md generation (dep LCLI-47 Done) — also unblocked; consumes normalizeLink.
   - LCLI-1 (fork Backlog --json) — the only path that eventually unblocks LCLI-26/the adapter track.
...
| Item | Status |
| --- | --- |
| **PR #19** (`feat(LCLI-28): links.ts`) | **OPEN** into `dev`, **MERGEABLE**, all 4 CI checks **pass** (head `c38448e`) |
| Branch `feat/lore-28-links` | `c38448e` == origin; clean tree; checked out |
| `dev` | `ad8c5fe` == origin (unchanged; PR unmerged) |
| LCLI-28 | **In Progress** — AC#1 ✔ AC#2 ✔; full notes recorded; → **Done** only after #19 merges |
...
| LCLI-26 lore sync | still **blocked** (chain bottoms at LCLI-1 fork) |
| LCLI-30 / LCLI-29 / LCLI-1 | **To Do**, unblocked — candidate next tasks |

## Next steps

1. `backlog instructions overview`; `gh pr view 19 --json state` → branch on merged vs not (paste prompt A/B).
2. If merged: ff dev (gh-token), mark LCLI-28 Done + "delivered via #19", archive both consumed handovers on dev, pick next.
3. If not merged: address #19 review comments, or start LCLI-30 (composes this PR's `validateLink`) / LCLI-29 / LCLI-1 — ask Jeremy.
...
- **Merge convention = SQUASH** → one `feat(LCLI-28): … (#19)` commit on dev; ALL branch commits
  (feat/fix/chore/docs) collapse into it. Post-merge chores (mark Done, archive handovers) are
  SEPARATE commits on dev, mirroring LCLI-47's `3f5423a`/`987e0df`. [[lore-git-workflow]]
...
- **LCLI-28 scope was deliberately thin**: it ships the per-link primitives only. The graph-wide
  `validateLinks(g)`+anchors (remark-validate-links) → **LCLI-30**; `rewriteInbound(g,from,to)` (composes
  `normalizeLink`) → **LCLI-35**; body-text wikilink/Obsidian/MDX scan → **LCLI-30**. Don't re-scope these
...
- **Don't trust `grep dependencies backlog/tasks/*.md`** — it returned "all unblocked" (false). Use
  `backlog task view LORE-N --plain`. LCLI-26 is genuinely blocked behind the LCLI-1 fork chain.
...
- **LCLI-28** Backlog: status In Progress, **AC#1 + AC#2 checked**, plan recorded, full impl + the
  10-finding `/code-review max` disposition appended as notes.
- **CHANGELOG.md** (Unreleased / LCLI-28): added the `core/links.ts` entry incl. the `resourceFor`
