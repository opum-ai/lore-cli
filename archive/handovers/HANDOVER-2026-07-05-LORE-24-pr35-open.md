# Handover — LORE-24 (`lore link`/`unlink`) PR #35 open, 2nd code-review pass pending a fix decision

**Date**: 2026-07-05 | **Grounded against**: `feat/lore-24-link-unlink`@`5caec0c` (dev@`5b8921a`, unmoved) | **Backlog**: LORE-24 **In Progress** (both ACs checked, PR open, 2nd review triage recorded); LORE-26 **To Do** (blocked on LORE-24 merging+Done); LORE-27 **To Do** (already unblocked, independent of LORE-24)
...
and on LORE-24's task notes. NONE of these 10 are fixed yet. The user was asked
...
prioritize those first —
  ...
  cleanup/DRY) are listed in full on the PR comment and LORE-24's notes — don't
...
to the standard finalize path (git pull --ff-only on dev, mark LORE-24 Done, pick
LORE-26 or LORE-27 next — see predecessor context below, still accurate).
...
ADR-0009 semantics yet — implement it + amend ADR-0009 §3 as part of LORE-26's work.
...
| **LORE-24** (`lore link`/`unlink`) | **In Progress** (not Done — mark Done only after #35 merges+promotes). Both ACs checked; 1st-pass review (6 findings, fixed) and 2nd-pass review (10 findings, **not yet fixed**) both recorded on task notes. |
...
3. Once #35 merges: `git pull --ff-only` on `dev`, mark LORE-24 Done, pick LORE-26 or LORE-27.
4. For LORE-26 specifically: resolve the `reconcile.overrides` design question (locked decision above) before wiring `reconcileStatus` in.
5. LORE-26 needs a way to read `backlog/config.yml`'s `statuses:` into a `StatusFlow` — no adapter method reads Backlog config yet; decide whether that's a new `BacklogAdapter` method or a command-layer YAML read (js-yaml already pinned, [[lore-serialization-invariants]]).
...
- **LORE-24** → 2nd-pass `/code-review max` triage (10 findings) appended via `--append-notes`.
...
- **Predecessor handover** `HANDOVER-2026-07-04-LORE-24-pr35-open.md` (superseded by this one, same topic) → archived to `archive/handovers/` this session.
