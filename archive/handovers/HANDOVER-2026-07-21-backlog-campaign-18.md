# Handover — third backlog campaign, cursor at LORE-78 (LORE-69..87)
...
Cursor: LORE-78 — lore rename's destination id is not validated for `..` path
...
(LORE-80→79→78→81 — LORE-80 and LORE-79 are both Done). Queue order confirmed
by the user on 2026-07-21 ("Use this order (Recommended)"); do not re-ask
before taking the next item. 1 issue remains in the queue after LORE-78
(LORE-81).

LORE-78's own AC: "#1 The destination id argument is validated to reject `..`
...
(`backlog task view LORE-78 --plain`) — do not trust this summary alone.

CRITICAL — apply this session's own newly-recorded campaign convention before
writing any new code: LIVE-CLI-VERIFY LORE-78's own repro FIRST, before
assuming it's still an open gap. LORE-79 (this session) found that LORE-80
...
confinement check (`assertDestinationConfined`, LORE-79, checked immediately
...
argument-parsing layer LORE-78 is scoped to even exists as a distinct check
today. Confirm empirically (`lore rename reference/orders foo/../../pwned`
against a scratch bundle) whether this already satisfies LORE-78's AC in
substance, or whether LORE-78 still has independent value (e.g. rejecting at
the TRUE argument-parsing boundary — `parseRenameArgs`/`args.ts` itself,
before `runRename`'s body runs at all — vs. LORE-79's check, which runs
inside `runRename` after parsing but is still "before command execution" in
every practical sense). Read LORE-79's task notes/final summary AND the diff
...
ALSO CRITICAL — same session's convention: when LORE-78 needs its own
...
logic a third time. If LORE-78's own scope is genuinely just "the `newId`
...
also handles absolute-path forms already covered by LORE-79/80) — read the
task's own AC wording carefully; it's narrower than LORE-79/80's own scope
...
across LORE-74 (after a correction), LORE-75, LORE-80, and LORE-79; don't
regress on it.

Also read the tracker's (doc-1) Not-queued section before finishing: LORE-79's
...
`docs/`, bypassing every lexical id-based check LORE-78/79/80 collectively
implement (none of them validate resolved filesystem identity, only the id
string). This mirrors LORE-76/77's already-fixed symlink-following gap in
scaffold/init (`assertNoSymlinkInPath`) — `rename.ts` (and `new.ts`) never
got the equivalent guard. NOT in scope for LORE-78/81 (a distinct fix, a
distinct code path) — just be aware it exists and don't let LORE-78's own
...
| Item | Status |
| --- | --- |
| Tracker doc | doc-1, updated this session (Cursor → LORE-78, Queue = 1 item, LORE-79 moved to Resolved with its evidence + review outcome, two new campaign conventions recorded, two new Not-queued follow-up candidates added from LORE-79's review) |
| Review doc | doc-2, full Codex second-opinion review (201 confirmed findings, 25/25 clusters) — source of all queued tasks |
| Queue | 1 task remaining after LORE-78 (LORE-81), `To Do`, `bug`, `High` priority, with AC + a `--ref` to doc-2 |
...
| Leftover branches/PRs | none — `feature/LORE-79` fully merged (PR #81, rebase-merged) and pruned (local + remote, confirmed via `git fetch --prune` + `git branch -a`) |
| Not queued | LORE-42/43/44/45 (deferred) plus all prior follow-up candidates, plus two new ones from LORE-79: (1) a symlink-based filesystem escape in `lore rename` (see "CRITICAL" block above — the more severe one), and (2) a minor empty/self-cancelling-`newId` edge case in `escapesRoot` (renames to a hidden `docs/..md`, no security escape, just confusing) |

## Next steps

1. Run the per-issue lifecycle on **LORE-78** (`lore rename` destination id not validated for `..` traversal at the argument-parsing layer): branch `feature/LORE-78` off `dev`, read the task's AC (`backlog task view LORE-78 --plain`), **live-CLI-verify its own repro first** (per this session's newly-recorded convention — LORE-79's own check may have already substantially closed this at a different layer; confirm empirically before assuming either way), implement, verify, review, PR, self-merge, prune.
2. Read **LORE-81** (already queued next, "renaming FROM the reserved root index") before finishing LORE-78 — per doc-1's own recorded convention, LORE-79/80's fixes may have already substantially reduced or reshaped LORE-78's remaining scope.
3. Update doc-1's Cursor/Queue/Resolved/Session-log sections on the feature branch before merging (per the skill's step 4), advancing the cursor to **LORE-81** — the next (and last) queue item; re-confirm against the tracker's own Queue table at restore time in case of drift.
4. Run the lifecycle's step 6 independent review (`general-purpose` subagent) **after** committing the fix and its tests, **then** write the outcome into the tracker — the ordering LORE-74 (after a correction), LORE-75, LORE-80, and LORE-79 all followed cleanly. Keep following it.
...
- **`gh pr merge --rebase --delete-branch` auto-switches you off the feature branch** when it's the currently-checked-out one, and this session it also fully removed the LOCAL `feature/LORE-79` branch automatically (no separate `git branch -d` was needed) — verify with `git branch -a` + `git fetch --prune` rather than assuming a manual prune step is always required.
- **When a new layer's containment check sits above/below an already-fixed layer, reuse its exported check rather than re-deriving it** (LORE-79 exported `escapesRoot` from `core/rewrite.ts` this session specifically for this). Check whether it already fits before writing new segment-walk logic for LORE-78/81.
...
- Don't assume a task's own filed repro is still an open gap — LORE-79's own filing's exact escape was already closed by LORE-80's engine-layer fix by the time LORE-79 was picked up; live-CLI-verify before assuming either way (new this session, directly relevant to LORE-78 next).
