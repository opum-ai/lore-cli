# Handover — third backlog campaign, cursor at LCLI-73 (LCLI-69..87)
...
Cursor: LCLI-73 — lore replace can corrupt lore:tasks managed blocks
...
since LCLI-72. Queue order confirmed by user on 2026-07-21 (independent fixes
first, the LCLI-78/79/80 rename-traversal cluster last); do not re-ask. Merge
...
| Item | Status |
| --- | --- |
| Tracker doc | doc-1, updated this session (Cursor → LCLI-73, Queue = 7 items, LCLI-77 moved to Resolved with its clean review documented, two new campaign conventions recorded, one new Not-queued follow-up) |
| Review doc | doc-2, full Codex second-opinion review (201 confirmed findings, 25/25 clusters) — source of all queued tasks |
| Queue | 7 tasks remaining (LCLI-73, 74, 75, 80, 79, 78, 81), all `To Do`, `bug`, `High` priority, each with AC + a `--ref` to doc-2 |
| Branch | `dev`, clean (0 unpushed after this session's final push) |
| Leftover branches/PRs | none — `feature/LCLI-77` fully merged (PR #76, rebase-merged) and pruned (local + remote, confirmed via `git fetch --prune`) |
| Not queued | LCLI-42/43/44/45 (deferred) plus seven unfiled follow-up candidates: two from LCLI-84, one from LCLI-69, two from LCLI-72, one from LCLI-76 (TOCTOU), one from LCLI-77 (`new.ts`/`agents.ts`/`sync.ts`/`schema.ts`/`rename.ts` still unguarded by the symlink check LCLI-76/77 added) |

## Next steps

1. Run the per-issue lifecycle on **LCLI-73** (`lore replace` can corrupt
   `lore:tasks` managed blocks, correctness — NOT security-labeled, but the
   same lifecycle applies): branch `feature/LCLI-73` off `dev`, read the
   ...
   - `src/core/replace.ts:88-90` — `MANAGED_MARKERS` currently has exactly
     ...
     LCLI-22's `managed-block.ts`, which shipped AFTER `replace.ts`) was
   ...
   - `src/core/replace.ts:106-115`'s `managedRanges` already calls the
     ...
     LCLI-86-hardened to fail loud on a malformed marker pair) generically
...
3. Update doc-1's Cursor/Queue/Resolved/Session-log sections on the feature
   ...
   the NEXT item in the queue (LCLI-74 per the last-read order, but
...
- **LCLI-73 is NOT security-labeled** (`codex-review, correctness`) — the
  first non-security task since LCLI-72. Still run the full lifecycle
...
- **The last six tasks in a row (LCLI-85, 69, 72, 71, 76, 77) were all
  ...
  Don't let LCLI-73 being "just correctness" lower the bar on verification
...
- **`.repro-scratch/` keeps accumulating scratch files from every review**
  (LCLI-85, 69, 72, 71, 76, 77 all left files there) — all harmless,
...
- Don't trust a module docstring's forward-looking design claim ("the
  ...
  this handover did (and what LCLI-73's own filing task correctly
