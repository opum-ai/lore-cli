...
label): (1) LORE-236 (cmd-meta-a) and LORE-249 (errors-output-git) BOTH edit src/errors.ts →
serialize them into different waves. (2) LORE-235 (cmd-meta-a) is now UNBLOCKED (its wave-24
blocker LORE-233 merged) but MUST import mapWithConcurrency + TASK_DETAILS_CONCURRENCY from
the NEW src/commands/concurrency.ts (LORE-233 relocated them there; reconcile-shared.ts only
re-exports them for back-compat). Also: LORE-237 (validate.ts) is PARTIALLY pre-satisfied by
merged LORE-228 (which already added --strict=<value> rejection) — 237's residual is
repeated-`--strict`/repeated-`--type` rejection; read validate.ts on dev before dispatching.
Filename trap: LORE-232 = src/commands/query.ts, LORE-246 = src/core/query.ts — DIFFERENT
...
| Wave 24 (this session) | LORE-222,197,229,230,233,238 merged (PR#217-222), integration review clean |
...
- **cmd-crud-b**: LORE-231 (src/commands/fswrite.ts — writeFileAtomic leaks an uncleaned temp file on mid-write failure), LORE-232 (src/commands/query.ts — trim --type/--status/--tag values). Same cluster ⇒ serialize.
- **cmd-link**: LORE-234 (src/commands/link.ts — runLink doc-membership check is exact-case while unlink's is case-insensitive → casing-variant entry duplicates instead of dedups).
- **cmd-meta-a**: LORE-235 (src/commands/tasks.ts — bound resolveRollup fan-out; **import the shared cap from src/commands/concurrency.ts**, NOT reconcile-shared.ts), LORE-236 (src/errors.ts — strip ANSI/OSC/control on WarningCollector.flush stderr; **shares errors.ts with LORE-249**). Same cluster ⇒ serialize.
- **cmd-meta-b**: LORE-237 (src/commands/validate.ts — reject repeated `--strict`/repeated `--type`; the `--strict=<value>` half is ALREADY done by merged LORE-228 — scope to the residual).
- **core-bundle-check**: LORE-239 + LORE-240 (src/core/check.ts — callout false-positive; leading indented code block), LORE-241 + LORE-242 (src/core/profile.ts — non-object profile.json error; validate default vs kind/enum). 239/240 same file; 241/242 same file; all same cluster ⇒ one per wave.
- **core-engine-a**: LORE-243 (src/core/log.ts — harden resolveRoot against equivalent bundle roots).
- **core-index-context**: LORE-244 (src/core/indexes.ts — index.md conceptTitle numeric/boolean coercion via frontmatterScalar).
- **core-links-resolution**: LORE-245 (src/core/links.ts — validateLink flags bare '.'/'..' destinations).
- **core-query-validate**: LORE-246 (src/core/query.ts — matchesField resolves case-insensitive key across ALL variants). NOTE: core/query.ts ≠ commands/query.ts (LORE-232).
- **core-rewrite-engine**: LORE-247 (src/core/rewrite.ts — preserve above-repo-root outbound links during rename).
- **core-scaffold-consumer**: LORE-248 (src/core/schema.ts — warnSummary counts UTF-16 not chars for non-BMP).
- **errors-output-git**: LORE-249 (src/errors.ts — stderrHint control-seq strip + length cap; **shares errors.ts with LORE-236**), LORE-250 (src/output.ts — suppress ANSI on non-TTY stderr). Same cluster ⇒ serialize.

An illustrative ≤6-worker wave 25 (RECOMPUTE LIVE — this is NOT a plan): one-per-cluster, file-disjoint, ordinal order → LORE-231, 234, 235, 237, 239, 243 (cmd-crud-b / cmd-link / cmd-meta-a / cmd-meta-b / core-bundle-check / core-engine-a). Respect the two conflicts in the paste-ready prompt.
...
- **Cross-cluster same-file conflict this round**: LORE-236 (cmd-meta-a) + LORE-249 (errors-output-git) BOTH edit `src/errors.ts` → never co-schedule. Same-cluster/same-file pairs: fswrite is done, but 231 (fswrite.ts) vs the merged 230 is fine (230 merged); 239/240 (core/check.ts), 241/242 (core/profile.ts) same file; 231/232 same cluster cmd-crud-b; 235/236 same cluster cmd-meta-a; 249/250 same cluster errors-output-git.
- **LORE-233 relocated the concurrency helper**: `mapWithConcurrency` + `TASK_DETAILS_CONCURRENCY` now live in `src/commands/concurrency.ts` (dependency-free, avoids the link→reconcile-shared→link cycle). `reconcile-shared.ts` re-exports them for back-compat (its own resolveTaskDetails + check.ts's probeLiveness are the live consumers). LORE-235 should import from `./concurrency`.
...
- **Don't treat the tracker `[file]` Note as the edit target** — it's audit provenance. Read the task BODY/ACs for the real file(s). (Wave 24 caught LORE-236's real target = errors.ts, not tasks.ts, this way.)
- **Don't co-schedule LORE-236 with LORE-249** (both edit errors.ts) or any same-cluster pair — file-citation read first.
- **Don't dispatch LORE-235 assuming the concurrency helper is in reconcile-shared.ts** — it moved to src/commands/concurrency.ts (LORE-233).
- **Don't dispatch LORE-237 for the full arg-parser hardening** — the `--strict=<value>` half is already merged (LORE-228); scope the worker to the repeated-flag residual.
