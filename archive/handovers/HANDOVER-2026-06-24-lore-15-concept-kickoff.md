# Handover — core library kickoff: build LCLI-15 concept.ts (LCLI-12 output layer DONE)

**Date**: 2026-06-24 | **Grounded against**: `dev`=`8c797f9` (== origin/dev, clean tree; **no open PRs**) | **Backlog**: M0 + core-infra Done (LCLI-6/7/8/10/11/12); **LCLI-15 is next, unblocked** (dep LCLI-11 Done)

## Paste-ready prompt for the next session

```
Continue building lore (OKF-native docs CLI, Bun+TS). The infra layer is DONE on dev — errors.ts
(LCLI-11), config.ts (LCLI-10), output.ts (LCLI-12), plus M0 scaffolding/CI. NO open PRs; dev clean.

NEXT = LCLI-15 (src/core/concept.ts): a single .md file ⇄ a typed Concept, with byte-stable
serialization. Unblocked (dep LCLI-11 Done). Claim first:
  backlog task edit LCLI-15 -s "In Progress" -a @claude
...
LCLI-15 scope (confirm via `backlog task view LCLI-15 --plain`): Concept {id, path, type, frontmatter,
...
§2.1). schema.ts (Zod source of truth) may be split out — check the design module tree; LCLI-15's
...
mark Done ON MERGE via a direct-to-dev chore commit (LCLI-10/12 precedent). CHANGELOG (Unreleased).
...
| LCLI-12 — output layer | **Done** (merged via PR #11, squash `01d3eb4`; Done `cd9c10e`). No follow-up |
| LCLI-10 / LCLI-11 | **Done** (config.ts / errors.ts on dev) |
| LCLI-15 — concept.ts | **To Do — NEXT** (unblocked; dep LCLI-11 Done; milestone m-2) |
| LCLI-16 (bundle.ts) → LCLI-17 (lore init) | To Do, **blocked** behind LCLI-15 (then LCLI-16) |

## Next steps

1. Build **LCLI-15** (`src/core/concept.ts`) off `dev` → PR into dev. Claim first; read design §2.1/§6/§9 + ADR-0011.
2. Then LCLI-16 (`bundle.ts`, dep LCLI-15), then LCLI-17 (`lore init`, dep 15+16). M2/Backlog-adapter tasks (LCLI-21+) stay blocked on BJP (the Backlog.md `--json` fork).
...
- **/code-review max needs 3-4 passes to converge** on a foundational module — LCLI-12 took FOUR (each round found defects in the prior round's fixes; a finding repeated across rounds = act, don't re-defend). Decline latent/no-caller-yet findings with written rationale. Auto-memory: `code-review-vs-review-command`.
...
- **Don't mark a Backlog task Done at PR-open** — In Progress until merge (LCLI-10/12 precedent).
...
- **LCLI-12 fully recorded and complete** — plan, ACs #1/#2, implementation + declined-findings notes, final summary, PR #11 link; status Done. CHANGELOG (Unreleased) entry merged to dev. Nothing pending.
- **Auto-memory:** `code-review-vs-review-command` gained the LCLI-12 4-pass-convergence data point.
- **dev (direct housekeeping):** consumed handovers `lore-12-output-layer` (a83baf5) and `lore-12-pr-open` (8c797f9) archived. This handover is forward-looking (LCLI-12 work is closed; it kicks off LCLI-15).
