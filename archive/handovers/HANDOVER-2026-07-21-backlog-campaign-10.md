# Handover — third backlog campaign, cursor at LORE-71 (LORE-69..87)
...
Cursor: LORE-71 — `lore check --external` is vulnerable to SSRF via
...
CRITICAL: read doc-1's three newest campaign conventions (added LORE-69/72,
...
| Item | Status |
| --- | --- |
| Tracker doc | doc-1, updated this session (Cursor → LORE-71, Queue = 10 items, LORE-72 moved to Resolved with its review findings documented, three new campaign conventions recorded, two new Not-queued follow-up entries) |
| Review doc | doc-2, full Codex second-opinion review (201 confirmed findings, 25/25 clusters) — source of all queued tasks |
| Queue | 10 tasks remaining (LORE-71, 76, 77, 73, 74, 75, 80, 79, 78, 81), all `To Do`, `bug`, `High` priority, each with AC + a `--ref` to doc-2 |
| Branch | `dev`, clean (0 unpushed after this session's final push) |
| Leftover branches/PRs | none — `feature/LORE-72` fully merged (PR #73, rebase-merged) and pruned (local + remote, confirmed via `git fetch --prune`) |
| Not queued | LORE-42/43/44/45 (deferred) plus five unfiled follow-up candidates: two from LORE-84 (rewriteInbound's profile gap; lore check's separate validation path), one from LORE-69 (commitBacklogFiles's guard is POSIX-only via `posix.normalize`), two from LORE-72 (a symlink-read gap in `lore new --template`, read-path counterpart to the still-open LORE-76/77; a profile-declared-template traversal, deliberately excluded scope) |

## Next steps

1. Run the per-issue lifecycle on **LORE-71** (`lore check --external` SSRF,
   security-labeled): branch `feature/LORE-71` off `dev`, read the task's AC,
...
4. **AC3**: add a test using the existing injectable `FetchLike` fake to
   ...
   fails.
5. Apply the freshly-recorded campaign convention from LORE-69/72's sessions:
   ...
   value, use a different value downstream" gap LORE-69 found (e.g.
...
6. Update doc-1's Cursor/Queue/Resolved/Session-log sections on the feature
   ...
   **LORE-76** (item #2 of the remaining queue).
7. Archive this handover to `archive/handovers/` and write the next one for
   LORE-76. Note: today's date (`2026-07-21`) already has NINE prior archived
   handovers (base, `-2` through `-9`) — this session's own archival will
   need suffix `-10`.

## Critical context / traps

- **This is the FOURTH security-labeled task this campaign** (after LORE-85,
  LORE-69, LORE-72). Both prior sessions' independent adversarial reviews
  found REAL issues: LORE-69's review found a live bypass of the first fix
  attempt (fixed before merge); LORE-72's review found no bypass of the
  ...
  review as a formality for LORE-71 or any remaining security task
  (LORE-76/77 symlink escapes, LORE-75 destructive deletion) — explicitly
...
- **When an out-of-scope finding surfaces during review**: this campaign's
  now-established decision rule (doc-1's Campaign conventions, LORE-72
  session) is — same code this task already touches + well-precedented fix
  → fix in-task (LORE-69's `porcelainPaths` defense-in-depth did this);
  ...
  silently expand scope or silently ignore it (LORE-72's symlink gap did
  this). Also: before assuming a finding overlaps an already-queued item,
  actually read that item's own AC wording, not just its title — LORE-72's
  symlink finding looked like it might overlap LORE-76/77 but turned out to
...
- **`.repro-scratch/` keeps accumulating scratch files from every security
  review** (LORE-85, LORE-69, LORE-72 all left files there) — all harmless,
