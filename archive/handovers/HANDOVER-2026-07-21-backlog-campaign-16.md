# Handover — third backlog campaign, cursor at LORE-80 (LORE-69..87)
...
Cursor: LORE-80 — rewriteInbound (core/rewrite.ts), the shared engine behind
...
task since LORE-71/76/77 (LORE-75 in between was correctness/destructive, not
security-labeled). Queue order confirmed by user on 2026-07-21 (independent
fixes first, the LORE-80→79→78→81 rename-traversal cluster last, LORE-80
first of that cluster since its shared-engine containment fix is what LORE-79
and LORE-78 build on/reference — read all three tasks' descriptions before
starting LORE-80). Merge gate is self-merge (skill default, user-confirmed
...
followed cleanly two sessions running (LORE-75, and the corrected mistake
originally made in LORE-74); don't regress on it. 3-issue queue remaining
after LORE-80 (LORE-79, 78, 81), all from a full-codebase Codex review (see
...
| Item | Status |
| --- | --- |
| Tracker doc | doc-1, updated this session (Cursor → LORE-80, Queue = 4 items, LORE-75 moved to Resolved with its review evidence, one new campaign convention recorded, two non-blocking review follow-ups added to Not-queued) |
| Review doc | doc-2, full Codex second-opinion review (201 confirmed findings, 25/25 clusters) — source of all queued tasks |
| Queue | 3 tasks remaining (LORE-80, 79, 78, 81 minus 80 = LORE-79, 78, 81 after LORE-80), all `To Do`, `bug`, `High` priority, each with AC + a `--ref` to doc-2 |
| Branch | `dev`, clean, 1 commit ahead of `origin/dev` (this session's archive commit) at handover-write time — push before starting LORE-80's preflight if not already pushed |
| Leftover branches/PRs | none — `feature/LORE-75` fully merged (PR #79, rebase-merged) and pruned (local + remote, confirmed via `git fetch --prune` + `git branch -a`) |
| Not queued | LORE-42/43/44/45 (deferred) plus the same follow-up candidates as before, plus two new ones from LORE-75's review (near-miss directory-name test coverage; `isManagedSchemasDir`'s lexical-only comparison has no realpath/symlink resolution) |

## Next steps

1. Run the per-issue lifecycle on **LORE-80** (`rewriteInbound` shared engine does not confine `fromId`/`toId` to `docs/` bundle root): branch `feature/LORE-80` off `dev`, read the task's AC, implement, verify, review, PR, self-merge, prune. Grounded code pointers (verified this session, not just the filing task's own prose):
   ...
   - `src/commands/new.ts` already has a proven containment pattern for a comparable problem (`resolveOutPath`, referenced by LORE-79's own AC as the pattern to mirror) — read it before designing LORE-80's own fix; the shapes may not be identical (`new.ts` confines a directory argument, `rewriteInbound` confines a concept id that must resolve to a path under `docs/`), but the resolve+relative containment idiom this campaign has used repeatedly (LORE-69, LORE-72, LORE-76/77) likely still applies.
...
- **LORE-80 is security-labeled** (`codex-review, security`) and is the deepest of the three rename-traversal-cluster layers — a fix here is the one the other two (LORE-79, LORE-78) may build on or reference. Give it this campaign's established full-rigor treatment for security tasks: live pre-fix repro against the real CLI, adversarial review, cross-platform (`win32.isAbsolute`) and NUL-byte-at-exec-boundary checks per LORE-69/72's own recorded conventions (both flagged CRITICAL for every remaining security-labeled task).
- **This session (LORE-75) followed the review-then-write ordering cleanly** — committed the fix+tests first, ran the independent review, THEN wrote the outcome into the tracker, avoiding the correction-commit LORE-74 needed. Keep this discipline: write nothing about a review's outcome (clean, or what it found) until the review has actually returned.
- `.repro-scratch/` keeps accumulating scratch files from every review (unchanged this session — LORE-75 added none beyond ephemeral scratch-repo dirs under the system temp dir, not this project's own scratchpad) — still don't delete its contents without being asked again.
