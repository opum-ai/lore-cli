# Handover — LCLI-18 `lore new` landed; next up LCLI-19 `lore validate` (LCLI-18 Done / LCLI-19 To Do)

**Date**: 2026-06-25 | **Grounded against**: `dev`=`11f908a` (==origin/dev) | **Backlog**: LCLI-18 **Done**, LCLI-19 **To Do**

## Paste-ready prompt for the next session

```
LCLI-18 (`lore new`) is DONE and merged to dev (PR #15, squash 9d46955). dev is at
11f908a == origin/dev, clean tree, no open PRs. Start LCLI-19: `lore validate` — tiered
per-type validation.

Per CLAUDE.md, FIRST run `backlog instructions overview`, then `backlog task view LCLI-19
--plain` and read its Documentation pointers: docs/adr/0007-validation-and-coherence.md
and docs/reference/okf-conformance.md. Plan with `backlog task edit LCLI-19 --plan ...`
before coding (task-execution workflow); confirm scope with Jeremy if it widens.

LCLI-19 = a `lore validate [PATHS]` command. The validation ENGINE largely exists — REUSE it,
don't reinvent:
  ...
  - NEW for LCLI-19 (per ADR-0007): per-type REQUIRED SECTIONS (body headings) as an ERROR
...
CLI router + arg parsing, follow the conventions LCLI-18 established (see traps below).
...
| Item | Status |
| --- | --- |
| LCLI-18 | **Done** — `lore new` delivered via PR #15 (squash `9d46955`); both ACs checked, review fixes landed |
| LCLI-19 | **To Do** — `lore validate`, deps LCLI-15 (satisfied). Docs: docs/adr/0007-validation-and-coherence.md, docs/reference/okf-conformance.md |
| `dev` | `11f908a` == origin/dev (merge + `chore(LCLI-18) mark Done` + archived kickoff handover). Clean tree |
| feature branch | none (feat/lore-18-new deleted, remote + local) |

## Next steps

1. `backlog instructions overview` → `backlog task view LCLI-19 --plain` → read ADR-0007 + okf-conformance.md.
2. Branch `feat/lore-19-validate` off `dev`. Plan via `backlog task edit LCLI-19 --plan`.
3. Confirm what LCLI-19 OWNS vs what exists: the `validateFrontmatter` engine + `loadBundle`
   are built; LCLI-19 adds the **command**, **per-type required-section** checks, and
...
- **CLI arg-model convention (set by LCLI-18 in src/cli.ts)**: global flags (`--json/--plain/
...
- **lore owns frontmatter, the author owns the body** (LCLI-18 lesson, now in template.ts /
  ...
  "frontmatter quote-safety" in LCLI-19 is about VALIDATING author-written frontmatter, not lore's
...
- **Case-sensitivity (CI is Linux)**: file lookups must not assume case-insensitive FS. LCLI-18's
...
- **Don't refactor the shared CLI parser without re-running the full cli.test suite** — LCLI-18's
...
## Deferred from LCLI-18 (not lost — pick up when relevant)

- **Coupling flags `--epic`/`--story`/`--resource`** for `lore new` → deferred to the story-task
  coupling work (ADR-0009 / LCLI-23/24). Recorded in LCLI-18 notes.
...
- **LCLI-18** → **Done**: ACs #1/#2 checked; notes capture the two-commit delivery (4f48345 +
...
- **dev** → `11f908a`: squash merge + `chore(LCLI-18): mark Done` + archived kickoff handover.
