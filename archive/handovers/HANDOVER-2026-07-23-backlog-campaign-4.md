...
| Waves 19-21 (prior sessions) | 19 resolved (18 merged + LORE-206 dup). See doc-4 wave log. |
| Wave 22 (this session) | LORE-216,217,219,223,225,227 merged (PR#205-210), integration clean |
| Wave 23 (this session) | LORE-218,220,221,224,226,228 merged (PR#211-216), integration clean |
...
- **errors-output-git**: LORE-249 (src/errors.ts — stripHint control-seq + length cap), LORE-250 (src/output.ts — suppress ANSI on non-TTY stderr). Both same cluster ⇒ serialize; also 250 shares src/output.ts with the merged 221.
- **adapter-backlog**: LORE-222 (src/adapters/backlog.ts — map spawn-rejections on ALL backlog calls to typed LoreErrors, not just probe --version).
- **cli-entry-state**: (none left — 223, 224 merged).
- **cmd-check**: LORE-226 done; LORE-197 (src/commands/check.ts — discovery advisories lost when a later root throws in collectBundles, LORE-191 residual).
- **cmd-crud-a**: LORE-229 (src/commands/replace.ts — sanitize discovered file paths in the report, strip ANSI/control).
- **cmd-crud-b**: LORE-230 + LORE-231 (src/commands/fswrite.ts — existingIsRegularFile masks non-ENOENT lstat; writeFileAtomic temp-file leak), LORE-232 (src/commands/query.ts — trim --type/--status/--tag values). 230/231 same file.
- **cmd-link**: LORE-233 + LORE-234 (src/commands/link.ts — bound viewTask fan-out concurrency; case-insensitive doc-membership dedup). Same file.
- **cmd-meta-a**: LORE-235 (src/commands/reconcile-shared.ts — bound resolveRollup fan-out), LORE-236 (src/commands/tasks.ts — strip ANSI/OSC on WarningCollector.flush stderr).
- **cmd-meta-b**: LORE-237 (src/commands/validate.ts — reject --strict=<value>/repeated --strict/repeated --type). NOTE: LORE-228 (merged) already added --strict inline-value rejection to validate.ts — CHECK whether 237 is now partially satisfied before dispatching; it also wants repeated-flag rejection which 228 did not add.
- **cmd-meta-c**: LORE-238 (src/commands/scaffold.ts — differentiate --force structural-dir-blocker conflict hint).
- **core-bundle-check**: LORE-239 + LORE-240 (src/core/check.ts — callout false-positive; leading indented code block), LORE-241 + LORE-242 (src/core/profile.ts — non-object profile.json error; validate default against kind/enum). 239/240 same file; 241/242 same file; all same cluster ⇒ one per wave.
...
- **core-query-validate**: LORE-246 (src/core/query.ts — matchesField resolves case-insensitive key across ALL variants).
...
A ≤6-worker wave picking one-per-cluster in queue order (#-column) would naturally be roughly: LORE-222, 197, 229, 230, 232, 233 (adapter-backlog / cmd-check / cmd-crud-a / cmd-crud-b / [232 is same cluster as 230 → skip] cmd-link / cmd-meta-a …). RECOMPUTE LIVE — this is illustrative only.
...
4. **Watch LORE-237 vs merged LORE-228**: 228 already added `--strict=<value>` rejection to validate.ts; 237 wants that PLUS repeated-`--strict`/repeated-`--type` rejection. Before dispatching 237, read validate.ts on dev to see what remains — 237 may be partially pre-satisfied.
...
- **Non-defect noted by wave-23 integration review** (no task filed): 3 pre-existing `.length`-based padEnd sites (help.ts:97/100/130, instructions.ts:176, orphans.ts:420) — all ASCII-by-construction, the residual sites if display-width alignment (LORE-221) is ever wanted repo-wide.
...
- **Don't dispatch a worker for a task already resolved (or partially pre-satisfied) by a merged sibling** — reconcile Done (LORE-206 pattern), or scope the worker to only the residual (see LORE-237 vs 228 note above).
