# Handover — Phase 1 finale: LCLI-33 shipped; next is LCLI-48 `check` follow-ups (LCLI-48)

**Date**: 2026-06-30 | **Grounded against**: `dev`=`origin/dev`=`8716b67` (clean, on `dev`); `main`=`23f3733` (intentionally behind) | **Backlog**: LCLI-33 **Done** (#28); LCLI-48 **To Do** (next)

## Paste-ready prompt for the next session

```
State: dev == origin/dev == 8716b67, clean, on dev, no open PRs. main == 23f3733 (intentionally
behind; do NOT promote unless asked). LCLI-33 (lore query) is DONE (squash #28 = 84a914e, chore
...
Next task = Phase 1 task 5 (the LAST Phase-1 task): LCLI-48 `lore check` follow-ups
(`backlog task view LCLI-48 --plain`). It EXTENDS the existing shipped `lore check` (core/check.ts +
commands/check.ts, from LCLI-30 which is Done). LCLI-48 has NO acceptance criteria defined yet —
FIRST action: read the task's Scope + Implementation Notes, draft ACs from them, and set them via
`backlog task edit LCLI-48 --ac "..."` (one per scope item) before coding. Present the plan to Jeremy.

LCLI-48 scope (all in the Scope + Notes of the task):
...
3. Carry-forward items parked in LCLI-30 notes (from PR #19/#21 /code-review max):
   ...
   (b) a PRECISE Obsidian block-ref `^id` detector (avoid carets in prose/math AND catch digit-leading
       auto-IDs like `^3f9a2b`); the naive text-node regex couldn't, so it was removed in LCLI-30;
...
NOT in LCLI-48: status-reconciliation + managed-block-drift passes of `check` — those are LCLI-27,
gated on the Backlog JSON adapter (LCLI-21) + lore sync (LCLI-26), Phase 2. **Do LCLI-48 BEFORE
LCLI-27** (both edit `check`; LCLI-27 layers adapter-dependent drift dims on top).
...
| Item | Status |
| --- | --- |
| **LCLI-33** (lore query) | **Done** — squash #28 (`84a914e`) + chore (`283d48d`); both ACs checked; review fold recorded |
...
| **LCLI-48** (check follow-ups) | **To Do — next**; dep LCLI-30 **Done**; **NO ACs defined yet** (define them first) |
| Phase 1 status | LCLI-20/31/34/33 **Done**; **LCLI-48 is the last Phase-1 task** |
| Phase 2 (fork+coupling) | LCLI-1→2→{3,4}→21→{22,23,24,25,32}→{26,27}; **LCLI-5 parked**; nothing started |
| Shared foundation on dev | `core/query.ts` (`subgraph` + `query`/BM25), `core/context.ts`, `core/bundle.ts` (`estimateTokens`/`frontmatterScalar`), `core/check.ts`/`commands/check.ts` (LCLI-30, the LCLI-48 base) |

## Next steps

1. **LCLI-48 `lore check` follow-ups** — `backlog task view LCLI-48 --plain`; **define ACs first**
...
2. Then Phase 2 begins (the fork): **LCLI-1** (create `jeremy-newhouse/Backlog.md` fork) → **LCLI-2**
   (`--json` matching `docs/reference/backlog-json-schema.md`) → LCLI-13(golden)/3/4 → LCLI-21 adapter.
...
- **Commit backlog task edits BEFORE syncing dev.** A post-merge `git reset --hard` to sync dev wipes
  ...
  session the LCLI-33 AC-checks/review-notes were made AFTER the last feat commit, so they were NOT in
...
- **Code review is the WORKFLOW one**: `Skill code-review args "max"` (calls `Workflow{name:"code-review"}`),
  NOT inline /review. LCLI-33's max review found 15 verified (6 folded) — budget a real fold pass.
...
- **Don't add speculative caching / premature micro-optimization.** LCLI-33's review flagged a per-doc
  tf-Map retention and it was DEFERRED as premature for current bundle sizes (same lesson as the LCLI-34
...
- **Don't hand-roll NEW arg-parser divergence** — clone the graph.ts/context.ts parser verbatim; the
  ...
  shared-helper debt (`readValue` 4th copy, `toHit`/`neighborOf` shaping dup) was deferred in LCLI-33,
...
- **LCLI-33 → Done**; ACs #1/#2 checked; notes record the impl + the folded #28 `/code-review max`
