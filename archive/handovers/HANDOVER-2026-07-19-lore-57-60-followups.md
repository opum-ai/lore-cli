# Handover — fix the 4 real defects the Docker E2E harness found (LORE-57, LORE-58, LORE-59, LORE-60)

**Date**: 2026-07-19 | **Grounded against**: dev/main @ `9650cd44bdb8a1b2e2c0e43da30bcde8cd91e1bc` (PR #52, merged, CI green on both branches) | **Backlog**: LORE-56 (Done), LORE-57, LORE-58, LORE-59, LORE-60 (all To Do)

## Paste-ready prompt for the next session

```
Pick up LORE-57 (highest-impact first — it breaks lore link/unlink/rename's Backlog
back-ref write for real, against the pinned upstream binary). Read backlog task view
LORE-57 --plain for the full repro and acceptance criteria, follow this project's
...
After LORE-57 lands, LORE-58 (link/unlink's stdout/stderr contract violation on
partial failure) becomes easier to verify cleanly, since LORE-57's fix removes the
main trigger path — but LORE-58 is a distinct, still-real structural gap (any future
...
LORE-59 (Story template missing the lore:tasks managed-block markers) and LORE-60
(low-priority ADR-0002 doc-accuracy fix) are independent of LORE-57/58 and can be
...
purpose (search run-e2e.sh for "LORE-57", "LORE-58", "LORE-59", "LORE-60" — the
comments say exactly which step and why). Flip that step's expected exit code (and
delete the LORE-59 workaround block once the template ships the markers itself), then
...
| Item | Status |
| --- | --- |
| PR #52 (LORE-56: Docker E2E harness) | Merged to `dev` (squash `9650cd4`), `dev` ff-merged into `main`, both pushed, CI green on both, feature branch pruned locally + remotely |
| LORE-56 | Done — all 6 ACs checked with evidence (see task notes/final summary) |
| LORE-57 | To Do — `editTask()` (`src/adapters/backlog.ts`) sends `--json` to `backlog task edit`, which doesn't support it (only `task list`/`view`/`search` do). Breaks every `lore link`/`unlink`/`rename` back-ref write against a real backlog binary. Concrete repro on the task. |
| LORE-58 | To Do, depends on LORE-56 + LORE-57 (informational dep, not a hard blocker — see task) — `lore link`/`unlink --json` emits a full success-shaped envelope on stdout even when exiting nonzero (6), violating the documented stdout/stderr contract |
| LORE-59 | To Do — `lore new Story`'s built-in template has no `<!-- lore:tasks:begin/end -->` markers; `lore sync` fails once real tasks are linked to a fresh Story. Breaks the documented canonical `new → link → sync` loop out of the box |
| LORE-60 | To Do, Low priority — `docs/adr/0002-backlog-integration-json-only.md` Decision point 5 says missing/too-old/incapable backlog all map to exit 6; real code deliberately uses exit 3 for "missing entirely" (see `probeBacklog`'s own inline comment). Doc-only fix. |
| `docker/e2e/` harness | Working, 81/81 green as of the merge commit. Two of its steps intentionally assert LORE-57/58's and LORE-59's *current buggy* exit codes as a regression baseline — see "Critical context" below |

## Next steps

1. `backlog task view LORE-57 --plain` — read the full repro and ACs, then follow the
...
2. Fix is likely a one-line removal (`"--json"` out of `editTask`'s args array in
   `src/adapters/backlog.ts`), but LORE-57's AC4 asks for a regression test asserting
   ...
   against a **real** pinned-upstream `backlog` binary per LORE-57's AC2/AC3 (mirror
   how LORE-53/54 did their real-binary verification — see those tasks' implementation
   notes for the clone/checkout/`bun install`/`bun run build` sequence, or just reuse
   `docker/e2e/`).
3. LORE-58 needs a design decision before implementation (its AC1) — likely worth a
   quick check-in on which of the two options the user prefers before writing code.
4. LORE-59: decide between adding the markers to `STORY_TEMPLATE`
...
5. LORE-60 is docs-only: correct ADR-0002 §Decision point 5, and check
...
- **The mocked-adapter unit test suite (1497+ tests, all green) did not and cannot
  catch any of LORE-57/58/59/60** — they're all real-binary-only findings. Don't treat
  ...
  LORE-53/54's documented real-binary verification technique).
- `run-e2e.sh` currently has explanatory comments at the exact lines that encode
  LORE-57/58/59/60's current-buggy expectations — grep the script for the LORE-NN
...
- LORE-56: plan, extensive implementation notes (harness design, all 12 harness-bug
  triage findings, all 4 real defects found), final summary, all 6 ACs checked,
  status Done.
- LORE-57, LORE-58, LORE-59, LORE-60: created with full repro, acceptance criteria,
  and cross-references back to LORE-56.
