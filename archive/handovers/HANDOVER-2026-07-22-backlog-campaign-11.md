...
remain To Do (+4 deferred LCLI-42..45 in "Not queued"). This session drained wave 13
(LCLI-160,152?-no: 160,163,166,169,180,186 — PRs #160-#165) and wave 14 (LCLI-161,167,
170,179,184,194 — PRs #166-#171): all 12 Fable-approve (only LCLI-184 needed 1 fix round),
0 escalations; both integration reviews non-blocking. Also reconciled LCLI-165 as
resolved-by-merge by LCLI-164. The 15 remaining To-Do: 153,168,171,172,178,181,182,183,
185,187,188,189,191,192,193. Zero formal deps among all in-scope (re-verify live via YAML
parse; only LCLI-42..45 carry deps to already-Done non-campaign tasks). Queue order/scope
...
| **Round 2 wave 13 (this session)** | **COMPLETE: LCLI-160,163,166,169,180,186 merged (PRs #160-#165). 6/6 Fable approve, 0 fix, 0 escalations. Base `f64dd23` → dev `bed190e`. Integration review CLEAN. Suite 1859→1872.** |
| **Round 2 wave 14 (this session)** | **COMPLETE: LCLI-161,167,170,179,184,194 merged (PRs #166-#171). 6/6 Fable approve (LCLI-184 after 1 fix), 0 escalations. Base `33f7f2a` → dev `fc12a2b`. Integration review non-blocking (2 info, folded into LCLI-181). Suite 1874→1887.** |
| Reconciled this session | LCLI-165 → Done (resolved-by-merge by LCLI-164; commit `3b0ef25`, Resolved row 78). LCLI-181 widened to cover LCLI-161's 3rd sanitizer copy (+2 ACs). Duplicate LCLI-195 archived. |
| Queue | **84 Done, 0 held/in-flight, 15 in-scope To Do** (+4 deferred LCLI-42..45 in "Not queued"). |
...
2. **Do NOT re-do waves 13/14.** All 12 are Done/merged (Resolved rows 66-84). Don't re-mint LCLI-96..195.
3. **The 15 remaining To-Do and their clusters/pinned files (recompute live, but this is the last-known map):**
   - core-links-resolution **LCLI-153** (`src/core/links.ts`) — ⚠️ its AC permits fixing at the finding **print site** (`src/commands/check.ts` / `src/commands/validate.ts`) instead of at message construction; it MIGHT touch check.ts. Keep it away from cmd-check **LCLI-191** (`src/commands/check.ts`), or constrain it to links.ts. (validate.ts is now free — 160/161 done.)
   - core-scaffold-consumer **LCLI-168** (`src/core/schema.ts`) — okf_version extra-key warning scope.
   - errors-output-git **LCLI-171** (`src/errors.ts`) / **LCLI-172** (`src/errors.ts`) — same file AND same cluster → serialize (pick one).
   - core-engine-a **LCLI-188** (`src/adapters/git.ts`).
   - build-ci-config **LCLI-178** (docs-only runbook via `lore` — `docker-e2e-testing-environment.md`, mention it's now a CI gate). Docs task: verify via `bun run src/cli.ts check`.
   - cmd-crud-b **LCLI-181** (now WIDENED: `src/errors.ts` + `src/output.ts` + `src/commands/query.ts` + `src/core/validate.ts` — shared-sanitizer consolidation; broad file set, schedule carefully) / **LCLI-189** (`src/commands/sync.ts`) — same cluster → serialize.
   - cmd-meta-a **LCLI-182** (`src/commands/schema.ts`).
   - cmd-link **LCLI-183** (`src/commands/link.ts`).
   - core-bundle-check **LCLI-185** (`src/core/profile.ts` + `src/commands/new.ts`) / **LCLI-192** (`src/core/bundle.ts`) / **LCLI-193** (`src/core/bundle.ts` — parseItems/itemToZod empty-enum, sibling of LCLI-140; reuse assertNonEmptyEnum) — same cluster → serialize; 192 & 193 both bundle.ts.
   - cmd-meta-c **LCLI-187** (`src/commands/schema.ts` + `src/commands/fswrite.ts`) — NOTE: shares `src/commands/schema.ts` with cmd-meta-a **LCLI-182** → cross-cluster file conflict, keep apart.
   - cmd-check **LCLI-191** (`src/commands/check.ts`).
...
- **Backlog ID minting** stays sequential, orchestrator-only, primary-checkout-only, between waves. This session minted then archived LCLI-195 (a duplicate of the pre-existing LCLI-181) — **always search existing tasks before filing an integration-review follow-up** (`backlog task list --status "To Do" --plain | grep <keyword>`); several consolidation/sanitizer/guard tasks already exist (LCLI-181, 185).
- **The Fable review + integration review earn their keep:** wave-14 integration review caught the sanitizer triplication (→ LCLI-181 widened); wave-13's confirmed serializer coherence. Budget an integration review + possible follow-up disposition every wave.
...
- Don't file an integration-review follow-up without first grepping existing To-Do tasks (LCLI-195 was a dup of LCLI-181).
- Don't re-mint LCLI-96..195; don't dispatch LCLI-165 (Done, resolved-by-merge).
