# Handover — LORE-54 shipped and merged to main; upstream-adoption thread (LORE-5) has no actionable next step (LORE-54, LORE-5)

**Date**: 2026-07-18 | **Grounded against**: `dev`/`main` both @ `21fa4b9` (clean, in sync with `origin/dev` and `origin/main`, no open PRs) | **Backlog**: LORE-54 Done; LORE-5 In Progress (2/2 own ACs now checked, blocked on an external tagged release); LORE-53/LORE-4/LORE-3 Done
...
Context: LORE-54 (rewrite src/adapters/backlog.ts against upstream's real --json
...
committed (173605e "feat(LORE-54): rewrite backlog adapter against upstream's real
...
LORE-5 (the umbrella "open the upstream --json PR and migrate lore on release"
task) had both its own ACs checked this session, since LORE-54 satisfied AC#2 (the
full adapter now matches upstream's contract) on top of LORE-53's AC#1 (the probe
...
`backlog task view LORE-5 --plain` (or the upstream repo's release list) first if
picking this thread back up later.

There is no other in-flight thread from this work. The Backlog "To Do" queue is
currently all LOW-priority, explicitly deferred items (LORE-41 obsidian scaffold,
LORE-42 MCP server v2, LORE-43/44 Confluence adapters, LORE-45 typed importable
library, LORE-52 stale remark/unified doc-reference cleanup) -- none are a natural
...
| LORE-54 | **Done**, committed (`173605e`) and pushed/merged |
| LORE-5 | In Progress; both its own ACs now checked; remaining scope (semver dependency + floor bump) blocked on an external upstream release |
| LORE-53 / LORE-4 / LORE-3 | Done (unchanged this session, confirmed as LORE-5's now-complete dependency set) |
...
1. No specific next step is owed on this thread — it is closed out until an external event (a tagged `MrLesk/Backlog.md` release including PR #790 / commit `22a091b`) unblocks LORE-5's remaining scope.
...
3. Periodically (e.g. next session touching this area) it may be worth checking `https://github.com/MrLesk/Backlog.md/releases` for a tag past `22a091b570d44c4f302ca47e7fd36fa28ad8bcb0` — if one exists, LORE-5's final step (real `package.json` dependency + version-floor bump) becomes actionable.
...
- Don't leave the two-tier probe/read fake in `test/backlog-adapter.test.ts` in place once the probe and the full adapter target the same contract — LORE-54 collapsed it back to one shared golden per the prior handover's explicit instruction; if a future contract migration re-diverges the two, expect to re-split it, but don't leave stale two-tier logic around once they reconverge again.

## System of record updated

- **LORE-54** → Done, with full plan/notes/ACs/final-summary (see task history for detail); no further action needed on it.
- **LORE-5** → both ACs checked with a note explaining what remains and why it's deliberately not marked Done yet.
- **Git**: `173605e` (LORE-54 implementation + doc updates) and `21fa4b9` (the handover-archive commit) both pushed to `origin/dev`, then fast-forwarded and pushed to `origin/main` (main has no independent history — a straight ff, not a real merge), per explicit user instruction this session.
