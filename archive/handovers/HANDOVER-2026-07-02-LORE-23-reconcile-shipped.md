# Handover — LCLI-23 shipped; pick up LCLI-24 (lore link/unlink + sync/check wiring)

**Date**: 2026-07-02 | **Grounded against**: `dev`=`main`=`0fe9170` (clean, pushed, no open PRs) | **Backlog**: LCLI-23 **Done** (PR #34, merged+promoted); LCLI-24 **To Do** (unblocked, dep LCLI-21 Done)

## Paste-ready prompt for the next session

```
FIRST run `backlog instructions overview`. Then pick up LCLI-24 (lore link /
unlink + the first CLI wiring of the coupling stack) — the last unblocked
task in the M2 Backlog-coupling chain (LCLI-23 landed this session).

LCLI-24 wires src/core/reconcile.ts (LCLI-23, ships reconcileStatus(taskStatuses,
statusFlow)) and src/core/managed-block.ts (LCLI-22, regenerateTaskBlock) into
...
amend ADR-0009, or split into its own follow-up task — before LCLI-24 needs a
statusFlow-building step that has to decide whether overrides apply.

LCLI-24 also needs a way to read `backlog/config.yml`'s `statuses:` ordered
...
Finalize the standard lore way: feature branch off dev → feat(LCLI-24): … →
...
| **LCLI-23** (reconcile.ts status rollup) | **Done** — PR #34 (squash `7080752`), `chore` `0fe9170`, promoted to `main` |
| **LCLI-24** (lore link/unlink + sync/check wiring) | **To Do**, dep LCLI-21 (Done) → unblocked, next up |
| `/code-review max` on PR #34 | ran before merge; 2 findings fixed in-PR, 2 flagged (see below) |

## Next steps

1. `backlog task view LCLI-24 --plain`; re-read `docs/adr/0009-story-task-coupling-reconciliation.md`.
...
4. Fold in the still-outstanding deferred LCLI-22 findings (recorded on LCLI-22's task notes): `normalizeLink` throws a bare `Error` (not `LoreError`) on an absolute path — the command must pass a repo-relative `docPath`; the `offsetsOf`/`cell`/`escapeLinkText` DRY overlap with `rewrite.ts`/`indexes.ts` is still open.

## Critical context / traps

- **`reconcile.ts` is ready to consume, with one unresolved gap.** `reconcileStatus(taskStatuses, statusFlow)` (pure, `src/core/reconcile.ts`) rolls up by elimination per ADR-0009 §3 / `backlog-cli-contract.md` §3.2: all-terminal → `done`, any-active → `in-progress`, else → `todo`; empty `taskStatuses` → `null` (leave authored status). It throws `LoreError("validation")` on an unrecognized task status or a degenerate `statusFlow` (now: fewer than 2 entries, or a duplicate). **Gap**: `src/config.ts` (LCLI-10) ships `reconcile.overrides` (`Record<string,string>`, a Backlog status → rollup-status override) whose doc comment assigns ownership to "reconcile.ts (LCLI-23)" — but ADR-0009 §3 never describes an override mechanism, and `reconcileStatus` has no parameter for it. Posted as a PR #34 comment; unresolved. Don't invent semantics for it — ask first.
...
- **Reads are JSON-only** via `createBacklogAdapter(spawn)`; never `--plain`, never grep `backlog/tasks/*.md`. [[backlog-fork-checkout]] [[backlog-dependency-grep-trap]] — no adapter method yet reads `backlog/config.yml` (needed for the `statusFlow` LCLI-24 must build).
...
- **LCLI-23** → **Done** (PR #34, squash `7080752`; `chore` `0fe9170`; promoted to `main`). Both ACs checked; plan + notes (incl. the `/code-review max` triage, both fixed and deferred findings) + final-summary recorded.
- **PR #34 comment** — posted the full `/code-review max` findings (2 fixed in-PR, 2 flagged as scope questions) for visibility, matching the LCLI-22 precedent of recording review triage on the PR itself.
- **CHANGELOG.md** (Unreleased/Added) — the `core/reconcile.ts` entry.
- **Predecessor handover** `HANDOVER-2026-07-02-lore-coupling-sync-check.md` (its LCLI-23 step now done, its LCLI-24 step carried forward here) → archived to `archive/handovers/` (uncommitted at handover-write time; commit alongside this file).
