# Handover — M2 Backlog coupling: LCLI-23 reconcile + LCLI-24 link/unlink + wire sync/check

**Date**: 2026-07-02 | **Grounded against**: `dev`=`main`=`3e597cc` (clean, pushed, no open PRs) | **Backlog**: LCLI-22 **Done** (PR #33, merged+promoted); LCLI-23 / LCLI-24 **To Do** (both unblocked, dep LCLI-21 Done)

## Paste-ready prompt for the next session

```
FIRST run `backlog instructions overview`. Then pick up the M2 Backlog-coupling
chain. Both are unblocked (dep LCLI-21 Done). Suggested order: LCLI-23 → LCLI-24.

LCLI-23 — reconcile.ts: status rollup. Pure core engine (src/core/reconcile.ts)
that computes a Story/Spec's rolled-up `status` from its linked tasks' statuses
(consumes the LCLI-21 adapter reads). See docs/adr/0009-story-task-coupling-
...
LCLI-24 — lore link / unlink (+ the first CLI wiring of the coupling stack).
This is where src/core/managed-block.ts (regenerateTaskBlock, shipped in LCLI-22)
...
WHEN WIRING managed-block in LCLI-24, fold in the DEFERRED /code-review findings
(recorded on LCLI-22's task notes):
  ...
  - findMarkers is tasks-only; retrofitting `lore:index` onto structural
    ...
    future scope, not required by LCLI-24.
...
| **LCLI-22** (managed-block.ts) | **Done** — PR #33 (squash `22ccd6b`), `chore` `3ddf52c`, promoted to `main`; handover archived `3e597cc` |
| **LCLI-23** (reconcile.ts status rollup) | **To Do**, dep LCLI-21 (Done) → unblocked |
| **LCLI-24** (lore link/unlink + sync/check wiring) | **To Do**, dep LCLI-21 (Done) → unblocked |

## Next steps

1. `backlog task view LCLI-23 --plain` (+ LCLI-24); read `docs/adr/0009-story-task-coupling-reconciliation.md`.
2. LCLI-23: build `src/core/reconcile.ts` (pure status rollup) + tests. Not wired to CLI.
3. LCLI-24: `lore link`/`unlink` commands + wire `reconcile` + `managed-block` into `lore sync`/`lore check`; fold in the deferred findings above.
...
## System of record updated (this session — LCLI-22, all landed on `dev`/`main` `3e597cc`)

- **LCLI-22** → **Done** (PR #33 `22ccd6b`; `chore` `3ddf52c`; promoted). ACs #1/#2 checked; plan + notes (incl. the /code-review triage) + final-summary recorded.
...
- **Auto-memory** [[lore-no-md-serializer]] updated: LCLI-22 delivered; the mdast-keeps-marker-whitespace `.trim()` gotcha + LF precondition.
- **Predecessor handover** `HANDOVER-2026-07-02-LCLI-22-managed-block.md` consumed → archived (`3e597cc`).
