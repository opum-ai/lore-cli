# Handover — LORE-4 (compile fork binary + capability probe) & LORE-13 (golden fixtures) — finish the BJP/m-0 milestone

**Date**: 2026-07-01 | **Grounded against**: lore `dev`=`origin/dev`=`70f340a`; fork `jeremy-newhouse/Backlog.md` `tasks/back-510-json-output`=`origin/...`=`a80b7a1`; both working trees clean | **Backlog**: LORE-1/2/3 **Done**; LORE-4 **To Do** (dep LORE-2 ✓); LORE-13 **To Do** (no deps); LORE-5 **parked**
...
28e0755 = --json impl [LORE-2], a80b7a1 = fork task marked Done). LORE-1/2/3 all
...
LORE-4 (m-0, MEDIUM, dep LORE-2 ✓) — build the patched fork as a compiled binary
for lore to consume as a git dependency, + a capability probe.
  ...
  LORE-21's adapter — decide with the user whether LORE-4 ships only a minimal
  standalone probe or defers the probe wiring to LORE-21. Contract:
  docs/reference/backlog-cli-contract.md.

LORE-13 (m-1, MEDIUM, no deps) — golden fixtures: a sample OKF bundle (valid +
...
Finalize each in the lore repo the LORE-1/2/3 way: check ACs + notes + final
...
| Item | Status |
| --- | --- |
| **LORE-1** | **Done** — fork + in-fork task `back-510` (18eb886) |
| **LORE-2** | **Done** — `--json` serializer + flag; fork `28e0755`; lore `4059011` |
| **LORE-3** | **Done** — tests + help-schema + CLI-INSTRUCTIONS **delivered inside the LORE-2 fork commit**; lore `70f340a` |
...
| **LORE-4** (next) | **To Do**, Medium, m-0, dep LORE-2 ✓ — compile binary + capability probe |
| **LORE-13** (next) | **To Do**, Medium, m-1, no deps — golden fixtures + JSON-contract tests |
| **LORE-5** | **parked** — upstream PR deferred |
| **LORE-21** (adapter) | **To Do**, High, m-3; dep is **LORE-5** (parked) — see trap re: dependency oddity |

## Next steps

1. **LORE-4** — clone the fork branch to a **real internal-disk** path (NOT `~/repos/Backlog.md`, which symlinks to `/Volumes/external`), `bun install` + `bun run build`, smoke-test `backlog task list --json`. Wire lore's git-dep pin (`github:jeremy-newhouse/Backlog.md#tasks/back-510-json-output`) per runbook §6. Resolve the probe-scope overlap with LORE-21 (ask user). Finalize LORE-4 on dev.
2. **LORE-13** — author the fixture OKF bundle (valid + broken concepts, every type) and record golden `--json` outputs from the live fork; add golden idempotency + JSON-contract test scaffolding pinned to `docs/reference/backlog-json-schema.md`. Finalize LORE-13.
3. After m-0 closes (LORE-4), the coupling/adapter phase (LORE-21 → LORE-22/23/24/26/27) begins — but reconcile the **LORE-21 dep on parked LORE-5** first (likely should be LORE-4, not LORE-5).

## Critical context / traps

- **BUILD ON REAL INTERNAL DISK — the fork path lies.** `~/repos/Backlog.md` is a **symlink to `/Volumes/external/repos/Backlog.md`** (verify: `realpath ~/repos/Backlog.md`). On `/Volumes/external`, `bun build --compile` silent-fails to a 0-byte binary and isolated `bun install` hits EXDEV. Everything ELSE (non-isolated `bun install`, `bun src/cli.ts`, `bunx tsc`, `biome check`, `bun test`) runs fine there — proven in LORE-2. Only the **compile** step needs a non-symlinked internal path. [[external-volume-bun-exdev-traps]] [[backlog-fork-checkout]]
...
- **LORE-21 dependency oddity**: LORE-21 (JSON-only adapter) lists dep **LORE-5** (parked upstream PR), but the adapter actually needs LORE-4 (compiled binary + probe). Flag to the user before starting the coupling phase; don't silently rewire deps.
- **Don't open an upstream PR** (LORE-5 parked) and **don't merge/prune** `tasks/back-510-json-output`.
...
- **Don't re-implement LORE-3** — its scope (cli-json-output.test.ts incl. the non-TTY pipe case, addHelpSchema, CLI-INSTRUCTIONS) already shipped in the LORE-2 fork commit `28e0755` and LORE-3 is marked Done.
- **Don't split tests from code again expecting a separate task** — LORE-2 correctly bundled the tests+docs (LORE-3's scope) with the serializer; that's why LORE-3 was pre-satisfied.

## System of record updated (this session)

- **LORE-2** → **Done** (ACs checked, notes, final summary); lore `dev` `4059011` `chore(LORE-2): Done`.
- **LORE-3** → **Done** (ACs checked, notes, final summary — delivered via LORE-2); lore `dev` `70f340a` `chore(LORE-3): Done`.
...
- **Auto-memory**: corrected [[backlog-fork-checkout]] — `~/repos/Backlog.md` is a SYMLINK to `/Volumes/external` (was wrongly recorded as internal disk); recorded the pre-existing `cli-doc-search` test-baseline failure; noted LORE-2/3 Done + fork HEAD `a80b7a1`. Updated MEMORY.md index line.
