...
(1) errors.ts TRIO — LCLI-236 (cmd-meta-a, WarningCollector.flush), LCLI-249 (errors-output-git,
    stderrHint), LCLI-250 (errors-output-git, output.ts+cli.ts, MAY touch errors.ts for the
...
(2) test/query.test.ts COLLATERAL — LCLI-232 (edits src/commands/query.ts) and LCLI-246 (edits
...
(3) same-cluster/same-file — LCLI-240 (src/core/check.ts) is a core-bundle-check singleton now
    (239 merged); LCLI-241 + LCLI-242 BOTH edit src/core/profile.ts (core-bundle-check) → serialize.
(4) Verified NON-conflicts (do NOT over-serialize): LCLI-247 edits ONLY src/core/rewrite.ts (guard
    before normalizeLink) — does NOT edit core/links.ts, so it does NOT conflict with LCLI-245
    (core/links.ts); LCLI-243 (done) never edited git.ts/sync.ts. LCLI-244 edits ONLY
...
| Wave 25 (this session) | LCLI-231,234,235,237,239,243 merged (PR#223-228), integration review clean |
...
- **cmd-crud-b**: LCLI-232 (src/commands/query.ts — trim --type/--status/--tag values; adds to test/query.test.ts).
- **cmd-meta-a**: LCLI-236 (src/errors.ts — strip ANSI/OSC/control on WarningCollector.flush; **errors.ts trio**).
- **core-bundle-check**: LCLI-240 (src/core/check.ts — leading indented code block in frontmatter-free files; may also touch core/concept.ts normalizeInput), LCLI-241 (src/core/profile.ts — non-object profile.json error), LCLI-242 (src/core/profile.ts — validate default vs kind/enum). 241/242 same file ⇒ serialize; all three same cluster ⇒ one per wave.
- **core-index-context**: LCLI-244 (src/core/indexes.ts — conceptTitle numeric/boolean coercion via frontmatterScalar).
- **core-links-resolution**: LCLI-245 (src/core/links.ts — validateLink flags bare '.'/'..' destinations).
- **core-query-validate**: LCLI-246 (src/core/query.ts — matchesField resolves case-insensitive key across ALL variants; adds to test/query.test.ts ⇒ **serialize vs LCLI-232**).
- **core-rewrite-engine**: LCLI-247 (src/core/rewrite.ts — preserve above-repo-root outbound links during rename; edits ONLY rewrite.ts).
- **core-scaffold-consumer**: LCLI-248 (src/core/schema.ts — warnSummary counts UTF-16 not chars for non-BMP).
- **errors-output-git**: LCLI-249 (src/errors.ts — stderrHint control-seq strip + length cap; **errors.ts trio**), LCLI-250 (src/output.ts + src/cli.ts, maybe src/errors.ts — suppress ANSI on non-TTY stderr; **errors.ts trio**). 249/250 same cluster ⇒ serialize.

An illustrative ≤6-worker wave 26 (RECOMPUTE LIVE — this is NOT a plan): one-per-cluster, file+test-disjoint, ordinal order → LCLI-232, 236, 240, 244, 245, 247 (cmd-crud-b / cmd-meta-a / core-bundle-check / core-index-context / core-links-resolution / core-rewrite-engine). That would leave 241, 242, 246, 248, 249, 250 for later waves (note 241+242 can't co-wave; 249+250 can't co-wave; so the tail is ~2-3 waves, shrinking as clusters drain).
...
- **Read the AC BODY for the edit target, not the tracker Note.** Proven this round: LCLI-232's Note cites core/query.ts but it edits commands/query.ts; LCLI-247 edits only rewrite.ts (not links.ts, despite citing it); LCLI-243 never edited git.ts/sync.ts (unchanged callers). Over-approximate only when a filename is genuinely ambiguous.
...
- **Concurrency helper location (post-LCLI-233, relevant to LCLI-235-like tasks — 235 now DONE):** `mapWithConcurrency` + `TASK_DETAILS_CONCURRENCY` live in `src/commands/concurrency.ts` (dependency-free); `reconcile-shared.ts` re-exports for back-compat. If any remaining task needs bounded fan-out, import from `./concurrency`.
...
- **Don't treat the tracker `[file]` Note as the edit target** — it's audit provenance. Read the task BODY/ACs for the real file(s) AND the test file(s). (Wave 25 confirmed LCLI-232 edits commands/query.ts not core/query.ts, LCLI-247 edits only rewrite.ts.)
