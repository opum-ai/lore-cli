# Handover — LORE-49 shipped; next in the ship sequence is LORE-39/40 (scaffold)

**Date**: 2026-07-11 | **Grounded against**: `dev` @ `a10bac4`; `main` @ `a10bac4` (equal) | **Backlog**: LORE-49 Done; next = LORE-39/40
...
LORE-49 (retrofit link/unlink/rename to commit backlog/ via state.ts) is DONE and
...
pending on LORE-49. See [[backlog-commit-seam]] for the invariants that fold locked in.

Continue the "finish the backlog and ship" sequence the user is driving:
  RECOMMENDED NEXT: LORE-39 (`lore scaffold mkdocs`) then LORE-40 (`lore scaffold
  docusaurus` + a build smoke test) — the two remaining command-surface features.
  Then de-risk + ship: LORE-14 (Bun compile compatibility spike, LOW) -> LORE-9
  (release pipeline: compiled binaries + dual-artifact npm publish). LORE-5 (open the
  upstream --json PR + migrate lore) is gated on the release. Deferred/out-of-v1:
  LORE-41 (obsidian), 42 (mcp), 43/44 (Confluence), 45 (importable lib).
  ALSO AVAILABLE: LORE-51 (LOW cleanup) — dedup the task-summary-row TYPE + aligned-row
  renderer across tasks.ts/orphans.ts (carries the Math.max spread-free-maxLen note for
  tasks.ts). NOTE: LORE-49 already extracted the *backlog-commit* line renderer
  (renderBacklogCommitLine); LORE-51 is the SEPARATE task-summary-row dedup.
...
| Item | Status |
| --- | --- |
| LORE-49 (`commitBacklogFiles`) | **Done** — merged via #44 (`387f3b9`), review-hardened (9 findings folded, +8 tests) |
...
| Next | LORE-39 (`lore scaffold mkdocs`) — To Do, branch off `dev` |
| Also queued | LORE-40, LORE-14, LORE-9, LORE-5; LORE-51 (LOW cleanup) |

## Next steps

1. `backlog task view LORE-39 --plain` (and LORE-40); confirm deps Done; branch `feat/lore-39-scaffold-mkdocs` off `dev`.
...
- **Do NOT re-litigate LORE-49's accepted-by-design item**: the new hard git dependency (link/unlink/rename now shell git for any real back-ref write, so a non-git repo exits 6 where it previously exited 0) is accepted — consistent with sync + ADR-0012; running outside a git repo was never a supported lore workflow. Recorded in the LORE-49 task notes.
...
- **LORE-49 task** → Done; notes appended with the full `/code-review max` disposition (8 fixed + 1 accepted-by-design), committed pre-merge so the reset couldn't wipe it.
- **CHANGELOG.md** → the LORE-49 "Changed" entry updated with the `:(literal)` scoping + capture-not-throw report behavior.
