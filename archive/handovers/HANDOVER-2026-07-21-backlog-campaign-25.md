# Handover — tenth backlog campaign session, cursor at LORE-88 (item 6 of 8)
...
Cursor: LORE-88 — core/rewrite.ts's rewriteInbound (the shared engine behind
...
citation of 318 because LORE-95 added ~45 lines earlier in this same file) and
its moved-but-textually-unchanged fallback serializeConcept(concept)
(rewrite.ts:211, this one did NOT drift — it's before LORE-95's insertion
...
the "thread a project's custom profile through" pair — LORE-89 (just resolved)
did the identical pattern for check.ts's own concept-scan path. This is item 6
of the 8-item queue (LORE-90/94/92/95/89/88/91/93) confirmed by the user on
2026-07-21 ("Risk-ascending, sweep last (Recommended)"); do not re-ask before
taking the next item.

Read the task itself first (`backlog task view LORE-88 --plain`) — do not
...
touched it this campaign — LORE-95 shifted it once already; check again).

READ LORE-89's OWN RESOLUTION FIRST (`backlog task view LORE-89 --plain`,
its Implementation Notes) — this task is explicitly the sibling of that one,
same underlying pattern, and LORE-89 already resolved the "which profile,
...
remote `feature/LORE-89` branch (left over because `--delete-branch` never
...
Also budget real time for a review round: LORE-89's own independent review
...
After LORE-88: advance cursor to LORE-91, then LORE-93 in that confirmed
order. LORE-93 (ensureDir call sites in 5 commands follow symlinks) is the
...
| Item | Status |
| --- | --- |
| Tracker doc | doc-1, updated this session (LORE-89 moved to Resolved as row #32, Cursor → LORE-88, Queue renumbered to 3 remaining items, session-log entry appended for session 33) |
| Queue | 3 tasks remaining, all `To Do`, all `bug` type: LORE-88/91/93 |
| Resolved this session | LORE-89 — see Resolved table row 32 for full evidence summary |
...
- Don't re-derive the "thread a profile through" pattern from scratch for LORE-88 — LORE-89 (and, further back, LORE-84) already solved this shape; read those precedents' notes/diffs before designing something new.
...
- Don't call `loadProfile` twice for the same command invocation when one already-loaded value can be threaded through instead (AC#4's exact concern for LORE-88) — hoist into a named variable and reuse it.
