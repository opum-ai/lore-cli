# Handover — LORE-15 concept layer delivered as PR #12 (reviewed, awaiting merge); LORE-16 next

**Date**: 2026-06-24 | **Grounded against**: `dev`=`5a4857c` (== origin/dev) · PR #12 head `cfc6e3d` (mergeable=CLEAN) | **Backlog**: LORE-15 **In Progress** (both ACs checked, final summary written) — Done on merge

## Paste-ready prompt for the next session

```
Continue building lore (OKF-native docs CLI, Bun+TS). M0 + core-infra are DONE on dev
(errors/config/output + scaffolding/CI). LORE-15 (src/core/concept.ts + src/core/schema.ts —
...
- If MERGED: mark LORE-15 Done via a DIRECT-TO-DEV chore commit (LORE-10/12 precedent), NOT a PR:
    git checkout dev && (fetch/pull dev)
    backlog task edit LORE-15 -s Done
    git add backlog/ && git commit -m "chore(LORE-15): mark Done (delivered via #12)" \
...
THEN start LORE-16 (src/core/bundle.ts): walk docs/, build the concept graph (cross-links +
frontmatter refs), generate index/log, token estimates. Dep = LORE-15 (consume parseConcept/
serializeConcept + the Concept type from src/core/concept.ts). Claim: backlog task edit LORE-16
...
| Item | Status |
| --- | --- |
| `dev` | `5a4857c` (== origin/dev; consumed kickoff handover archived). No LORE-15 code yet — it's in PR #12 |
| PR #12 (feat/lore-15-concept → dev) | **OPEN** at `cfc6e3d`, mergeable=CLEAN, no reviews yet. 199 tests + lint + typecheck green |
| LORE-15 (concept.ts + schema.ts) | **In Progress** — both ACs checked, final summary + 4-round review + /review-resolution notes recorded. Done ON MERGE |
| LORE-16 (bundle.ts) | To Do — **next**, dep LORE-15 (then LORE-17 `lore init`, dep 15+16) |
| M2 / Backlog-adapter (LORE-21+) | Blocked on BJP (the Backlog.md `--json` fork) |

## Next steps

1. Resolve PR #12 (merge or address review). On merge → mark LORE-15 Done via direct-to-dev chore commit; archive this handover.
2. Build **LORE-16** (`src/core/bundle.ts`) off merged `dev` → PR into dev.
3. Then LORE-17 (`lore init` — also emits the Zod→JSON-Schema files + the above-fence editor modeline, both **deferred from LORE-15** to here).
...
- **Documented LORE-15 limitations** (pathological, fixpoint-converging, pinned by tests): in-frontmatter YAML comments lost; `|+` trailing-newline scalar; integer-like keys reorder; large unquoted numbers lose precision. bundle.ts reads concepts, won't author exotic frontmatter, so won't hit these.
...
- **Don't mark LORE-15 Done at PR-open** — In Progress until #12 merges (LORE-10/12 precedent).
...
- **A single review pass is not enough for a foundational module** — LORE-15 took 4 /code-review max + 1 /review.

## System of record updated (this session)

- **LORE-15**: plan, ACs #1/#2 checked, notes for 4 /code-review max rounds + the /review pass (4 findings resolved in cfc6e3d: case-insensitive idFromPath, readonly id/path/type, dropped needless descriptor read, single-sourced Zod-issue projection), final summary, PR #12 link. Status In Progress (Done on merge).
- **PR #12** (`cfc6e3d`): body documents the 4 decisions + 4 limitations; a /review-resolution comment posted.
- **CHANGELOG (Unreleased)**: LORE-15 entry (in the PR).
