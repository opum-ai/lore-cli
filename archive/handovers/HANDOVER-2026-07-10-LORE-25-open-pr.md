# Handover — LORE-25 (`lore tasks`) delivered as open PR #42, review-hardened, awaiting merge

**Date**: 2026-07-10 | **Grounded against**: `feat/lore-25-tasks` @ `acc75fa`; `dev` @ `8ccdfd9`; `main` @ `8ccdfd9` | **Backlog**: LORE-25 (In Progress, both ACs checked)
...
LORE-25 (`lore tasks <id> [--status <S>]`) is DONE and delivered as PR #42
...
recorded on the LORE-25 task). It is the user's to review/merge — do NOT self-merge
...
- If PR #42 is ALREADY MERGED: finalize LORE-25 — `backlog task edit LORE-25 -s Done`,
...
Recommended next pick: LORE-32 (orphans — the bidirectional dangling-link report that
...
| LORE-25 | In Progress; both ACs checked; → Done on merge |
| `dev` / `main` | both `8ccdfd9` (LORE-38 promoted this session) |

## Next steps

1. Merge PR #42 (user's call) → then finalize LORE-25 (mark Done; ff `dev`→`main`; prune; archive this handover).
2. Pick the next ship-sequence task — **LORE-32 (`orphans`)** recommended (branch off `dev`; independent of #42).
...
- **Backlog-adapter read discipline** (reused by LORE-32 orphans): `adapter.probe()` UP FRONT (fail-fast 3/6) so a later `viewTask` null unambiguously = dangling `tasks:` id (soft: drop + WarningCollector stderr advisory, exit 0) vs a real throw = drift (hard: rethrow). `allSettled` + first-in-order rethrow (not `Promise.all`). Empty `tasks:` short-circuits before probing.
...
- **Do NOT hand-list per-command exit codes** — derive from seams (`exitCodesFor`) + the independent golden row; they must agree (LORE-38 class of bug).
...
- **LORE-25 task** → full plan + implementation notes + the review trail (5 fixed / 4 declined with rationale); both ACs checked (committed to the branch, `acc75fa`).
...
- **Auto-memory** → [[lore-cli-command-pattern]] extended with the post-LORE-38 surface-coherence ripple + Backlog-adapter read discipline + object-wrapped-envelope convention.
