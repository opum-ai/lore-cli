# Handover — remaining LORE-56 Docker E2E follow-ups (LORE-59, LORE-60) + two open PRs

**Date**: 2026-07-19 | **Grounded against**: `dev @ 9650cd44bdb8a1b2e2c0e43da30bcde8cd91e1bc` (unchanged this session) | **Backlog**: LORE-56/57/58 Done, LORE-59/60 To Do

## Paste-ready prompt for the next session

```
Two of the four LORE-56 Docker E2E follow-ups are Done (LORE-57, LORE-58), each shipped as its
own PR into dev (#53, #54 — see "State" below for merge order). Two remain, independent of each
other and of the two open PRs:

LORE-59: `lore new Story`'s built-in template has no `<!-- lore:tasks:begin/end -->` markers, so
`lore sync` fails once real tasks are linked to a fresh Story. Read `backlog task view LORE-59
...
as an implementation choice, not a product decision like LORE-58's was).

LORE-60: docs/adr/0002-backlog-integration-json-only.md's Decision point 5 overstates the
...
overclaim (not yet checked, per LORE-56's original notes).
...
LORE-57/58 both needed this), and update docker/e2e/run-e2e.sh's LORE-59 regression step (search
"LORE-59" — flip its expected exit code and delete the workaround block once the template ships
the markers itself) if LORE-59 lands. LORE-60 is docs-only and has no run-e2e.sh step to update.
...
2. Pick up LORE-59 or LORE-60 (either order, independent) following `backlog instructions
   task-execution` → `task-finalization`, same pattern as LORE-57/58 this session: plan recorded
...
4. Add a `CHANGELOG.md` `[Unreleased]` → `### Fixed` entry for whichever of LORE-59/60 you fix,
   matching the style of the LORE-57/58 entries already added (top of the `### Fixed` list).
...
- **CHANGELOG.md merge-conflict risk between #53 and #54.** Both PRs insert their own entry at
  ...
  **second** will conflict on that hunk. This is a real, expected conflict — not a sign of drift —
  resolve it by keeping both entries (LORE-58's above LORE-57's, or vice versa; order between them
...
- **The mocked-adapter unit suite cannot catch a `backlog` CLI flag-surface mismatch** — it mocks
  `BacklogAdapter` entirely. Both LORE-57 and LORE-58 were real-binary-only findings from LORE-56;
...
- **LORE-58's fix changed `runLink`/`runUnlink`'s failure contract**: they now **throw** a `drift`
  ...
  commands.
- **`docs/.obsidian/` sits untracked in the working tree, unrelated to LORE-57/58/59/60** — present
...
- Don't return a plain exit code from `runLink`/`runUnlink` on a partial-failure path and also
  call `emit()` first — that's exactly LORE-58's bug. On any failure, either throw before `emit()`
...
- Don't assume `docker/e2e/run-e2e.sh`'s "known regression" comments are exhaustive proof of what's
  still broken — LORE-58 turned out to have **no dedicated `run-e2e.sh` step** at all (LORE-57's
...
- LORE-57: plan, implementation notes (fix + verification evidence), final summary, all 4 ACs
  checked, status Done.
- LORE-58: plan (implicit via the design-decision note), implementation notes (fix + verification
...
- `docs/reference/cli-contract.md` §4, `docs/reference/cli-surface.md` (link/unlink Output rows),
  ...
  multi-item command follows the same stdout-empty-on-failure rule as any other failure (LORE-58
  AC3).
- `docker/e2e/run-e2e.sh` Phase 4 and `docs/runbooks/docker-e2e-testing-environment.md`'s
  known-regressions section: updated to drop LORE-57 (fixed) from the regression baseline; LORE-58
  never had a dedicated step (see trap above).
- `CHANGELOG.md`: added `[Unreleased]` → `### Fixed` entries for both LORE-57 and LORE-58 (on
  their respective branches — see the merge-conflict trap above). LORE-56 itself still has **no**
