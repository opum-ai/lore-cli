# Handover — Backlog campaign session 7: LCLI-66 (docker/e2e command-surface tail + housekeeping)
...
(`backlog doc view doc-1 --plain`). Cursor: LCLI-66 — docker/e2e:
...
LCLI-66 depends on LCLI-56 and LCLI-61 (both Done). This is an E2E task:
...
wrong assumption in its own filing task's description; LCLI-65 specifically
...
trusting the number: LCLI-65 added ~214 net lines to run-e2e.sh, so every
...
Two new campaign conventions from the LCLI-65 session, apply here too if
...
irrelevant to LCLI-66's own surface, noted only for completeness; (b) when
...
the fixture — LCLI-65's own "metachar-titled task" premise for the
`:(literal)` pathspec guard turned out to be exactly this trap.

LCLI-68 (docs/reference broken-link bug LCLI-64 discovered) is still NOT
...
| Branches / PRs | No `feature/*` branches local or remote; no open PRs (`feature/LCLI-65` deleted both sides via `gh pr merge --delete-branch`, confirmed via `git fetch --prune` removing the stale local remote-tracking ref). |
| `dev` vs `origin/dev` | In sync — both LCLI-65's merge commit and the handover-archive commit (`18b516f`) are pushed. |
| LCLI-66 dependencies | LCLI-56 and LCLI-61, both Done. Fully unblocked, no re-verification needed next session. |
| **Queue after LCLI-66**: EMPTY. This is the last confirmed item (67, 61-66). The next session should resolve LCLI-66 and then, per the skill's "Queue empty" branch, summarize the full campaign and suggest `/backlog-handover init` for a fresh queue rather than assuming more work. | |
| LCLI-42/43/44/45 | Still parked in tracker "Not queued" — deferred by recorded product decisions. Do not pick up. |
| LCLI-68 | Still parked in tracker "Not queued" — real, verified, pre-existing broken-link bug, kept unqueued per user confirmation. Do not pick up unless asked. |

## Next steps

1. Per-issue lifecycle on LCLI-66 (`backlog task view LCLI-66 --plain` for the six ACs): branch `feature/LCLI-66` off `dev`, plan on the task.
2. Re-read the CURRENT `docker/e2e/run-e2e.sh` end-to-end before writing anything — every line number the filing task cites is stale after LCLI-65's ~214-line addition. Locate the vacuous `replace`/`supersede --rewrite-links` steps, the `lore --version`/git-log-wc-l/auto-plain steps, and the misleading "stale-cache" step by their own comment headers and content.
...
10. Update the tracker on the branch: since the queue is empty after LCLI-66, do NOT set "Next issue: <KEY>" to a nonexistent item — record the queue as empty, move LCLI-66 to Resolved, append the session-log entry, and note the campaign is complete pending a fresh `init`.
...
- **LCLI-66 requires the real Docker harness**, not just `bun test`. Budget the ~2-3 min build+run cycle, and never skip `down -v` teardown even on failure.
- **This is the LAST queued item.** Do not invent additional queue items or silently pull in LCLI-68/42/43/44/45 — those are explicitly parked. When LCLI-66 resolves, the correct next action is to report campaign completion, not to keep driving.
- `docs/.obsidian/` is untracked, pre-existing, unrelated — leave it alone (confirmed still present and harmless this session).
- **Line numbers in a filing task's description go stale fast** — LCLI-65 alone added ~214 net lines to `run-e2e.sh`. Locate existing phases and the specific steps LCLI-66 targets by their own comment headers/content, never by a filing task's cited line numbers.
- **Don't trust a filing task's proposed induction technique or shape assumption at face value** — every E2E session so far (61/62/63/64/65) found at least one wrong assumption in its own filing task's description; LCLI-65 found two, both from trusting a Backlog CLI title string would carry through unmodified when Backlog's own input normalization (title→filename sanitization, `--title` never renaming a file) silently disproved the premise.
- **When proving a command is UNAFFECTED by a surface under test, diff before/after output — don't assert a bare exit code/baseline** (LCLI-64's lesson, still standing).
- **A step_json/step_fail jq filter must stay on ONE physical line**; `check()`'s `eval "$expr"` can span multiple lines via backslash continuation (LCLI-62's lesson).
- **Anchor a status/value grep on the managed-block row's own `"[TASK-n]"` link-text bracket**, never a bare `grep -i "$TASKn"` (LCLI-63's lesson).
...
- Leaving induced test state uncleaned when nothing later re-checks it — even if currently dormant/harmless, it silently breaks the moment a later phase does re-check it. Restore/re-heal before moving on (LCLI-65's Phase 4b/4c/4d each explicitly restored or scoped their own induced state; the `/tmp/nested-e2e` fixture in Phase 24b is fully outside the main bundle and self-removed).
- Trusting a filing task's own proposed technique/shape/line-number citations without independently re-reading the actual current source first — LCLI-61 through LCLI-65 each found at least one wrong assumption this way; LCLI-65 found two (the `--title`-renames-file premise, and the metachar-title premise), both caught by reading the pinned fork's real source (`file-system/operations.ts`) BEFORE writing any fixture, saving a wasted docker cycle each time.
...
- Assuming a `git diff-tree HEAD` (no `--root`) will show a commit's files when that commit might be the repo's very first (root) commit — it shows nothing without `--root` (LCLI-65's own AC4 test bug, caught by the real harness run, not by review).
- Asserting a per-write commit (`lore link`'s scoped `commitBacklogFiles`) leaves the WHOLE `backlog/` tree clean — it deliberately does NOT sweep pre-existing dirty files outside what it wrote (ADR-0012 §1); that's `lore sync`'s catch-all sweep's job. Assert "clean" only after a sync, not after a bare link (LCLI-65's own AC4 test bug, same run).
