# Handover — LORE-22: managed-block.ts (DELIVERED via PR #33, awaiting user merge)

**Date**: 2026-07-02 | **Grounded against**: branch `feat/lore-22-managed-block`=`94709e2` (pushed); `dev`=`f722eab` | **Backlog**: LORE-22 **In Progress** (both ACs checked), delivered via **PR #33** (open, base `dev`)

## Paste-ready prompt for the next session

```
FIRST run `backlog instructions overview`. Then check whether PR #33 (LORE-22,
...
IF MERGED (the user merges/promotes himself — ASK before doing it for him):
  1. Finalize LORE-22 the standard lore way (mirror how LORE-21/#32 closed):
     `backlog task edit LORE-22 -s Done` (ACs #1/#2 already checked; plan/notes/
     ...
     Commit `chore(LORE-22): mark Done (delivered via #33)` on dev.
  2. Archive THIS handover: `mv .claude/handovers/HANDOVER-2026-07-02-LORE-22-managed-block.md
     archive/handovers/` + `docs: archive consumed handover lore-22-managed-block`.
  3. Next unblocked work: LORE-23 (reconcile.ts status rollup) and LORE-24
     (lore link/unlink) — both dep LORE-21 (Done), sequence after 22. LORE-24 is
...
| **LORE-22** | **In Progress**; ACs #1 + #2 checked; plan/notes/final-summary recorded |
...
| LORE-23 / LORE-24 | To Do; unblocked (dep LORE-21 Done); sequence after 22 |
...
- **Engine only** — deliberately NOT wired into `cli.ts`; `link`/`unlink`/`sync` are LORE-24+.
...
- Don't wire coupling commands into `cli.ts` under LORE-22 — that's LORE-24+.
- Don't self-merge #33 — the user reviews/merges/promotes.

## System of record updated (this session)

- **LORE-22** → In Progress, ACs #1/#2 checked; plan, implementation notes, and final-summary recorded via `backlog task edit`.
