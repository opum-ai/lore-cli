# Handover — LORE-30 merged & Done; pick the next ready task (LORE-35 recommended)

**Date**: 2026-06-28 | **Grounded against**: `dev`=`c15a3d0` (==origin, clean tree, checked out) | **Backlog**: LORE-30 **Done** (delivered #21), LORE-48 **To Do** (follow-up), LORE-28 **Done**

## Paste-ready prompt for the next session

```
State: dev is c15a3d0 (== origin), clean tree, no open PRs. LORE-30 (`lore check`: link/anchor +
...
Task: PICK THE NEXT READY TASK. The LORE-28 → LORE-30 link/check thread is complete. Recommended next
is LORE-35 (lore replace/rename/supersede) — it is the OTHER LORE-28 consumer: links.ts names
`rewriteInbound(graph, from, to)` (composes normalizeLink) as the LORE-35 piece, so it directly
continues this thread. Other READY tasks (deps satisfied — verify with the CLI): LORE-31 (lore graph,
dep LORE-16 ✓), LORE-33 (lore query, dep LORE-16 ✓), LORE-34 (lore context, dep LORE-16 ✓ + LORE-28 ✓),
LORE-20 (lore schema export, dep LORE-15 ✓). All read the loadBundle graph (bundle.ts) — pure core +
thin command, the established split.

FIRST run `backlog instructions overview`, then `backlog task view LORE-35 --plain` (or whichever you
pick) and CONFIRM its Dependencies with the CLI (raw grep lies — [[backlog-dependency-grep-trap]]).

Do NOT pick: the backlog-fork cluster stays BLOCKED behind the LORE-1 fork chain — LORE-1/2/3/4/5
(fork + --json), LORE-21 (backlog.ts adapter), LORE-22/23/24/25/26 (managed-block/reconcile/link/
tasks/sync), LORE-27 (lore check's status-recon + managed-block-drift passes — the OTHER half of
`lore check`, gated on the adapter+sync), and LORE-32 (lore orphans, dep LORE-21). LORE-48 is the
LORE-30 follow-up (deferred lint items) — low priority, not blocking.
...
| Item | Status |
| --- | --- |
| **LORE-30** (`lore check` link/anchor + portability) | **Done** — squash-merged **#21** (`96a69bc`); branch deleted everywhere |
| `dev` | `c15a3d0` == origin; clean; **checked out**; green (651 tests, biome, tsc; core/check+links+concept 100%) |
| Open PRs | **none** |
| **LORE-48** (lore check follow-ups) | **To Do**, dep LORE-30 (satisfied); deferred lint items (block-ref, colon-filename, trailing-slash policy, finding-model, IO-errno helper) — low priority |
| **LORE-35 / 31 / 33 / 34 / 20** | **To Do, READY** (deps satisfied) — recommended next is LORE-35 |
| LORE-27 (`lore check` drift passes) + the LORE-1/21/26 chain | **Blocked** behind the LORE-1 fork (backlog `--json` adapter + sync) |
...
- **Don't pull `remark-validate-links`/`unified` into `lore check`** — LORE-30 built link/anchor on the existing `mdast-util-from-markdown` + `walkMdast` (zero-config bunx; dodges the external-volume install traps). Keep that path.
- **Don't trust `grep dependencies backlog/tasks/*.md`** — use `backlog task view --plain`.

## System of record updated (this session)

- **LORE-30** → **Done**; both ACs checked; notes record the implementation, the folded #21 `/code-review max` fixes, and "delivered via #21 / 96a69bc".
- **LORE-48** created (dep LORE-30): deferred `lore check` lint items + the parked PR #19/#21 review minors.
- **CHANGELOG.md** (dev) → LORE-30 Added entries (`lore check`, the `validateLink` classifier hardening, the max-review fold), coexisting with LORE-28/29/47.
...
- Archived consumed handover `HANDOVER-2026-06-27-lore-30-next.md` (LORE-30 delivered + merged).
