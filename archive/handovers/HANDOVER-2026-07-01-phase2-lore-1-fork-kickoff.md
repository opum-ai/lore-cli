# Handover — Phase 1 landed; begin Phase 2 = the Backlog.md fork (LCLI-1 → LCLI-2 → … → LCLI-21)

**Date**: 2026-07-01 | **Grounded against**: `dev`=`origin/dev`=`e1c28bf`; `main`=`23f3733` (intentionally behind); working tree CLEAN; no active handovers | **Backlog**: LCLI-48 **Done** (Phase 1 complete); next = LCLI-1 **To Do**
...
(LCLI-48 delivered via squash #29 = b9049fe, plus chore mark-Done e1c28bf). main==23f3733
...
Execution order (deps verified via `backlog task view LORE-N --plain`, NOT grep):
  LCLI-1  (To Do, dep: none)     — fork MrLesk/Backlog.md -> jeremy-newhouse/Backlog.md +
                                    create the in-fork tracking task per its PR template.
  LCLI-2  (To Do, dep: LCLI-1)   — src/formatters/task-json.ts + per-command --json flag
  ...
  LCLI-13 (golden) / LCLI-3 / LCLI-4 — lock the schema with golden fixtures BEFORE the adapter.
  LCLI-21 (adapter)              — lore's consumer-side adapter over the fork's --json.

Plan bookkeeping to enact at Phase 2 start (from the master plan):
  - PARK LCLI-5: re-scope to "consume fork as git dep + version floor" (no upstream PR yet).
    `backlog task edit LCLI-5 -s "To Do"` won't park it — check `backlog` for the parked/blocked
    status verb; if none, leave a --comment recording the re-scope + set a blocking dep.
  - NOTE LCLI-32 carries the LCLI-21 dependency despite its m-4 milestone label (don't start
    LCLI-32 until LCLI-21 lands).
...
| Item | Status |
| --- | --- |
| **Phase 1** | **COMPLETE** — LCLI-20/31/34/33/48 all merged into `dev` |
| **LCLI-48** | **Done** — delivered via squash PR #29 (`b9049fe`); mark-Done chore `e1c28bf` |
...
| **LCLI-1** (next) | **To Do**, High, no deps — fork the repo + in-fork task |
| **LCLI-2** | **To Do**, dep LCLI-1 — `--json` serializer + flag |
| Active handovers | none (this session's two were archived + committed in `e1c28bf`) |

## Next steps

1. **LCLI-1** — fork `MrLesk/Backlog.md` → `jeremy-newhouse/Backlog.md` (`gh repo fork MrLesk/Backlog.md --clone=false`), then create the in-fork tracking task per that repo's PR template. AC: public fork exists + in-fork task with AC/plan/Testing.
2. **LCLI-2** — `src/formatters/task-json.ts` + per-command `--json` (json-before-plain) on `task list` / `task view` (+`task id`) / `search`; ISO `lastModified`; omit `rawContent` by default. Emit the canonical `{schemaVersion,kind,data}` envelope from `docs/reference/backlog-json-schema.md`.
3. Lock the schema with **golden fixtures (LCLI-13)** + LCLI-3/4 **before** building **LCLI-21** (lore's adapter).
4. Enact plan bookkeeping: **park/re-scope LCLI-5**; note **LCLI-32 → LCLI-21** dep.

## Critical context / traps

- **Phase 2 edits an EXTERNAL repo**, not the lore CLI. The `docs/reference/backlog-json-schema.md` in THIS repo is the contract lore builds the fork to satisfy; `docs/runbooks/backlog-json-patch.md` is LCLI-1's runbook.
...
- **Don't open an upstream PR to MrLesk/Backlog.md yet** — LCLI-5 is being re-scoped to "consume fork as git dep + version floor"; upstreaming is deferred.
...
- **LCLI-48** → **Done**; delivery note appended (`delivered via #29, squash b9049fe`).
- **dev** `e1c28bf` = `chore(LCLI-48): mark Done (delivered via #29); archive consumed handovers` (backlog task .md AC-checks/notes + both consumed handovers).
...
- No CHANGELOG/docs/memory changes needed — LCLI-48's release notes shipped in #29; Phase-2 bookkeeping is not yet enacted (listed as next steps).
