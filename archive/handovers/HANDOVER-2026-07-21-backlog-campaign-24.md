# Handover — ninth backlog campaign session, cursor at LCLI-89 (item 5 of 8)
...
Cursor: LCLI-89 — src/commands/check.ts's own concept-scan path
...
`loadBundle` (which LCLI-84 already fixed to thread a project's
...
(LCLI-90/94/92/95/89/88/91/93) confirmed by the user on 2026-07-21
("Risk-ascending, sweep last (Recommended)"); do not re-ask before taking the
next item.

Read the task itself first (`backlog task view LCLI-89 --plain`) — do not
...
Precedent to follow (LCLI-84, the prior "thread the profile through" task,
...
1. `runCheck` needs to `loadProfile({ root: options.root })` once (mirroring
   how `context.ts`/`graph.ts`/etc. already do it per LCLI-84) and forward it
...
2. OPEN QUESTION to resolve before implementing, not already decided: `lore
   ...
   LCLI-84's own multi-bundle callers (if any) handled this, before assuming
...
3. `error`-swallowing symmetry: `tryConceptsForBundle` already isolates one
   ...
   whole scan (LCLI-27 round 9's own fix, documented right above the
...
(not just core/bundle.ts's loadBundle in isolation, which LCLI-84 already
...
LCLI-92's review found genuine blocking defects (session 31), and this
...
After LCLI-89: advance cursor to LCLI-88, then LCLI-91, LCLI-93 in that
confirmed order. LCLI-88 is closely related (same "thread the profile
through" pattern, applied to `rewriteInbound`'s `serializeConcept`
re-serialize path instead of a parse path) — read LCLI-89's own resolution
notes before starting LCLI-88 next session, since the same design questions
(which root, how to avoid re-deriving what LCLI-84/LCLI-89 already solved)
...
| Item | Status |
| --- | --- |
| Tracker doc | doc-1, updated this session (LCLI-95 moved to Resolved as row #31, Cursor → LCLI-89, Queue renumbered to 4 remaining items, session-log entry appended for session 32) |
| Queue | 4 tasks remaining, all `To Do`, all `bug` type: LCLI-89/88/91/93 |
| Resolved this session | LCLI-95 — see Resolved table row 31 for full evidence summary |
...
| Leftover branches/PRs | none — `feature/LCLI-95` was merged (PR #87, `gh pr merge --rebase --delete-branch`) and pruned both remotely and locally automatically |
| Not queued | LCLI-42/43/44/45 (deferred by recorded product decisions, unchanged) plus the accumulated independent-review follow-up candidates from sessions 9-32 (see tracker's Not-queued section, including last session's Windows `O_NOFOLLOW` gap) |

## Next steps

1. **First action of the next session**: `git push origin dev` — this session's archive commit (`ea414c0`) is local-only.
2. Run the per-issue lifecycle on **LCLI-89** (`lore check`'s own concept-scan never forwards a custom profile): branch `feature/LCLI-89` off `dev`, read the task's AC (`backlog task view LCLI-89 --plain`), re-verify the gap fresh against current `src/commands/check.ts`, resolve the "which root does the profile load against" open question above by reading LCLI-84's own diff/PR first, implement (AC#1), add the reject/accept regression tests (AC#2/#3), add a check.ts-command-layer test proving `check` now agrees with `validate`/`query` on the same file (AC#4), verify, review, commit fix+tests, run independent review, THEN update the tracker with the outcome, PR, self-merge, prune.
3. Update doc-1's Cursor/Queue/Resolved/Session-log sections on the feature branch (per the skill's step 4) — advance cursor to LCLI-88 — but only AFTER the independent review completes.
4. Continue the confirmed queue order: LCLI-88 → LCLI-91 → LCLI-93. Read LCLI-89's own resolution before starting LCLI-88, per the paste-ready prompt's note above (closely related task, same design questions likely recur).

## Critical context / traps

- **This session's archive commit is unpushed** — push it first thing next session.
- **LCLI-89 and LCLI-88 are closely related** (same "thread a project's custom profile through" pattern that LCLI-84 already established for `loadBundle`'s 9 call sites, now needed for check.ts's separate parse path and, next, for `rewriteInbound`'s re-serialize path) — resolve LCLI-89's design questions carefully since LCLI-88 will likely face the same ones.
...
- **The review step can find genuine blocking defects, not just non-blocking follow-ups** — confirmed at LCLI-92 (session 31): a discarded `writeSync` return value and an undocumented Windows platform gap, both real, both required a second implementation round. Budget for this on every task, especially one with an open design question like LCLI-89's "which root" question.
- **A useful review-verification pattern has emerged across the last two sessions' independent reviews**: revert the fix's source files (via `git stash` or similar), rerun the NEW tests, confirm they genuinely fail without the fix (not vacuous/tautological), then restore and rerun the full suite. Worth explicitly asking for this in the review prompt for LCLI-89 too, especially for AC#4's "check and validate/query now agree" claim — a reviewer should independently confirm the pre-fix disagreement was real, not just accept the task's own filed repro at face value.
...
- **`backlog task edit`'s AC-checking flag is `--check-ac <index>`, not `--ac <index:checked>`** — caught and fixed at session 30 (LCLI-94); this session used the correct flag from the start. Keep doing that.
...
- Don't trust a synthetic test suite alone as proof a correctness fix is real — live-CLI-verify against a real scratch bundle too. For LCLI-89 specifically: confirm the three-way disagreement (`check` vs. `validate`/`query`/`sync`) is real on current dev HEAD BEFORE implementing, the same way LCLI-95 confirmed its own two gaps were real via `git stash` pre/post comparison.
...
- Don't re-derive the "thread a profile through" pattern from scratch — LCLI-84 already solved this for `loadBundle`'s 9 callers; read that precedent (task notes, PR diff) before writing new profile-loading code for check.ts.
