...
remain To Do (+4 deferred LCLI-42..45 in "Not queued"). This session drained wave 11
(LCLI-138,144,147,150,151,156 — PRs #147-#152, all Fable-approve on first pass, 0 fix, 0
escalations) PLUS one integration follow-up LCLI-190 (PR #153, 1 fix round). Filed 2 more follow-ups still To Do: LCLI-191 (cmd-check, low — flush discovery
advisories before checkBundles can throw) + LCLI-192 (core-bundle-check, medium — loadBundle
profile asymmetry, sibling of LCLI-144). Queue order confirmed by user 2026-07-21/re-affirmed
2026-07-22 (medium-only); do NOT re-ask scope or order. Zero formal deps among all 32 in-scope
To-Do (re-verify live via YAML parse; only LCLI-42..45 carry deps, to already-Done non-campaign
...
| **Round 2 wave 11 (this session)** | **COMPLETE: LCLI-138,144,147,150,151,156 merged (PRs #147-#152). 6/6 Fable approve, 0 fix, 0 escalations. Integration review FINDINGS_PRESENT (behavior composes cleanly; 1 medium + 2 low + 2 info). Base `43c9415` → dev `048470c`.** |
| **Wave-11 integration follow-up (this session)** | **LCLI-190 (medium) FIXED same session: PR #153, dev @ `c11ef0e`, Fable approve after 1 fix round (first pass over-corrected into a falsely-exhaustive claim; re-review caught it).** |
| Follow-ups filed this session | LCLI-190 (core-engine-b, medium — check/sync validation-cause + marker-shape prose drift) **DONE**; LCLI-191 (cmd-check, low — advisory flush before scan-phase throw) **To Do**; LCLI-192 (core-bundle-check, medium — loadBundle profile asymmetry, LCLI-144 sibling) **To Do** |
| Queue | **65 Done, 0 held/in-flight, 32 in-scope To Do** (+4 deferred LCLI-42..45 in "Not queued"). |
...
None — wave 11 + the LCLI-190 integration follow-up fully settled and merged. No mid-wave leftovers. (Intentionally no rows.)
...
2. **Do NOT re-do wave 11.** All 6 + LCLI-190 are Done/merged (Resolved rows 59-65). Don't re-mint LCLI-96..192 — they exist.
3. **Cluster leaders to consider (informational only — recompute live):** core-bundle-check LCLI-140 (profile.ts, parseFieldSpec empty enum) / 184 (bundle.ts+rewrite.ts+supersede.ts, **medium**) / 185 (profile.ts+new.ts) / 192 (bundle.ts, **medium**, LCLI-144 sibling) — serialize, pick ONE; core-rewrite-engine LCLI-164/165/180 + 184 (all rewrite.ts — serialize, pick ONE); core-links-resolution LCLI-152/153 (both links.ts, and 153 CITEs check.ts — serialize + keep off any check.ts wave); core-query-validate LCLI-159/160/161 (all validate.ts — serialize, pick ONE); core-scaffold-consumer LCLI-166 (consumer-scaffold.ts) / 167 / 168 (167+168 both schema.ts — serialize); errors-output-git LCLI-169/170 (git.ts) + 171/172 (errors.ts); core-managed-template LCLI-157 (template.ts); core-replace LCLI-162 (replace.ts+indexes.ts) / 163 (replace.ts); cmd-link LCLI-179 / 183 (both link.ts — serialize); cmd-crud-b LCLI-181 (output.ts+query.ts) / 189 (sync.ts); cmd-meta-a LCLI-182 (schema.ts); cmd-meta-c LCLI-187 (schema.ts+fswrite.ts); cmd-check LCLI-191 (commands/check.ts, low — advisory flush); build-ci-config LCLI-178 (docs-only runbook via lore). A clean example next wave (verify live): 140, 157, 159, 162, 166, 169 — six disjoint clusters/files (profile.ts / template.ts / validate.ts / replace.ts+indexes.ts / consumer-scaffold.ts / git.ts).
...
- **Merge queue re-verify is mandatory even on a clean rebase.** Every wave-11 rebase was clean yet each got full `bun test` + `bun run typecheck` in-worktree before merge. Suite grew 1818→1844 across the wave, then 1845 after LCLI-190.
...
- **Backlog ID minting stays sequential, orchestrator-only, primary-checkout-only, between waves.** Minted LCLI-190/191/192 that way via `backlog task create` (labels `codex-review-followup,<cluster>`, `--priority`, `--type bug`, `--ordinal`, single-quoted args for backtick text). Safe to mint while a worker runs IF the worker only `backlog task edit`s its OWN task (edits don't mint IDs). NEVER mint from a worktree.
- **The Fable review loop + integration review earn their keep.** Wave 11 impl was clean (0 fix rounds), but the integration review found a real medium (LCLI-147's own new prose was factually wrong about check's `validation` cause, made worse by LCLI-156 same wave) → LCLI-190. And LCLI-190's OWN first fix pass over-corrected into a NEW false claim ("two distinct causes" — falsely exhaustive) which the Fable re-review caught → 1 fix round. Budget the integration review + a fix round every wave.
- **LCLI-192 fix note (for whoever takes it):** loadBundle's path space is bundle-relative (`index.md`), NOT repo-relative (`docs/index.md`) — a naive reuse of LCLI-144's validate.ts `effectiveProfileFor` (which matches `docs/index.md`) would silently miss. Apply the reserved-root exemption at loadBundle's bundle-relative root path.
...
- Don't re-implement/re-merge the 6 wave-11 issues or LCLI-190 — all Done/merged (Resolved rows 59-65).
- Don't re-run `backlog task create` for LCLI-96..192 — they exist.
...
- Don't treat the wave-11 "session stops" as an escalation — clean context checkpoint (R4j); nothing is blocked. LCLI-191/192 are normal queued follow-ups.
