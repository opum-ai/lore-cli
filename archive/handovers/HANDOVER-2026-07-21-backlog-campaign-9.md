# Handover — third backlog campaign, cursor at LORE-72 (LORE-69..87)
...
Cursor: LORE-72 — `lore new --template` allows path traversal to read
...
CRITICAL: read doc-1's newest campaign convention (added this session, LORE-69)
about validate-vs-execute divergence at exec/argv boundaries BEFORE
implementing the fix — it applies directly to LORE-72's own file-path
...
| Item | Status |
| --- | --- |
| Tracker doc | doc-1, updated this session (Cursor → LORE-72, Queue = 11 items, LORE-69 moved to Resolved with the review-caught bypass documented, two new campaign conventions recorded, one new Not-queued follow-up filed) |
| Review doc | doc-2, full Codex second-opinion review (201 confirmed findings, 25/25 clusters) — source of all queued tasks |
| Queue | 11 tasks remaining (LORE-72, 71, 76, 77, 73, 74, 75, 80, 79, 78, 81), all `To Do`, `bug`, `High` priority, each with AC + a `--ref` to doc-2 |
| Branch | `dev`, clean (0 unpushed after this session's final push) |
| Leftover branches/PRs | none — `feature/LORE-69` fully merged (PR #72, rebase-merged) and pruned (local + remote, confirmed via `git fetch --prune`) |
| Not queued | LORE-42/43/44/45 (deferred) plus three unfiled follow-up candidates: two from LORE-84 (rewriteInbound's profile gap; lore check's separate validation path), one new from LORE-69 (commitBacklogFiles's guard is POSIX-only via `posix.normalize`, not currently reachable but not a designed barrier either) |

## Next steps

1. Run the per-issue lifecycle on **LORE-72** (`lore new --template` path
   traversal, security-labeled): branch `feature/LORE-72` off `dev`, read the
...
2. **Read LORE-69's newly-recorded campaign convention (doc-1, Campaign
   conventions section, "CRITICAL for every remaining security-labeled task")
   BEFORE designing the fix.** LORE-69's first fix attempt (normalize-then-
   ...
   shorter string than what was validated. LORE-72's own fix will almost
   ...
   but verify it holds against the same categories LORE-69 needed: `.`/`..`
   variants, doubled slashes, NUL bytes, absolute paths, and — per LORE-69's
...
3. Update doc-1's Cursor/Queue/Resolved/Session-log sections on the feature
   ...
   **LORE-71** (item #2 of the remaining queue).
4. Archive this handover to `archive/handovers/` and write the next one for
   LORE-71. Note: today's date (`2026-07-21`) already has EIGHT prior archived
   handovers (base, `-2` through `-8`) — this session's own archival will need
   suffix `-9`.

## Critical context / traps

- **This is the THIRD security-labeled task this campaign** (after LORE-85,
  LORE-69). LORE-69's independent review caught a real, live, exploitable
  ...
  remaining security task (LORE-71 SSRF, LORE-72 this session, LORE-76/77
  symlink escapes, LORE-75 destructive deletion) — explicitly ask the
  ...
  session did.
- **`.repro-scratch/` now has several extra files from LORE-69's security
  review** (`lore69-nul-bypass.ts`, `lore69-nul-bypass-verbose.ts`,
  `lore69-mixed-array.ts`, `nul-truncation-generic.js`, plus LORE-85's
...
- **Timing lesson from LORE-82's session** (recorded in that session's
  ...
  AFTER the review pass — BEFORE calling `gh pr merge`. This session (LORE-69)
...
- **`gh pr merge --rebase --delete-branch` auto-switches you off the feature
  ...
  `git branch -d feature/LORE-69` reported "not found" because `gh` had
...
- Don't assume a fix that passes self-review and its own tests is done on a
  security-labeled task — LORE-69's first attempt looked complete and still
