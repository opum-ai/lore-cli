# Handover — third backlog campaign, cursor at LORE-87 (LORE-69..87)
...
Cursor: LORE-87 — rewriteInbound mis-locates reference-definition destinations
...
| Item | Status |
| --- | --- |
| Tracker doc | doc-1, updated this session (Cursor → LORE-87, Queue = 17 items, LORE-86 moved to Resolved, one new convention recorded) |
| Review doc | doc-2, full Codex second-opinion review (201 confirmed findings, 25/25 clusters) — source of all queued tasks |
| Queue | 17 tasks remaining (LORE-87, 83, 84, 82, 85, 69, 72, 71, 76, 77, 73, 74, 75, 80, 79, 78, 81), all `To Do`, `bug`, `High` priority, each with AC + a `--ref` to doc-2 |
| Branch | `dev`, clean (0 unpushed after this session's final push) |
| Leftover branches/PRs | none — `feature/LORE-86` fully merged (PR #66, rebase-merged, commit `82672d4`) and pruned (local + remote) |
| Not queued | LORE-42/43/44/45 — unchanged, deferred by recorded product decision |

## Next steps

1. Run the per-issue lifecycle on **LORE-87** (`rewriteInbound mis-locates
   reference-definition destinations when the label contains an escaped
   bracket`): branch `feature/LORE-87` off `dev`, read the task's AC,
...
2. Update doc-1's Cursor/Queue/Resolved/Session-log sections on the feature
   ...
   **LORE-83** (item #2 of the remaining queue).
3. Archive this handover to `archive/handovers/` and write the next one for
   LORE-83. Note: today's date (`2026-07-21`) already has TWO prior archived
...
- **Three queued issues are one gap at three layers** (see doc-1's Campaign
  ...
  intentionally — LORE-80 last of the three (now #14 in the remaining queue)
  so its containment fix is available before/while working the other two.
  Read all three task descriptions before starting any one. Note: LORE-87
...
- **New convention from this session**: when a "malformed input recovery"
  ...
  (LORE-22/36's mdast-based `lore:tasks` engine) already solved the
  ...
  with that pattern until LORE-86 — check other "recover from malformed
...
- Don't assume every merged bugfix needs a CHANGELOG.md entry just because
  ...
  Neither LORE-68 nor LORE-70 (both merged in this same campaign lineage)
  added one; the tracker doc, not CHANGELOG.md, is this campaign's record of
  truth. LORE-86 deliberately skipped a CHANGELOG entry on this basis.
