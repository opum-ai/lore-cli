# Handover — third backlog campaign, cursor at LCLI-76 (LCLI-69..87)
...
Cursor: LCLI-76 — lore scaffold --force writes follow symlinks, escaping the
repo root (security-labeled, fifth security task this campaign). Queue order
confirmed by user on 2026-07-21 (independent fixes first, the LCLI-78/79/80
...
CRITICAL: read doc-1's newest campaign conventions (added LCLI-69/71/72,
...
| Item | Status |
| --- | --- |
| Tracker doc | doc-1, updated this session (Cursor → LCLI-76, Queue = 9 items, LCLI-71 moved to Resolved with its review findings documented, three new campaign conventions recorded) |
| Review doc | doc-2, full Codex second-opinion review (201 confirmed findings, 25/25 clusters) — source of all queued tasks |
| Queue | 9 tasks remaining (LCLI-76, 77, 73, 74, 75, 80, 79, 78, 81), all `To Do`, `bug`, `High` priority, each with AC + a `--ref` to doc-2 |
| Branch | `dev`, clean (0 unpushed after this session's final push) |
| Leftover branches/PRs | none — `feature/LCLI-71` fully merged (PR #74, rebase-merged) and pruned (local + remote, confirmed via `git fetch --prune`) |
| Not queued | LCLI-42/43/44/45 (deferred) plus five unfiled follow-up candidates: two from LCLI-84, one from LCLI-69, two from LCLI-72 (a symlink-read gap in `lore new --template`, DIRECTLY RELEVANT to this session's cursor issue — see below; a profile-declared-template traversal, deliberately excluded) |

## Next steps

1. Run the per-issue lifecycle on **LCLI-76** (`lore scaffold --force` writes
   follow symlinks, security-labeled): branch `feature/LCLI-76` off `dev`,
   ...
   - `src/core/bundle.ts` and `src/commands/replace.ts` are the task's own
     ...
     — already used as a template once this campaign, for LCLI-72's (out-of-
     ...
     guard — LCLI-76 is the WRITE-path counterpart in the same escape class.
2. **Directly relevant prior finding**: LCLI-72's independent review (this
   same campaign, same day) found a symlink-READ gap in `lore new
   --template` and confirmed it is NOT covered by LCLI-76/77 (those are
   specifically WRITE-path) — but now that you're actually working LCLI-76,
   re-read that Not-queued entry (doc-1) once more: if LCLI-76's fix
...
3. **AC2** needs a test with a REAL symlink (`symlinkSync` from `node:fs`,
   already imported in `test/check.test.ts` for an unrelated LCLI-71-era
...
4. Update doc-1's Cursor/Queue/Resolved/Session-log sections on the feature
   ...
   **LCLI-77** (item #2 of the remaining queue — the SIBLING symlink task,
   `lore init` following pre-existing symlinks; read both LCLI-76 and
   LCLI-77 together now if it saves rework later, but resolve only LCLI-76
   this session per the one-issue-per-session rule).
5. Archive this handover to `archive/handovers/` and write the next one for
   LCLI-77. Note: today's date (`2026-07-21`) already has TEN prior archived
   handovers (base, `-2` through `-10`) — this session's own archival will
   need suffix `-11`.

## Critical context / traps

- **This is the FIFTH security-labeled task this campaign** (after LCLI-85,
  LCLI-69, LCLI-72, LCLI-71). Every one so far has had its independent
  adversarial review find SOMETHING — a live bypass (LCLI-69), a real but
  out-of-scope gap (LCLI-72), or a real-but-narrow classifier gap plus a
  test-coverage gap (LCLI-71, via differential fuzzing). Do not treat the
  lifecycle's step-6 review as a formality for LCLI-76 or LCLI-77 — budget
  time for a second round of fix + re-verify + re-commit before the PR opens.
- **LCLI-76 and LCLI-77 are a matched pair** (write-path symlink-following:
  `scaffold --force` and `init` respectively) — similar in spirit to how
  LCLI-78/79/80 are three layers of the same rename-traversal gap. Read
  LCLI-77's own description now (already available) so the SAME symlink-
  detection approach can be reused/kept consistent across both sessions,
  even though only LCLI-76 is resolved this session.
- **`.repro-scratch/` keeps accumulating scratch files from every security
  review** (LCLI-85, LCLI-69, LCLI-72, LCLI-71 all left files there) — all
...
- No `code-reviewer` subagent type is registered in this project — use
  ...
  hand-rolled classifier/parser-shaped fix (unlikely here, but LCLI-71's WAS
  ...
  that way LCLI-71 wouldn't have caught otherwise.
...
- Don't assume a found gap overlaps an already-queued item just because the
  ...
  either way (this already burned/saved LCLI-72's session once).
- When a fix adds a NEW injectable IO seam beside an existing one, share ONE
  ...
  error path (LCLI-71's self-caught bug: an uncaught DNS fault silently
