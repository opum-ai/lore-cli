# Handover — LCLI-2: implement the `--json` serializer + flag in the Backlog.md fork (Phase 2)

**Date**: 2026-07-01 | **Grounded against**: lore `dev`=`origin/dev`=`18eb886`; fork `jeremy-newhouse/Backlog.md` branch `tasks/back-510-json-output` pushed; working trees clean | **Backlog**: LCLI-1 **Done**; LCLI-2 **To Do** (dep LCLI-1 ✓ satisfied); LCLI-5 **parked**
...
State to trust (verify SHAs anyway): Phase 2 kickoff (LCLI-1) is DONE. lore dev==18eb886
(chore(LCLI-1): fork + in-fork task + park LCLI-5). Working tree clean, on dev, no lore PRs.
...
LCLI-2 = implement the --json patch in the fork on that branch, following the runbook
...
upstream PR (LCLI-5 parked). Do NOT prune / merge tasks/back-510-json-output.

Then finalize LCLI-2 in the LORE repo: check its 2 ACs, notes, mark Done via a chore
commit on dev (the LCLI-1 pattern). LCLI-13(golden)/LCLI-3/LCLI-4 lock the schema BEFORE
LCLI-21 (lore's adapter).
```
...
| Item | Status |
| --- | --- |
| **LCLI-1** | **Done** — fork + in-fork task `back-510` delivered; committed `18eb886` |
...
| Fork branch `tasks/back-510-json-output` | pushed; carries task `back-510` (5 AC + plan + 3 DoD); LCLI-2's working branch |
| **LCLI-2** (next) | **To Do**, High, dep LCLI-1 ✓ — implement `--json` in the fork |
| **LCLI-5** | **parked** — upstream PR deferred (no upstream issue/PR exists); notes+comment recorded |
| **LCLI-32** | already carries the LCLI-21 dep (no action) |
| Active handovers | this one only (LCLI-1 kickoff handover archived in `18eb886`) |

## Next steps

1. **LCLI-2** — in `~/repos/Backlog.md` on `tasks/back-510-json-output`, implement the fork patch per `docs/runbooks/backlog-json-patch.md` §2–§3 and `docs/reference/backlog-json-schema.md`. Green gate + push to fork origin.
2. Finalize **LCLI-2** in the lore repo: `--check-ac 1 --check-ac 2`, append notes, mark **Done** via a `chore(LCLI-2)` commit on `dev` (LCLI-1 pattern).
3. **LCLI-13 (golden) / LCLI-3 / LCLI-4** — lock the schema with golden fixtures + build the patched binary + wire lore's capability probe, **before** **LCLI-21** (lore's JSON-only adapter).
...
- **Don't open an upstream PR** (LCLI-5 parked) and **don't merge/prune** `tasks/back-510-json-output` — it's LCLI-2's live working branch.
...
- **LCLI-1** → **Done**; notes record fork URL, `~/repos/Backlog.md` remote layout, `back-510`, and the collision hazard.
- **LCLI-5** → parked/re-scoped (notes + comment): upstream PR deferred; git-dep/version-floor work reassigned to LCLI-2/LCLI-21.
- **lore `dev`** `18eb886` = `chore(LCLI-1): Done — forked Backlog.md + in-fork --json task; park LCLI-5` (backlog edits + archived LCLI-1 kickoff handover). Pushed.
