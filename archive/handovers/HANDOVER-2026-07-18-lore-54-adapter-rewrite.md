# Handover — LCLI-53 shipped and pushed; LCLI-54 (full adapter rewrite) is next (LCLI-54, LCLI-5)

**Date**: 2026-07-18 | **Grounded against**: `dev` @ `0b27e47` (clean working tree, in sync with `origin/dev`, no open PRs) | **Backlog**: LCLI-53 Done; LCLI-54 To Do (unblocked); LCLI-5 In Progress (umbrella, unchanged)
...
Context: LCLI-53 (migrate lore's Backlog.md capability probe from this fork's
...
(f27f9ea "feat(LCLI-53): migrate capability probe to upstream's --json
contract"), and pushed to origin/dev. Nothing pending from that task.

LCLI-54 (rewrite src/adapters/backlog.ts's EnvelopeSchema/parseEnvelope/
...
§1-7) is now unblocked -- its only dependency, LCLI-53, is Done.

Start here:
1. `backlog task edit LCLI-54 -s "In Progress"`, read its full plain view
   (`backlog task view LCLI-54 --plain`) for the 4 ACs.
2. Re-read docs/reference/backlog-json-schema.md §8 (the full envelope/kind/
   ...
   of record for what LCLI-54 migrates to.
3. Also re-read the now-updated docs/reference/backlog-cli-contract.md §5
   ...
   what LCLI-53 already changed (the probe) and what's still pending here
   (the full adapter).
4. Before editing src/adapters/backlog.ts: grep every call site of
   ...
   across both src/ and test/. LCLI-53 found that the probe and a real
   listTasks() read hit the IDENTICAL `task list --json` subprocess call --
   LCLI-54's blast radius is the whole adapter, so expect several more such
...
6. Recapture golden fixtures against a real pinned-upstream build once the
   ...
   from LCLI-53 (see Critical context below) is proven and directly reusable
...
| LCLI-53 | **Done**, committed (`f27f9ea`) and pushed |
| LCLI-54 | To Do, **unblocked** (depends only on LCLI-53, now Done) |
| LCLI-5 | In Progress; umbrella, unchanged |
...
1. Start LCLI-54: mark In Progress, read its full ACs (`backlog task view LCLI-54 --plain`).
...
5. Recapture golden fixtures against a real pinned-upstream build (reuse the LCLI-53 verification technique below).
6. Rewrite `docs/reference/backlog-json-schema.md` §1-7 once the adapter migration lands.

## Critical context / traps

- **`EXPECTED_SCHEMA_VERSION` (string `"1"`) and `PROBE_SCHEMA_VERSION` (number `1`, new in LCLI-53) are deliberately separate constants** in `src/adapters/backlog.ts`. LCLI-54 is exactly the task that should finally retire `EXPECTED_SCHEMA_VERSION`'s fork-shaped usage — once `EnvelopeSchema`/`parseEnvelope` are rewritten onto upstream's shape, consider whether the two constants can merge (they should end up representing the same numeric `1`) or whether `PROBE_SCHEMA_VERSION` should simply be deleted in favor of the (now-migrated) shared one. Same applies to `TASK_LIST_KIND` (`"task-list"`, LCLI-53) vs. the still-fork-shaped `"taskList"` literal hardcoded in the `EnvelopeSchema` Zod union.
- **The probe and a real `listTasks()` read issue the identical `["task", "list", "--json"]` argv.** `test/backlog-adapter.test.ts`'s `defaultProbe()` currently disambiguates by call order (first match → upstream-shaped `PROBE_ENVELOPE`, later match → the old fork-shaped `TASK_LIST` golden) specifically so LCLI-53 didn't have to touch LCLI-54's territory. Once LCLI-54 migrates the real read too, this whole test harness distinction can likely collapse back to one shared upstream-shaped fixture — don't leave the two-tier fake in place after both sides target the same contract.
- **Real-binary verification technique (proven in LCLI-53, reuse directly):** clone `MrLesk/Backlog.md` into scratch, checkout the pinned commit (`22a091b570d44c4f302ca47e7fd36fa28ad8bcb0`), `bun install`, run **interpreted** (`bun src/cli.ts ...`) — not `bun build --compile`, which silently fails on this repo's external-volume checkout ([[external-volume-bun-exdev-traps]]). `--version` needs `cwd` = upstream's own checkout dir (its `getVersion()` reads `package.json` from `cwd` when there's no build-embedded version — a wrong `cwd` silently returns whatever *that* directory's `package.json` says, which is how LCLI-53 briefly chased a phantom "reports an unreleased version below the floor" bug). Real project reads (`task list --json`, `task view <id> --json`, `search`) need `cwd` = the real target project.
- **No `package.json` dependency exists** (explicit decision on LCLI-53: lore hasn't shipped, so a manually-built pinned-commit binary is enough for dev/test; real dependency wiring is deferred to whenever a tagged upstream release ships). Nothing for LCLI-54 to change here unless that release lands first.

## Do not repeat

- Don't reuse a single shared schema-version/kind constant across the probe and the full adapter while they're on different contracts — LCLI-53 already hit this (see above) and split them; LCLI-54 is the task that gets to un-split them once both sides agree.
- Don't verify contract-shape acceptance criteria with fake-spawn unit tests alone when the AC says "against a real build" — LCLI-53's real clone+build+run caught a `cwd`-dependent verification bug that unit tests alone missed; budget the same real-binary step for LCLI-54's golden-fixture recapture.

## System of record updated

- **LCLI-53** → Done, with full plan/notes/ACs/final-summary (see task history for detail); no further action needed on it.
- **Git**: `f27f9ea` (LCLI-53 implementation) and the prior handover-archive commit both pushed to `origin/dev` this session, per explicit user instruction ("stage, commit, push").
