# Handover — start LORE-17 (`lore init`) on a clean `dev` (LORE-16 merged & Done)

**Date**: 2026-06-25 | **Grounded against**: `dev`=`6fd305c` (== origin/dev, clean tree, **no open PRs**) | **Backlog**: LORE-16 **Done** (PR #13 merged `08411e3`); LORE-17 **To Do**, deps LORE-15+16 satisfied

## Paste-ready prompt for the next session

```
Continue building lore (OKF-native docs CLI, Bun+TS). LORE-16 (src/core/bundle.ts) is
...
START LORE-17 (`lore init`). FIRST: `backlog task view LORE-17 --plain` (ACs: #1 init
...
WHAT LORE-17 DELIVERS:
  ...
  - ALSO owns the two items DEFERRED from LORE-15:
  ...
  - bundle.ts's generateIndexes() was DEFERRED from LORE-16 (graph-only scope, Jeremy's call;
    confirmed NOT present in src/). LORE-17/M2 is where the byte-stable index/log format gets
...
READ FIRST (all verified present on dev):
  ...
  - docs/reference/cli-surface.md (LORE-17's Documentation pointer)
...
| Item | Status |
| --- | --- |
| PR #13 (LORE-16) | **MERGED** into dev as squash `08411e3` (2026-06-25 10:48 UTC). Both ACs were checked |
| LORE-16 | **Done**. Post-merge `/code-review max` = clean (no actionable bugs); notes recorded |
...
| LORE-17 (`lore init`) | **To Do** — next. Deps LORE-15+16 satisfied. Branch off clean dev |
| LORE-15 deferrals | JSON-Schema emit + above-fence modeline → land in LORE-17 |
| `generateIndexes()` | Deferred from LORE-16; confirmed NOT in src/. Decide LORE-17 vs M2/sync with Jeremy |

## Next steps

1. `backlog task view LORE-17 --plain`; read the 5 docs above; plan via `backlog instructions task-execution`.
...
- **Default is NO self-merge** ([[lore-git-workflow]]): Jeremy reviews/merges. He delegated the #13 admin-merge to me explicitly this session — that was a one-off, not a standing change. Ask before merging the LORE-17 PR.
...
- **LORE-16** → status **Done**; appended a note recording PR #13 merge SHA `08411e3`, the clean post-merge `/code-review max` disposition, and dev finalized at `6fd305c` (Backlog CLI).
- **dev** → finalized: `db96922` chore(LORE-16): mark Done; `6fd305c` docs: archive consumed handover lore-16-pr-open. Pushed.
