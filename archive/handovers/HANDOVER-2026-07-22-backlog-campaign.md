...
78 medium tasks LCLI-96..173, all To Do; scope "medium only" confirmed by the user
...
| Item | Status |
|---|---|
| Round 1 (doc-1, LCLI-69..95) | Complete — all 20 high-severity doc-2 findings resolved |
| Round 2 queue | 78 medium tasks LCLI-96..173, all **To Do** — nothing worked yet |
...
| Not queued | LCLI-42..45 (deferred v2 mcp / Confluence / importable-library) — recorded in doc-3 "Not queued", out of scope |
...
2. First-wave tie-break order (informational only): queue positions 1–6 are LCLI-96, LCLI-97 (both `adapter-backlog` → conflict, so only one of them per wave), LCLI-98..101 (`build-ci-config`). The conflict graph will thin same-cluster items across waves — expect wave 1 to draw one item per distinct cluster, not the literal first 6 rows.
...
- Don't re-run `backlog task create` for LCLI-96..173 — they already exist; re-running drafting/creation would duplicate all 78.
