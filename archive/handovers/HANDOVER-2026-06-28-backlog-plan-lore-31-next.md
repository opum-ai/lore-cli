# Handover — Backlog plan underway: LORE-20 shipped; next is Phase 1 / LORE-31 (lore graph)

**Date**: 2026-06-28 | **Grounded against**: `dev`=`main`=`origin/dev`=`origin/main`=`23f3733` (clean, on `dev`) | **Backlog**: LORE-20 **Done** (#25)

## Paste-ready prompt for the next session

```
State: dev == main == origin == 23f3733, clean, no open PRs. LORE-20 (lore schema export) is DONE
...
Locked decisions from the planning session:
  ...
  - Upstream Backlog.md --json: FORK IT PRIVATELY and implement only the --json we need, consume as a
    git dep — NO upstream issue, NO PR yet. ⇒ LORE-5 is PARKED (re-scope in backlog when reached).
    (Verified this session: no jeremy-newhouse/Backlog.md fork, no upstream issue/PR exist.)

Next task = Phase 1 task 2: LORE-31 `lore graph` (`backlog task view LORE-31 --plain`). Build the
shared `core/query.ts` graph-traversal helper HERE (graph→context→query reuse it), then the thin
command. Then LORE-34 `context` (+core/context.ts), LORE-33 `query` (BM25 in core/query.ts),
LORE-48 check follow-ups. All read the existing BundleGraph (loadBundle), clone the
...
"code-review", args:"max"}); FOLD verified findings (it found 15 real ones on LORE-20, incl. a
...
| Item | Status |
| --- | --- |
| **LORE-20** (lore schema export) | **Done** — squash #25 (`bdbbf27`) + chore `23f3733` |
...
| Phase 1 remaining | LORE-31 graph, LORE-34 context, LORE-33 query, LORE-48 check follow-ups |
| Phase 2 (fork+coupling) | LORE-1→2→{3,4}→21→{22,23,24,25,32}→{26,27}; **LORE-5 parked** |
| Phase 3 / 4 | LORE-37→36→38, LORE-39/40/41 / release (9,14,45), deferred (42,43,44) |
...
- **Don't hand-roll a 7th arg-parser/`usage()`/`plural()` copy as if it's fine** — code-review flags
  ...
  don't add NEW divergence (the LORE-20 copy lost new.ts's flag-as-value guard until folded).
...
- **LORE-20 → Done**; ACs #1/#2 checked; notes record the impl + the folded #25 code-review cluster.
