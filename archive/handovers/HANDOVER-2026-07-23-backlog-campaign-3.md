...
+ LCLI-206 reconciled as a duplicate = 19/55 resolved. 36 round-3 tasks remain, all To Do,
...
| Wave 19 (docs-first) | LCLI-198,199,200,204,208,209 merged (PR#187-192), integration clean |
| Wave 20 | LCLI-201,203,205,207,195,211 merged (PR#193-198), integration clean; LCLI-195 restored lint→GREEN |
| Wave 21 | LCLI-202,210,212,213,214,215 merged (PR#199-204), integration clean; LCLI-206 reconciled Done (dup of LCLI-205) |
...
- **Watch for DUPLICATE findings across clusters.** LCLI-206 (build-runtime) turned out to be the SAME test/helpers.ts change as LCLI-205 (build-ci-config) — reconciled Done without a redundant worker. Before dispatching, sanity-check whether a task's edit target was already changed by a merged sibling; if fully satisfied on dev, reconcile it Done (verify ACs, `backlog task edit -s Done`, add a Resolved row) rather than dispatching.
...
- **Don't treat the tracker `[file]` Note as the edit target** — it's the audit provenance citation. Read the task BODY/ACs for the real file(s) to edit (LCLI-203 cited context.ts, edited cli-surface.md).
- **Don't dispatch a worker for a task already resolved by a merged sibling/duplicate** — reconcile it Done (LCLI-206 pattern).
...
- **Don't co-schedule same-cluster tasks** or cross-cluster same-file tasks — file-citation read first (real counter-examples this round: LCLI-199/203 both cli-surface.md; LCLI-205/206 both test/helpers.ts).
- **Don't create a doc-5** — the round-3 tracker is doc-4, updated in place. doc-1=round1, doc-2=source review, doc-3=round2.
- **Lint is GREEN now** (LCLI-195) — do NOT let a worker re-redden biome; each worker keeps its own changed files clean. The campaign still gates on `bun test`+typecheck, but lint is now a trustworthy secondary signal.
