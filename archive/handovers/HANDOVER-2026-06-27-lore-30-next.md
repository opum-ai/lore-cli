# Handover — start LORE-30 (`lore check`: link/anchor + portability lint); LORE-28 merged & clean (LORE-30, LORE-28)

**Date**: 2026-06-27 | **Grounded against**: `dev`=`36bf336` (==origin, clean tree, checked out) | **Backlog**: LORE-28 **Done** (delivered #19), LORE-30 **To Do** (now unblocked)

## Paste-ready prompt for the next session

```
State: dev is 36bf336 (== origin), clean tree, no open PRs. LORE-28 (links.ts primitive) is DONE —
...
Task: START LORE-30 — `lore check`: whole-bundle internal cross-link + heading-anchor validation plus
...
FIRST run `backlog instructions overview`, then `backlog task view LORE-30 --plain` — its
...
Also (from LORE-29, already noted as the link-gate follow-up): the link gate MUST treat reserved
index.md / log.md targets as RESOLVED — LORE-29's generated root hubs link to frontmatter-free
...
commands/validate.ts). LORE-30 is labeled cmd,ci so it has BOTH a `lore check` command surface and the
...
LORE-30 dep = LORE-28 (satisfied). LORE-26 lore-sync is still blocked behind the LORE-1 fork chain —
don't pick it.
```
...
| Item | Status |
| --- | --- |
| **LORE-28** (links.ts primitive) | **Done** — squash-merged **#19** (`822592a`); branch deleted everywhere |
| `dev` | `36bf336` == origin; clean; **checked out**; green (580 tests, biome, tsc) |
| Open PRs | **none** |
| **LORE-30** (`lore check` lint) | **To Do**, dep **LORE-28 (satisfied)** → **unblocked**; carries the folded `validateLink` defects + reserved-index link-gate exclusion |
| LORE-1 / LORE-26 | LORE-1 To Do (unblocks the LORE-26 adapter chain); LORE-26 still blocked behind the LORE-1 fork |

## Next steps

1. `backlog instructions overview`; `backlog task view LORE-30 --plain` (read the folded defect list in full).
...
- **Reserved-index link-gate exclusion (from LORE-29):** generated root/sub `index.md` hubs link to frontmatter-free sub-indexes that are NOT graph concepts; `lore check` must resolve reserved `index.md`/`log.md` targets, or it reports every hub link broken.
...
- **LORE-28** → **Done**; both ACs checked; notes record the rebase-to-clear-CHANGELOG + the resourceFor decode-tolerant drift fix + "delivered via #19".
- **LORE-30** → comment: unblocked by #19; the folded `[LORE-28/validate, SHIPPING]` resourceFor-drift item is RESOLVED on #19; remaining scope = `validateLink` classifier defects + `validateLinks(graph)`+anchors + body-text Obsidian/MDX scan + reserved-index link-gate exclusion.
- **CHANGELOG.md** (dev) → LORE-28 `core/links.ts` Unreleased/Added entry incl. the decode-tolerant drift-compare note (both LORE-28 + LORE-29 entries coexist post-merge).
