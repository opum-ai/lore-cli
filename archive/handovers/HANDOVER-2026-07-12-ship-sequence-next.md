...
**Date**: 2026-07-12 | **Grounded against**: `feat/lore-9-release-pipeline` @ `a95a1c4` (pushed, clean) | **Backlog**: LORE-39, LORE-51, LORE-14, LORE-9 (PRs open), LORE-52 (filed, not started)
...
passed):
  ...
  #48 LORE-9 (release pipeline mechanics — compiled binaries + npm publish)
...
2. CRITICAL, the single biggest remaining unknown for LORE-9: once PR #48
...
3. Once LORE-9 merges: LORE-40 (docusaurus scaffold) was blocked because it
   extends files that only exist on the (still unmerged) LORE-39 branch —
   check whether LORE-39 (#45) has merged by then and branch LORE-40 off
   `dev` once it has.
4. LORE-5 (upstream Backlog.md --json PR) is gated on an actual release
   (needs a working `npm publish` step, which does not exist yet — LORE-9
   ...
   LORE-9 merges. See docs/runbooks/release-publishing.md for what's left
   (npm Trusted Publisher / OIDC setup on npmjs.com for all 6 packages).
5. LORE-52 (reconcile stale remark/unified doc references — recreated this
...
| Item | Status |
| --- | --- |
| PR #45 (LORE-39) | Open, awaiting user review |
| PR #46 (LORE-51) | Open, awaiting user review |
| PR #47 (LORE-14) | Open, awaiting user review |
| PR #48 (LORE-9) | **Newly opened this session**, awaiting user review |
| LORE-9 gates | Reconfirmed clean: 1439 tests pass, biome clean, tsc clean, `lore check` 0/0, `lore validate` 0 errors, actionlint clean |
| LORE-9's `release.yml` | actionlint-clean, 4 full `/code-review high` rounds folded, every constituent command manually reproduced locally; **still never executed in real GitHub Actions** (hard `workflow_dispatch`-on-default-branch constraint) |
| LORE-52 | Recreated this session (LOW), not started, fully independent |
...
2. Once any PR carrying `release.yml` merges to `dev`: trigger the workflow's first real GitHub Actions run and watch it closely. This is the single biggest remaining unknown in all of LORE-9.
3. After that: LORE-40 (once LORE-39/#45 merges), then LORE-5 (gated on real publish wiring + npm Trusted Publisher setup), with LORE-52 available anytime.
...
- **`bun build --compile`'s EXDEV trap** (LORE-14, cross-referenced in `DEVELOPMENT.md` + `docs/reference/tech-stack.md` §1): `--outfile` must be on the same filesystem as the checkout — crossing a device boundary silently produces a 0-byte binary at exit `0`.
...
- **A handover claimed a Backlog task ("LORE-52") was filed when it never actually was** — the task didn't exist when this session checked. Ground-truth-verify every claim in a restored handover before trusting it, especially "I already did X" claims about external systems (Backlog, git, PRs) — don't just trust the prose.

## System of record updated

- **LORE-9 Backlog task notes** → now carry all 4 `/code-review high` rounds' full disposition (round 3 was already present from the prior session; round 4 — the setup-job `needs:` regression + 2 cleanup findings — appended this session).
- **LORE-52** → recreated this session (the prior session's claim that it was "filed" did not hold up under ground-truth verification); LOW priority, doc-drift reconciliation, not started.
