# Handover — third backlog campaign, cursor at LORE-82 (LORE-69..87)
...
Cursor: LORE-82 — loadBundle silently skips unreadable directories, letting
rename/supersede commit against an incomplete graph. Queue order confirmed by
user on 2026-07-21 (independent fixes first, the LORE-78/79/80 rename-
...
Campaign-conventions sections for the rest). LORE-82 is closely related to
LORE-84 (just resolved) — both touch loadBundle/its callers.
```
...
| Item | Status |
| --- | --- |
| Tracker doc | doc-1, updated this session (Cursor → LORE-82, Queue = 14 items, LORE-84 moved to Resolved, two new campaign conventions recorded, two follow-up candidates flagged in Not-queued) |
| Review doc | doc-2, full Codex second-opinion review (201 confirmed findings, 25/25 clusters) — source of all queued tasks |
| Queue | 14 tasks remaining (LORE-82, 85, 69, 72, 71, 76, 77, 73, 74, 75, 80, 79, 78, 81), all `To Do`, `bug`, `High` priority, each with AC + a `--ref` to doc-2 |
| Branch | `dev`, clean (0 unpushed after this session's final push) |
| Leftover branches/PRs | none — `feature/LORE-84` fully merged (PR #69, rebase-merged, commit `ea7c8e8`) and pruned (local + remote) |
| Not queued | LORE-42/43/44/45 (deferred by recorded product decision) plus **two new, unfiled follow-up candidates from LORE-84** (see below) |

## Next steps

1. Run the per-issue lifecycle on **LORE-82** (`loadBundle silently skips
   unreadable directories, letting rename/supersede commit against an
   incomplete graph`): branch `feature/LORE-82` off `dev`, read the task's
...
2. Update doc-1's Cursor/Queue/Resolved/Session-log sections on the feature
   ...
   **LORE-85** (item #2 of the remaining queue).
3. Archive this handover to `archive/handovers/` and write the next one for
   LORE-85. Note: today's date (`2026-07-21`) already has FIVE prior
   archived handovers (base, `-2` through `-5`) — this session's own
   archival will need suffix `-6`.

## Critical context / traps

- **LORE-82 and the just-resolved LORE-84 are closely related** — both are
  about `loadBundle`'s behavior and its command-layer callers. LORE-84 just
  ...
  feature/LORE-84` is now merged into `dev`, or read the Resolved-table
  row for LORE-84 in doc-1) before starting LORE-82, since the two tasks
  may share code paths worth being consistent with (e.g. if LORE-82 needs
  ...
  discipline LORE-84 established).
- **Two new, unfiled follow-up candidates were recorded in doc-1's
  Not-queued section this session** (from LORE-84, sharpened by
  independent review): (1) `core/rewrite.ts`'s `rewriteInbound` still
  serializes with the default profile even after LORE-84's fix, creating a
  ...
  path, never `loadBundle`, so it's untouched by LORE-84's fix too — a
...
- **Three queued issues are one gap at three layers** (see doc-1's Campaign
  ...
  remaining queue) intentionally — LORE-80 last so its containment fix is
  available before/while working the other two. Read all three before
  starting any one. Note: since LORE-80 also touches `rewriteInbound`, its
...
- No `code-reviewer` subagent type is registered in this project — use
  `general-purpose` for the lifecycle's step-6 independent review. LORE-84's
...
- Don't assume every merged bugfix needs a CHANGELOG.md entry — check actual
  recent precedent first (none of LORE-68/70/83/84/86/87 added one; the
...
- When a fix threads a new option through many callers, check each caller
  ...
  mechanically repeating the same edit (LORE-84's `sync.ts` had one) —
