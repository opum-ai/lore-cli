...
queued (LCLI-198..250 = 53 new from the re-audit, + LCLI-195 lint chore + LCLI-197 check.ts
...
LCLI-195 (in this queue) is the task that fixes it. NEVER build a tracker update from a piped
...
| Round 2 follow-ups (this-day, earlier) | LCLI-195 (lint chore) + LCLI-197 (check.ts bug) → folded into round-3 queue; LCLI-196 (docker-e2e required check, needs-human) → Not queued |
| **Round 3 (doc-4) — INIT complete** | **55 agent-resolvable tasks queued (LCLI-198..250 + LCLI-195 + LCLI-197), all To Do, zero deps. 0 waves run. 0 resolved.** |
...
| Not queued | 7 needs-human findings + LCLI-196 + deferred LCLI-42/43/44/45 (see doc-4 "Not queued") |
...
4. **Docs-first tie-break**: the queue ordinals run docs (LCLI-198..203) → chore (204..210 + LCLI-195) → task (211..216) → enhancement (217..221) → bug (222..250 + LCLI-197). Earliest-ordinal ready + conflict-disjoint items get wave priority; not a strict slot order.
...
- **Lint baseline is pre-RED** (biome, untouched files) — gate merges on `bun test` + `bun run typecheck` only, NEVER `bun run lint`. LCLI-195 (queued) fixes it; once it merges, a future wave could re-enable a lint gate.
...
- **Not-queued items** (do NOT dispatch): the 7 needs-human re-audit findings (build/CI product calls, a symlink-follow bundle walk, a rollback-verification design question, a resource_base URL-validation call, a golden-schema-independence call), LCLI-196 (docker-e2e repo-admin), LCLI-42/43/44/45 (deferred v2).
- **`lore` is NOT on PATH** → `bun run src/cli.ts <cmd>`. `lore check` clean = "38 files, 0 errors, 0 warnings".

## Do not repeat

- **Don't re-run the round-3 re-audit or re-mint LCLI-198..250** — init is done; tasks exist and are committed (`66e17a5`).
...
- **ID-parse trap (bit init once):** when scripting `backlog task create` and parsing the created id, a title/description that MENTIONS "LORE-N" (e.g. "…import the canonical encoder from links.ts (LCLI-28 landed)") is matched by a naive `/LORE-\d+/` on stdout — parse the `File:` path line's `lore-<N>` or the id right after `Task `, not the first LORE-match. (One id was misparsed as LCLI-28, corrected to LCLI-210.)
- **Don't co-schedule same-cluster tasks** or cross-cluster same-file tasks — file-citation read first.
- **Don't set docker-e2e branch protection autonomously** (LCLI-196 is needs-human, repo-admin).
