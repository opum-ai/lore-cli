# Handover — LCLI-35.3 `lore supersede` (frontmatter wiring + optional inbound rewrite)

**Date**: 2026-06-28 | **Grounded against**: `dev`=`main`=`b4e5912` (== origin, clean tree, checked out dev) | **Backlog**: LCLI-35.2 **Done** (#23); LCLI-35.3 **To Do** (supersede, READY)

## Paste-ready prompt for the next session

```
State: dev == main == b4e5912 (== origin), clean tree, no open PRs. LCLI-35 (replace/rename/
supersede) split into LCLI-35.1/.2/.3. .1 (lore replace) Done (#22). .2 (lore rename) Done (#23,
squash d85fd2c) — it shipped the shared pure engine core/rewrite.ts `rewriteInbound`. NEXT and LAST:
LCLI-35.3 = `lore supersede` (LCLI-35 AC: supersession wired both ways + --rewrite-links). FIRST run
`backlog instructions overview`, then `backlog task view LCLI-35.3 --plain`.
...
| Item | Status |
| --- | --- |
| **LCLI-35.1** (`lore replace`) | **Done** — #22 (`89330e4`) |
| **LCLI-35.2** (`lore rename`) | **Done** — squash-merged **#23** (`d85fd2c`); chore `b4e5912`; branch deleted everywhere |
| `dev` / `main` | both `b4e5912` == origin; clean; **dev checked out**; main promoted to dev this session (== dev) |
| Open PRs | **none** |
| **LCLI-35.3** (`lore supersede`) | **To Do, READY** — reuses `rewriteInbound(move:false)`; last LCLI-35 subtask |
...
1. `backlog instructions overview` → `backlog task view LCLI-35.3 --plain` (deps: LCLI-28 ✓, and now the LCLI-35.2 engine ✓).
...
- **Admin-merge sequence (used for #22 and #23):** on a CLEAN dev — `gh pr merge <n> --squash --admin --subject "feat(LCLI-35.3): … (#n)" --body "…"` → ff local dev (gh-token) → delete branch (`gh api -X DELETE repos/jeremy-newhouse/lore/git/refs/heads/<branch>` + `git branch -D` + `git update-ref -d` tracking) → `chore(LCLI-35.3): mark Done` → optionally promote `main` (ff) when asked. Only when Jeremy says "admin-merge". This session promoted main→dev, so **dev==main now**; don't assume they stay synced.
...
- **LCLI-35.2** → **Done**; all 3 ACs checked; notes record the surgical-splice implementation + the folded #23 `/code-review max` fixes (15, data-loss cluster) + "delivered via #23 / d85fd2c".
