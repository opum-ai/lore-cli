...
order confirmed by the user on 2026-07-24 (lowest-risk-first: LORE-258, 259,
...
- NOT queued — do NOT dispatch: LORE-253 (blocked on the upstream --json tag,
  external event), LORE-257 (needs-human repo-admin ruleset toggle, like
  LORE-196), LORE-42/43/44/45 (deferred-v2). See tracker "Not queued".
- LORE-260 is a LARGE feature (interactive-wizard onboarding); its design is
...
- LORE-256 and LORE-260 touch Windows/CI-verified paths: the windows-latest CI
  ...
  green but NOT yet a required check (that's LORE-257, human-only).
...
- The locked decisions/traps in the paste-ready prompt above are the load-bearing ones (not-queued set, patched backlog binary, cross-device compile trap, windows-CI/ruleset facts, LORE-260 size).
