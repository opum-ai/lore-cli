# Handover — Phase 1 continues: LORE-31 shipped; next is LORE-34 `lore context` (LORE-34, 33, 48)

**Date**: 2026-06-29 | **Grounded against**: `dev`=`origin/dev`=`e950ad2` (clean, on `dev`); `main`=`23f3733` (intentionally behind) | **Backlog**: LORE-31 **Done** (#26)

## Paste-ready prompt for the next session

```
State: dev == origin/dev == e950ad2, clean, no open PRs. main == 23f3733 (intentionally behind; do
NOT promote unless asked). LORE-31 (lore graph) is DONE (squash #26 = 1fb8b27, then chore e950ad2).
...
Next task = Phase 1 task 3: LORE-34 `lore context` (`backlog task view LORE-34 --plain`). Build a NEW
pure core/context.ts (token-budgeted graph expansion) that REUSES the shared core/query.ts subgraph()
traversal already shipped in LORE-31, then a thin commands/context.ts. Contract is locked in
...
FOLD HERE the deferred LORE-31 finding: memoize the BundleGraph adjacency in core/query.ts (build it
...
| Item | Status |
| --- | --- |
| **LORE-31** (lore graph) | **Done** — squash #26 (`1fb8b27`) + chore (`e950ad2`); both ACs checked |
...
| Phase 1 remaining | **LORE-34 context (next)**, LORE-33 query, LORE-48 check follow-ups |
| Shared foundation shipped | `core/query.ts` `subgraph()` (undirected, cycle-tolerant, depth-bounded) — context/orphans reuse it |
| Phase 2 (fork+coupling) | LORE-1→2→{3,4}→21→{22,23,24,25,32}→{26-sync,27}; **LORE-5 parked**; nothing started |

## Next steps

1. **LORE-34 `lore context`** — `backlog task view LORE-34 --plain`; mark In Progress. New pure
...
3. Then **LORE-33 `query`** (BM25 in the existing `core/query.ts`) and **LORE-48 `check` follow-ups**
   (extend `core/check.ts`/`commands/check.ts`; do it BEFORE LORE-27 since both edit `check`).
...
- **Code review is the WORKFLOW one**: `Skill code-review args "max"` (it calls `Workflow{name:"code-review"}`),
  NOT inline /review. Budget a real fold pass (LORE-31 surfaced 14 verified findings). [[code-review-vs-review-command]]
- **`--json` is the only machine-JSON path.** Do NOT add a per-command `--format json` value — on LORE-31 it
...
- **Don't trust that In-Progress backlog notes survived the dev-sync reset** — they didn't this session
  (had to reconstruct LORE-31's notes/ACs after `reset --hard`).
...
- **LORE-31 → Done**; ACs #1/#2 checked; notes record the impl + the folded #26 code-review cluster +
