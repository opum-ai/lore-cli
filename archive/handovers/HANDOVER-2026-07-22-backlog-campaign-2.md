# Handover — Codex review follow-up campaign, round 2 (waves: 2 done this session, issues: LORE-96,98,102,107,110,114,97,99,103,108,111,115 + dups 127,131,173)
...
in flight. 15 tasks Done (see doc-3 Resolved rows 1-15): wave 1 = LORE-96,98,102,107,
110,114 (PRs #92-#97); wave 2 = LORE-97,99,103,108,111,115 (PRs #98-#103); plus 3
resolved-by-merge duplicates closed at settlement (LORE-127,131 by LORE-107; LORE-173
by LORE-115). 2 follow-ups filed: LORE-174 (cmd-crud-a, from wave-1 integration review)
and LORE-175 (cli-entry-state low, from wave-2 integration review). 65 tasks remain open.
...
| Item | Status |
|---|---|
| Round 1 (doc-1, LORE-69..95) | Complete — all 20 high-severity doc-2 findings resolved (prior session) |
| Round 2 this session | **Waves 1 + 2 complete: 12 tasks implemented+reviewed+merged, 3 duplicates closed = 15 Done** |
| Wave 1 | LORE-96,98,102,107,110,114 — PRs #92–#97, all Fable-approved first pass, 0 escalations |
| Wave 2 | LORE-97,99,103,108,111,115 — PRs #98–#103, all Fable-approved (LORE-115 after 1 fix round), 0 escalations |
| Duplicates closed | LORE-127,131 (resolved-by-merge by LORE-107); LORE-173 (by LORE-115) — verified vs merged dev, marked Done w/ evidence, no re-fix |
| Follow-ups filed | LORE-174 (cmd-crud-a: `lore new` default-slug reserved-stem gap); LORE-175 (cli-entry-state, low: readConfigText denied error omits errno `code`) |
| Queue | **65 open** of 80 total (78 original medium + LORE-174 + LORE-175). Nothing in flight. |
| Formal deps | **None** across all tasks (YAML-verified). Readiness gated solely by live file-conflict graph. |
| Not queued | LORE-42..45 (deferred v2 mcp / Confluence / importable-library) — out of scope |
...
2. **Wave-3 tie-break candidates (informational — recompute live, do NOT hardcode):** next task per distinct cluster in queue order is roughly LORE-100 (build-ci-config), LORE-104 (build-runtime), LORE-109 (cli-entry-state), LORE-112 (cmd-check), LORE-116 (cmd-crud-a), LORE-117 (cmd-crud-b). **But** LORE-116 and LORE-174 and LORE-175 conflicts must be checked: LORE-116/LORE-174 are both cmd-crud-a; LORE-175 is cli-entry-state and touches src/config.ts (conflicts with LORE-108-area but LORE-108 is done). Verify file citations per R4b before committing the wave.

## Critical context / traps

- **Same-cluster ⇒ treat as conflicting; different cluster ≠ safe.** The file-citation read (R4b) is load-bearing: this session it caught 3 real cross-cluster duplicates (LORE-107↔127/131 in cli.ts/help.test.ts; LORE-115↔173 in output.ts/renderTaskSummaryRows; LORE-114↔174 in new.ts). **Budget the settlement step to re-check remaining flagged duplicates and close any that are resolved-by-merge** (mark Done w/ evidence, don't re-fix). No known unclosed duplicates remain after this session, but always re-scan when a shared-file task lands.
...
- **Backlog ID minting** (task create/promote/demote) stays sequential + orchestrator-only + primary-checkout-only. This session minted LORE-174, LORE-175 that way. Workers only `task edit` their own file (safe in parallel).
...
- Don't re-run `backlog task create` for LORE-96..175 — they exist; re-creating duplicates them.
- Don't re-fix LORE-127, 131, 173 — already Done (resolved-by-merge, evidence in doc-3 Resolved rows 13–15).
