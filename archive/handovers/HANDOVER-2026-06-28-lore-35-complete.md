# Handover — LCLI-35 refactoring trio COMPLETE; next: backlog-fork chain or standalone graph commands

**Date**: 2026-06-28 | **Grounded against**: `dev`=`1759f7b` (== origin, clean tree, dev checked out) | **Backlog**: LCLI-35 **Done** (all of .1/.2/.3 Done)

## Paste-ready prompt for the next session

```
State: dev == origin/dev == 1759f7b, clean, no open PRs. main is BEHIND at b4e5912 (not promoted —
promote with a ff only if asked). LCLI-35 (lore replace/rename/supersede) is fully DONE: .1 #22, .2 #23,
.3 #24 (squash c018fe7), parent LCLI-35 Done. The shared pure engine core/rewrite.ts rewriteInbound now
...
No work is mid-flight. Two candidate fronts (ask the user which):
  1) backlog-fork chain (HIGH, the critical path): LCLI-1 (fork Backlog.md + --json task) → LCLI-2
     (shared task-json serializer + --json on read cmds) → LCLI-21 (backlog.ts adapter: JSON read / CLI
     write) → LCLI-22 (managed-block.ts task block) → LCLI-23 (reconcile rollup) → LCLI-24 (link/unlink)
     → LCLI-26 (sync) / LCLI-27 (check drift gate). This unblocks the doc↔task coupling; everything here
     is blocked behind LCLI-1. Verify deps with `backlog task view --plain` (NOT grep — see memory).
  2) standalone graph/query commands that need no fork: LCLI-31 (lore graph), LCLI-32 (orphans),
     LCLI-33 (lore query), LCLI-20 (lore schema export), LCLI-25 (lore tasks), LCLI-36 (lore agents).
     These read the existing bundle graph and ship like LCLI-35's commands did.
...
| Item | Status |
| --- | --- |
| **LCLI-35** (replace/rename/supersede) | **Done** — parent + all 3 subtasks |
| LCLI-35.1 / .2 / .3 | **Done** — #22 `89330e4` / #23 `d85fd2c` / #24 `c018fe7` |
...
| backlog-fork cluster (LCLI-1/2/21/22/23/24/26/27) | **To Do, HIGH** — blocked behind LCLI-1 fork |
| standalone graph cmds (LCLI-31/32/33/20/25/36) | **To Do** — no fork dependency |
...
- **LCLI-35.3 → Done**; ACs #1/#2 checked; notes record the implementation + the folded #24 `/code-review
  max` cluster (15 confirmed). **LCLI-35 parent → Done** (all subtasks delivered).
