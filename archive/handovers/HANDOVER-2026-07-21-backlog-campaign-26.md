# Handover — eleventh (final) backlog campaign session, cursor at LORE-93 (item 8 of 8)
...
Cursor: LORE-93 — five `ensureDir` call sites (new.ts:127, agents.ts:72, sync.ts:174,
...
Read the task itself first (`backlog task view LORE-93 --plain`) — do not trust
...
AC#5 is actually asking for (that would be new scope well beyond LORE-93, and
...
LORE-94's own fix (`isManagedSchemasDir`'s pruning gate): LORE-94 stopped
...
conflate them or assume LORE-94 already covers this.
...
campaign needed a re-prompt before learning to ask for this up front — LORE-89's
and LORE-91's own prompts show the exact working phrasing). THEN update the
...
paid for:
1. (Session 33/LORE-89) A notes commit made AFTER `gh pr merge` had already
...
2. (Session 35/LORE-91, THIS session) An UNPUSHED local archive commit from a
...
| Item | Status |
| --- | --- |
| Tracker doc | doc-1, updated this session (LORE-91 moved to Resolved as row #34, Cursor → LORE-93, Queue is now the single final item, session-log entry appended for session 35) |
| Queue | 1 task remaining: LORE-93 (`bug`, `To Do`) — the final item |
| Resolved this session | LORE-91 — see Resolved table row 34 for full evidence summary |
...
| Leftover branches/PRs | none — `feature/LORE-91` was merged (PR #90) and pruned both remotely and locally; confirmed via `git branch -a` and `gh pr list --state open` |
| Not queued | LORE-42/43/44/45 (deferred by recorded product decisions, unchanged) plus the accumulated independent-review follow-up candidates from sessions 9-35 (see tracker's Not-queued section) |

## Next steps

1. Run the per-issue lifecycle on **LORE-93**: **branch FIRST this time** (`git checkout -b feature/LORE-93 dev`) — this session skipped that step initially for LORE-91 and had to recover mid-stream; don't repeat it. Read the task's AC (`backlog task view LORE-93 --plain`), re-verify all 5+ `ensureDir` call sites fresh, investigate the blanket-fix-at-`ensureDir` design sketched above (verify it's still sound, don't just implement it blindly), resolve AC#5's all-or-nothing question per the reasoning above (or overturn it with better evidence), implement (AC#1-#5), add regression tests for at least rename and new (AC#6), verify, review, commit fix+tests, run independent review (ask for complete findings in one response), **commit and push the review-outcome notes update BEFORE calling `gh pr merge`**, THEN update the tracker, PR, self-merge, prune.
...
- Don't trust a synthetic test suite alone as proof a destructive/security fix is real — live-CLI-verify (real scratch bundle, real symlink, `git stash` pre/post comparison) too, this campaign's now well-established pattern for exactly this class of fix (LORE-76/77/90/92/94/95/88/91 all did this).
...
- Don't re-derive `assertNoSymlinkInPath`/`findSymlinkSegment` from scratch for LORE-93 — both already exist in `fswrite.ts`, proven across 4 prior tasks this campaign (LORE-76/77/94/92); reuse, don't reinvent.
