# Handover — Backlog campaign session 6: LORE-65 (docker/e2e coupling mediums)
...
(`backlog doc view doc-1 --plain`). Cursor: LORE-65 — docker/e2e: four
...
LORE-65 depends only on LORE-56 (Done — this whole harness exists because of
...
wrong assumption in its own filing task's description. For LORE-65
...
Three new campaign conventions from the LORE-64 session, apply here too if
relevant: (a) `lore new` can NEVER populate a profile-declared custom
frontmatter field — irrelevant to LORE-65's own surface (Backlog coupling,
...
pre-existing drift (LORE-64 found LORE-68 exactly this way: a genuine,
pre-existing broken-link bug in `stories/e2e-renamed-story-f1.md`, unrelated
to LORE-64's own change, that a bare "lore check exits 0" assertion would
...
state (LORE-63's lesson, reused again in LORE-64 for its custom-type doc) —
LORE-65's AC1 multi-doc SET/REPLACE case in particular needs a task linked
from exactly two docs and nothing else, so neither doc's unrelated state can
mask which one's unlink actually ran.

LORE-68 (the broken-link bug LORE-64 discovered and filed) is NOT queued —
...
| Branches / PRs | No `feature/*` branches local or remote; no open PRs (`feature/LORE-64` deleted both sides — confirmed via `gh api .../branches/feature/LORE-64` -> 404, and `git fetch --prune` removed the stale local remote-tracking ref). |
...
| LORE-65 dependencies | Lists only `LORE-56` (Done). Fully unblocked, no re-verification needed next session. |
| LORE-66 | Still queued after LORE-65. Depends on LORE-61 (Done, unblocked). |
| LORE-42/43/44/45 | Still parked in tracker "Not queued" — deferred by recorded product decisions. Do not pick up. |

## Next steps

1. Per-issue lifecycle on LORE-65 (`backlog task view LORE-65 --plain` for the five ACs): branch `feature/LORE-65` off `dev`, plan on the task.
...
4. AC2: a backlog-side `backlog task edit <id> --title "..."` real-binary rename (which renames the task's own file) followed by `lore sync` — assert the sweep commit picks it up and the managed-block row's href follows the new filename. Exploratory-first for `backlog task archive <id>` on a linked task: run it once, observe the real exit code/signature (likely the same missing-task exit-1-null signature LORE-62 already pinned for a vanished file, but confirm), then pin the observed behavior as the fixed expectation — do not assume the archived case is identical to vanished before checking.
...
9. Advance the tracker cursor to LORE-66 on the branch (atomic with the fix), append the session-log entry via `backlog doc update doc-1`.
10. Archive this handover (check `archive/handovers/` for a name collision first), write the next one pointed at LORE-66, push dev.
...
- **When proving a command is UNAFFECTED by a surface under test, diff before/after output — don't assert a bare exit code/baseline** (LORE-64's lesson): the bundle can already carry unrelated pre-existing drift (proven by LORE-68), so a naive "should be exit 0" assertion can false-fail on something that was never this task's concern, or worse mask a real regression that happens to share an exit code with the pre-existing drift.
- **Isolate a coupling/config-surface probe onto a fresh, minimally-coupled concept** (LORE-63/64's lesson) — LORE-65's AC1 multi-doc case especially needs a task linked from exactly two docs and nothing else.
- **`lore schema export` (full, no `--type`) prunes any `.lore/schemas/*.schema.json` a profile no longer declares** — not directly relevant to LORE-65, but a reminder that any custom-profile/schema state introduced mid-harness must be fully restored (delete + re-export) before later phases that assume the default six.
...
- **LORE-68 is filed but deliberately NOT queued** — it's a real, verified bug (see State table), but out of scope for this campaign's queue unless the user asks otherwise.
...
- Trusting a filing task's own proposed technique/shape/line-number citations without independently re-reading the actual current source first (LORE-61/62/63/64 each found at least one wrong assumption this way).
- Asserting a command is "unaffected" by a surface under test via a bare exit-code/baseline assumption instead of a before/after diff — LORE-64's AC4 had to be rewritten after a real run disproved the naive version (the bundle already carried unrelated drift, LORE-68).
