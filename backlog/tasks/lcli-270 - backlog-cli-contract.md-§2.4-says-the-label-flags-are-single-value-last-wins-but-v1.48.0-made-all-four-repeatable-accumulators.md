...
## Observed
Found by the LCLI-265 review gate (round 5, wave 1) while auditing an adjacent claim; deliberately left out of LCLI-265's scope because it is a different drift class.
...
8. Handle architecture.md's stale adapter sketch (~line 138, secondary AC#5):
   found it more stale than flagged (also describes the pre-LCLI-54
...
9. Check CHANGELOG precedent via `git show` on LCLI-265's fix-pass commits
...
sketch also still describes the pre-LCLI-54 fork-consumption model ("lore
...
CHANGELOG: checked `git show` on LCLI-265's fix-pass commits (bbc084b, 7e07262,
...
flagged - also described the pre-LCLI-54 fork-consumption model and the old
...
on LCLI-265's fix-pass commits (bbc084b, 7e07262, 24010c6, 9508c63, 135bb3b,
