...
1. src/core/reconcile.ts (pure, no fs/spawn/clock): export reconcileStatus(taskStatuses: readonly string[], statusFlow: readonly string[]): "todo"|"in-progress"|"done"|null.
   - statusFlow is the ordered config-driven set (backlog/config.yml `statuses:`), index 0 = not-started, last = terminal; reading the config file is a command-layer concern (LCLI-24+), not this engine's.
...
3. Not wired into cli.ts (LCLI-24's job). Record the corner-case decision in task notes.
