...
## Observed
Recorded by the LCLI-255 reviewer and confirmed at merge time; documented in round-4 tracker doc-5, never fixed.
...
Every in-workflow guard LCLI-255 added is defeated by this, because the attacker supplies the workflow file itself.
...
2. release.yml: add `environment: release` to the `publish` job with an inline
   ...
   two repo-admin steps above are done. Verify all 6 LCLI-255 guarantees still
...
4. docs/runbooks/release-publishing.md: update Step 1's "Environment name" field
   ...
   protection and the workflow_dispatch-any-ref exposure LCLI-268 describes
   remains open.
5. CHANGELOG.md: add an [Unreleased]/Added entry for LCLI-268, verified against
...
7. Mark Done with notes naming verification evidence and the outstanding
   repo-admin steps; commit in small Conventional Commits with Refs: LCLI-268
   trailers, including backlog/tasks/ edits; push feature/LCLI-268.
<!-- SECTION:PLAN:END -->
...
"Repo-admin setup for the release Environment (LCLI-268)" — two explicit
...
respected per the task's scope split (same as LCLI-196/LCLI-257).
...
`git diff > /tmp/LCLI-268-release-yml.patch`, deleted the `environment:
...
/tmp/LCLI-268-release-yml.patch` (not git stash, per hard rules), re-ran ->
10 pass / 0 fail.

LCLI-255 guarantees re-verified present after the edit (grep + full suite):
...
Fix pass (post-review): CHANGELOG.md's bolded lead falsely claimed the environment: release declaration was 'closing' the workflow_dispatch-on-any-ref exposure, contradicting the same entry's own later claim that 'the declaration alone does not yet provide protection.' Reworded the lead to call environment: release 'the out-of-file hook for closing' the exposure, inert until two repo-admin steps are done -- no closure claim, no self-contradiction. Also corrected docs/runbooks/release-publishing.md's deployment-branch-policy guidance: it presented a branch policy restricting deploys to main/release/* as a standalone alternative to required reviewers, but verified live via gh api that main carries NO branch protection (repos/jeremy-newhouse/lore/branches/main/protection -> 404 'Branch not protected') and the repo's only ruleset (require-docker-e2e-on-dev, id 19698059) targets refs/heads/dev only and enforces a required status check, not PR-only pushes. So today a deployment-branch policy restricted to main would NOT stop the attack this section exists to prevent -- an actor with write access can push a forged release.yml straight to unprotected main and dispatch it there. The runbook now states required reviewers as the option that holds regardless of branch protection, and states the branch-policy option is only sound if the allowed branch(es) are themselves protected against direct pushes -- which, in this repo today, they are not. The user should treat this as a live fact to address when doing the repo-admin half (either configure required reviewers on the release Environment, or add branch protection/a ruleset barring direct pushes to whichever branch a deployment-branch policy would allow). Also moved the unnumbered 'Repo-admin setup for the release Environment (LCLI-268)' section from between Steps 1 and 2 to under ## Prerequisites, preserving the Steps section's 1/2/3 reading order; heading text and anchor (#repo-admin-setup-for-the-release-environment-lore-268) unchanged so the three existing cross-references (First-release checklist, Step 1, Step 2) still resolve, only two forward/backward-reference words ('below'/'above') were flipped to match the new position. lore check still reports 40 files/0/0. release.yml and test/release-workflow.test.ts untouched by this pass (confirmed via git diff).

Second fix pass (post-review): reviewer confirmed all 5 ACs, the CHANGELOG lead fix, the runbook body's branch-policy fix, the section move (anchor byte-identical, all 3 cross-references resolve, lore check green), and the untouched-19-claims/files at blob-SHA level -- none of that revisited or undone.

Fixed the MAJOR: First-release checklist item 2 still read "(required reviewers and/or a deployment branch policy)" with a closing sentence implying either option closes the exposure -- the same defect the prior pass fixed in the section body but left standing in the checklist, the doc's actual walk-through sequence. Reworded the parenthetical to "(required reviewers -- a deployment branch policy alone is not sufficient in this repo today; see the linked section)", deferring the branch-policy nuance to the already-corrected Repo-admin section instead of re-presenting it as a co-equal option.

Fixed the MINOR: L37-38's "Every item elaborates on a `## Steps` section below it" went false once the prior pass moved the Repo-admin section out of `## Steps` into `## Prerequisites` -- checklist item 2 now points at a Prerequisites subsection, not a Steps section. Changed to "a section below it" (still true: every linked section is structurally below the checklist; verified all 8 checklist items' link targets).

Folded in every nit since the major was being fixed anyway:
(1) CHANGELOG.md line 13's ragged wrap (84 chars vs 100-104 on lines 11/12/14) -- artifact of the prior pass's un-reflowed insertion. Reflowed lines 12-14 to 94/104/91 chars, content unchanged.
(2) CHANGELOG.md lines 27-31 and this task's Final Summary both still said "required reviewers and/or a deployment branch policy" -- the pre-correction framing. Reworded both to "required reviewers (a deployment branch policy alone is not sufficient in this repo today)" to match the corrected runbook guidance.
(3) Added two unstated caveats to the Repo-admin setup section's "Create the release GitHub Environment" bullet (docs/runbooks/release-publishing.md): (a) required reviewers only holds if the attacker isn't themselves a listed reviewer -- named GitHub's "Prevent self-review" environment option; (b) "a ruleset that requires a PR and bars pushing straight to the branch" omitted that bypass_actors defeat this. Verified live via `gh api repos/jeremy-newhouse/lore/rulesets/19698059` before writing it down: this repo's own require-docker-e2e-on-dev ruleset carries `"bypass_actors":[{"actor_id":5,"actor_type":"RepositoryRole","bypass_mode":"always"}]` -- named that fact explicitly (RepositoryRole id 5 = admin) rather than stating only the general risk.
(4) Corrected this task's own prior undercount: the first fix pass's notes said "only two forward/backward-reference words ('below'/'above') were flipped" describing its Repo-admin-section move. Re-diffed commit c6e2b3d against its parent: actually 2 were flipped (was L163 "above"->"below", was L215 "below"->"above") and 1 was added (was L115, "(Step 1)" -> "(Step 1, below)") -- an undercount of the pass's own work, not an overclaim. (These three lines are now at L116/L176/L228 after this pass's edits; all three re-verified to point at the correct side of the document.)

Consistency sweep (required discipline for this pass): grepped the whole runbook + CHANGELOG for every remaining "and/or" occurrence after these edits -- zero hits in either file. Re-checked every above/below/earlier/later/next/following/preceding hit: 17 in the runbook, 20 in CHANGELOG.md -- each resolves to a section/clause genuinely on the stated side (including the untouched pre-existing ones, e.g. "tag below points at a commit" at runbook L74, which is a git-history metaphor, not a doc cross-reference, and correctly left alone); none stale, none newly introduced by this pass's edits.

Verified: bun test 2177 pass / 0 fail (unchanged from before this pass). bun run lore check: 40 files, 0 errors, 0 warnings (unchanged). bun run lint (biome check .): clean, 112 files. bun run typecheck (tsc --noEmit): clean. `git diff e1d5109..HEAD -- .github/workflows/release.yml test/release-workflow.test.ts` is empty -- neither file touched this pass, confirming both stay byte-identical to what the reviewer already verified.

Files touched this pass: docs/runbooks/release-publishing.md, CHANGELOG.md, backlog/tasks/lore-268*. No sibling-task files touched.
<!-- SECTION:NOTES:END -->

## Final Summary

<!-- SECTION:FINAL_SUMMARY:BEGIN -->
Shipped the out-of-file hook for closing the workflow_dispatch-any-ref vector in .github/workflows/release.yml's publish job: added `environment: release` (GitHub Environment protection rules live in repo Settings, not workflow content, so they survive a wholesale-replaced release.yml on an attacker branch -- unlike an in-workflow ref guard, which was explicitly rejected and documented as insufficient). This does not itself close the exposure -- it stays fully open until a repo admin completes two follow-up steps: creating the release GitHub Environment with required reviewers (a deployment branch policy alone is not sufficient in this repo today), and setting npm Trusted Publisher's Environment name field to release for all six packages. Extended test/release-workflow.test.ts to pin the new field (mutation-verified: deleting the line drops 10/10 pass to 9 pass/1 fail; restored). Added a 'Repo-admin setup for the release Environment (LCLI-268)' section to docs/runbooks/release-publishing.md naming both manual steps and stating plainly that until both are done the environment declaration is inert and the exposure remains open.

First review round found the CHANGELOG lead implying the exposure was already closed and the runbook's branch-policy guidance presenting it as a co-equal alternative to required reviewers; both corrected, and the Repo-admin section moved under Prerequisites for reading-order cleanliness (heading/anchor unchanged, cross-references still resolve).

Second review round found the same and/or framing still standing in the First-release checklist item that actually points at the corrected section (the doc's real walk-through surface) -- fixed to name required reviewers and defer the branch-policy nuance to the linked section. Also fixed: a stale "every item elaborates on a `## Steps` section" claim broken by the section move, a ragged CHANGELOG line wrap, the same and/or framing lingering in CHANGELOG.md and this summary, two unstated caveats added to the Repo-admin guidance (required-reviewers self-review exposure, and ruleset bypass_actors -- verified live that this repo's own dev ruleset carries a RepositoryRole-5/admin bypass actor with bypass_mode "always"), and an undercount in this task's own notes about how many directional words the section move touched.

All six LCLI-255 guarantees verified intact across both rounds. Verified: bun test 2177/0 pass, lore check 40 files/0/0, lint and typecheck clean, actionlint clean on release.yml (round 1); bun test 2177/0 pass, lore check 40 files/0/0, lint and typecheck clean, and .github/workflows/release.yml / test/release-workflow.test.ts confirmed byte-identical to the pre-round-2 commit via git diff (round 2). Did not create the GitHub Environment, configure branch protection, or touch any other repo setting -- left to the repo admin per the task's scope split.
