...
2. Trace every call site of EXPECTED_SCHEMA_VERSION / EnvelopeSchema's kind literals / data-key
   ...
   LCLI-53's probe and a real listTasks() read shared one collision point and LCLI-54's blast
...
7. Rewrite docs/reference/backlog-json-schema.md §1-7 to describe upstream's contract as current
   ...
   LCLI-54" callouts in docs/reference/backlog-cli-contract.md (§2.2, §5) and
...
8. Verify: bun test, bun run typecheck, bun run lint, bun run lore check, plus a real end-to-end
   ...
   real, per the AC#3 "against a real build" bar LCLI-53 set.
<!-- SECTION:PLAN:END -->
...
Real end-to-end verification (not just fake-spawn unit tests, per LCLI-53's precedent and this
...
field contract, then ran the interpreted CLI (bun src/cli.ts, same technique as LCLI-53 and
...
needed by LCLI-53 because the probe and a real listTasks() read shared an argv but targeted different
...
"still pending LCLI-54" / "this flips on migration" callouts in docs/reference/backlog-cli-contract.md
...
pre-existing infos unrelated to this change, same ones LCLI-53 noted), bun run lore check (37 files, 0
