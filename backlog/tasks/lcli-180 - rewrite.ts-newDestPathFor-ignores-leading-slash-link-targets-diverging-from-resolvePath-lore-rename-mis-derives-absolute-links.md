...
After LCLI-133 aligned core/bundle.ts's resolvePath with check.ts's leading-'/' handling, src/core/rewrite.ts's newDestPathFor (~line 475) still inlines idFromPath(posix.join(dir, decoded)) with NO leading-slash special case — even though its doc comment claims it 'reuses resolvePath' (resolvePath is imported at rewrite.ts:76 but never called). Consequence after LCLI-133: for a '/'-absolute frontmatter ref or link the graph (via resolvePath) counts it as an inbound edge to the ROOT concept, but lore rename's per-link rewriter mis-derives that link's target id — renaming the true target selects the inbound file yet SKIPS the '/'-absolute link (it rots), and renaming a decoy concept at the dir-joined path can HIJACK a '/'-absolute link when the file is inbound via another edge. Both behaviors pre-existed LCLI-133 (unchanged by that diff) but are now inconsistent with the aligned graph/check resolution. Flagged by the wave-6 LCLI-133 review.
...
6. Run full bun test and bun run typecheck, both green. Check off ACs. Mark Done. Commit with Refs: LCLI-180 trailer and push feature/LCLI-180.
<!-- SECTION:PLAN:END -->
