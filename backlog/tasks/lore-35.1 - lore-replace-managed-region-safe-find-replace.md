...
Literal/regex find-replace across one doc or the whole bundle, skipping lore-managed regions (today: the <!-- lore:index:begin/end --> listing block). Pure core engine (core/replace.ts) + thin command (commands/replace.ts). Delivers LORE-35 AC#1.
