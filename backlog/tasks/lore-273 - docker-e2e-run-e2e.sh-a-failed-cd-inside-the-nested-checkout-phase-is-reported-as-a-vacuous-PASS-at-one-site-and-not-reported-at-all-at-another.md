...
## Observed
Found by LORE-269's review gate (round 5, wave 2) while auditing that task's AC#3 `cd` sweep, and documented in the script's own comment block rather than fixed (LORE-269's scope was the top-level `cd /workspace` fall-through).

Both sites are **safe** in the sense LORE-269 cared about — neither can leak the script's process-level cwd nor mutate the wrong directory, because each `cd` is inside a subshell / command substitution / `bash -c` child and is `&&`-chained. The defect is in **reporting**:
