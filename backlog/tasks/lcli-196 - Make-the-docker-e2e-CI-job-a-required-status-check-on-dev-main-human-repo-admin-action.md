...
LCLI-100 wired the hermetic Docker E2E harness (~300 real-binary assertions over the full `lore` command surface) into CI as the `docker-e2e` job, and LCLI-178 documented it in the runbook as a "CI gate (required, since LCLI-100)". But "required" there is aspirational — the actual GitHub enforcement that would make a red run block a merge lives in **repo settings (branch protection / rulesets)**, which no workflow file can set and which LCLI-100's own notes explicitly deferred ("AC2's 'required check' in the branch-protection sense is a repo-settings change outside any workflow file's reach … Branch protection must be updated separately by the orchestrator/user once the gate is green, or docker-e2e remains advisory"). Until then, a regression in anything the harness covers (LCLI-61..68) can merge cleanly.
...
- Confirm a recent `docker-e2e` run on the target branch is **green before** enabling enforcement — LCLI-100 flagged the risk of a temporarily-red required gate (the stale LCLI-64 `check`-profile assertion that made the harness baseline red was since fixed by LCLI-176, Done, so the baseline should now pass, but verify a real run rather than assume).
...
Provenance: follow-up flagged in LCLI-100 (Done) and LCLI-178 (Done); `backlog search "branch protection"` and `backlog search "docker-e2e"` returned no task tracking the required-check/branch-protection toggle, so this is net-new.
...
## Required-check eligibility caveat (from LCLI-251 review, 2026-07-23)

LCLI-251 (PR #241) made the CI test matrix event-scoped: on `pull_request` only `check (ubuntu-latest)` and `check (windows-latest)` run; `check (macos-latest)` runs only on push-to-main / workflow_dispatch. Consequence for THIS task when a human configures branch protection:
...
- `bypass_actors: [{actor_type: RepositoryRole, actor_id: 5 (Admin), bypass_mode: always}]` — the sole repo ADMIN (owner) can bypass, so the merge-queue's direct `git push origin dev` (tracker/archive commits) and dev→main fast-forwards keep working. This bypass is load-bearing: the current dev tip has 0 check-runs because LCLI-251 removed the push-to-dev CI trigger, so a non-bypassed required check would permanently wedge direct pushes to dev.
...
- The content now on dev (`e99f2b6`, the LCLI-251 squash merge) was verified green via its PR run **30092891454** (feature/LCLI-251, pull_request, 2026-07-24): job `docker e2e harness (real lore + backlog binaries)` → **success**.
- Last dev-push docker-e2e run **30064226491** (2026-07-24 03:26): docker-e2e → **success**.
- The current dev tip `e99f2b6` itself has **no** docker-e2e run — not a failure, but because LCLI-251 intentionally dropped the `push:[dev]` trigger; the required check reports on the PR-merge path, which is the intended enforcement point. The earlier (2026-07-23) 100%-failure window was the billing halt, now cleared.

### Eligibility (why only docker-e2e; confirmed against current ci.yml via a 4-agent adversarial verification) [context for AC1]
Six contexts run on every PR and are eligible: docker-e2e (green), `lint · typecheck · test (ubuntu-latest)` (green), `lint · typecheck · test (windows-latest)` (**RED** on both latest runs — excluded), `compile smoke (ubuntu)` (green), `mkdocs scaffold smoke` (green), `docusaurus scaffold smoke` (green). `lint · typecheck · test (macos-latest)` is INELIGIBLE (event-scoped to push/dispatch, never runs on a PR → would deadlock). Per the owner's decision, ONLY docker-e2e was required (minimal LCLI-196 scope). Observation (not actioned — no task filed): the windows leg is currently failing on dev HEAD; worth a separate fix before it could ever be required.
