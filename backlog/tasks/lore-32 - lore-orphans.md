...
- F3 (Math.max(...huge) spread RangeError): FIXED. Spread-free maxLen() loop; noted tasks.ts has the same latent pattern (bounded input there) → folded into LORE-51.
...
- F5/F6 (dup summary-row type + aligned renderer with shipped tasks.ts): DEFERRED to LORE-51 (cross-cutting extraction from shipped code needs its own focused review — the readValue precedent).
