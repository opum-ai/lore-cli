...
1. src/commands/link.ts — removeBackRefs (runUnlink's per-task loop): in the 'already-absent'
   ...
   returning "already-absent", mirroring LORE-121's fix to runLink's already-present branch. This
...
4. test/link.test.ts — add regression tests in the existing "backlog/ commit (LORE-49)" describe
   block:
...
<!-- SECTION:NOTES:BEGIN -->
Applied LORE-121's fix pattern symmetrically to the two sibling no-edit branches:
...
tests the genuinely-clean case (LORE-179 AC#3); fixed the now-inaccurate git.calls-empty assertion
...
New tests added to test/link.test.ts's "backlog/ commit (LORE-49)" describe block:
...
- Mutation check: `git diff -- src/commands/link.ts` saved to a patch, reverted with
  `git apply -R`, ran `bun test test/link.test.ts -t "LORE-179"` against the pre-fix code -> the
...
Applied LORE-121's fix pattern to the two sibling no-edit branches in src/commands/link.ts:
