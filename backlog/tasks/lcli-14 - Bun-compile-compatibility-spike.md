...
aspirational remark/unified architecture that was amended during LCLI-22 (per
...
1. [correctness] The note above says "Six OTHER docs" reference stale
   remark/unified framing but actually lists eight (matching LCLI-52's correct
   ...
   as LCLI-52 and the shipping commit message both correctly say.
...
3. [cleanup/provenance] This branch's `lore sync` run also backfilled
   ...
   the first time in this repo's history (unrelated to LCLI-14's own EXDEV/
   tech-stack work -- these directories existed before but a full-bundle sync
   apparently hadn't been run since LCLI-29 shipped index generation). Noting
