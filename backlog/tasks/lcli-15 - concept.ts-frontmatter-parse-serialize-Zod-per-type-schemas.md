...
DECISIONS (doc conflict — flag in PR): (a) Modeline canonical = ABOVE the --- fence per ADR-0011 (in-frontmatter YAML comments can't survive gray-matter/js-yaml round-trip); concept.ts canonical doc starts with '---'; modeline emission deferred to lore new (LCLI-17). Existing docs/ files use in-fence modeline (non-canonical, reflow-once accepted per ADR-0011). (b) byte-identical (AC#1) = fixpoint over CANONICAL fixtures, not arbitrary input. (c) schema.ts shipped with concept.ts (LCLI-15 title bundles 'Zod per-type schemas').
<!-- SECTION:PLAN:END -->
...
Delivered as PR #12 (https://github.com/jeremy-newhouse/lore/pull/12) into dev — awaiting review/merge by Jeremy. Status stays In Progress; mark Done ON MERGE via a direct-to-dev chore commit (LCLI-10/12 precedent).
