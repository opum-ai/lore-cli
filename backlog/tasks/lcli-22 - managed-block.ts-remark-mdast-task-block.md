...
4. Do NOT wire link/unlink/sync into cli.ts (LCLI-24+). Feature branch feat/lore-22-managed-block -> PR into dev; user merges.
<!-- SECTION:PLAN:END -->
...
Scope: engine only — NOT wired into cli.ts (link/unlink/sync are LCLI-24+). ADR-0008 records a remark-stringify mechanism that lore cannot ship; the module docstring records the splice supersession — whether to amend the ADR file itself is pending a user decision.

ADR-0008 amended in this PR (user decision): Status now carries an 'Amended — 2026-07-02 (LCLI-22)' note, §Decision item 3 records the frozen-string-splice mechanism (no serializer), and the 'Serializer coupling' tradeoff became 'Format coupling'. lore validate/check stay clean (0 errors/0 warnings).
...
DEFERRED (rationale):
- normalizeLink throws a bare Error (not LoreError) on an absolute path -> belongs at the LCLI-24 command boundary (docPath is a caller-supplied repo-relative precondition), not the pure engine.
- offsetsOf/Marker duplicate rewrite.ts positionOf/ByteRange, and cell/escapeLinkText duplicate indexes.ts linkText -> real DRY, but consolidation touches files outside LCLI-22; fold into a shared mdast-utils / md-cell-safety helper later (cf. indexes.ts encodePathSegments TODO(LCLI-28) pattern).
- findMarkers is tasks-only (not parameterized for lore:index structural location) -> forward scope beyond LCLI-22 ACs.
...
src/core/managed-block.ts + test/managed-block.test.ts land the pure lore:tasks managed-block engine: structural mdast marker location + frozen-string splice (no md serializer), byte-identical fixpoint (AC#1), and filePathRelative-sourced portable links with null-file tolerance (AC#2). ADR-0008 amended to match the shipped mechanism. Both ACs checked; gates green (typecheck, lint, 1071 tests, validate/check). Engine only — CLI wiring is LCLI-24+.
