...
1. src/commands/new.ts: add a checkSymlink parameter to readTemplateFile, using
   fswrite.ts's existing findSymlinkSegment (LORE-94 precedent) to refuse a
   ...
   resolveTemplate from parsed.template !== undefined, matching LORE-72's own
...
findSymlinkSegment (the same non-throwing per-segment lstatSync walk LORE-94 extracted from
