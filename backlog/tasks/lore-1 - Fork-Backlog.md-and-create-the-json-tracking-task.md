...
4. Create the in-fork tracking task with 'backlog task create' inside the fork (it dogfoods Backlog.md), with AC + plan + Testing per the PR template (AC#2). Capture the back-XXX id for the LORE-2 branch name.
...
- Pushed on branch tasks/back-510-json-output (commit 'BACK-510 - Add tracking task...') to origin (fork). This is the LORE-2 working branch.
