...
2. AC2: Rewire the five Phase 24 exit-class spot checks (L346-357) to pass
   ...
   one for error_type=drift, reusing AC3's induced LORE-58 write-failure
...
3. AC3: Seed a dedicated TASK4 (never linked elsewhere, so it doesn't disturb
   ...
   re-induce the same write failure for unlink) since LORE-58's fix and the
...
6. Adversarial review of the branch diff before opening the PR (independent
   subagent, not self-review -- caught a real defect in LORE-67 last session).
<!-- SECTION:PLAN:END -->
...
ErrorEnvelope check (AC2's validation half, see finding below); and a NEW LORE-58
