...
(same bug as LCLI-14, recurred here since this branch is off dev and
LCLI-14 hasn't merged).
...
2026-07-19: recorded a hard release gate discovered while closing LCLI-5 -- lore will NOT cut its first publish until MrLesk/Backlog.md tags a release containing PR #790 (BACK-545, the --json support lore's adapter depends on). Today's adapter (LCLI-53/LCLI-54) only works against an interim, developer-compiled pinned-commit build; there is no package.json dependency and nothing a published lore package could point a real end user's Backlog.md install at, so publishing now would mean every user's capability probe fails on first use. Documented as a Prerequisite in docs/runbooks/release-publishing.md and as a dated amendment in docs/adr/0002-backlog-integration-json-only.md. Check `gh release list --repo MrLesk/Backlog.md` against commit 22a091b570d44c4f302ca47e7fd36fa28ad8bcb0 before starting any release-cutting work.
<!-- SECTION:NOTES:END -->
