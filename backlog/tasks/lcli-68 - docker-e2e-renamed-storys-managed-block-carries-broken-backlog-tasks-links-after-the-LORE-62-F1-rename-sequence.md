...
Discovered while implementing LCLI-64 (declarative profile subsystem E2E coverage): a full `lore check --json` run against docker/e2e's scratch bundle, right after Phase 17 (schema export) and before any profile-subsystem changes, already reports 2 broken-link errors that have nothing to do with LCLI-64:
