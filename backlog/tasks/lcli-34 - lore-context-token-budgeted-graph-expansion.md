...
<!-- SECTION:PLAN:BEGIN -->
1. core/query.ts: memoize undirected adjacency per BundleGraph (WeakMap keyed by graph) + export adjacencyOf(); subgraph() reuses it (folds the deferred LCLI-31 adjacency-rebuild finding).
...
- FOLDED the deferred LCLI-31 finding: subgraph adjacency is now memoized per BundleGraph via adjacencyOf() (WeakMap), so multi-target traversals no longer rebuild the O(E) index.
