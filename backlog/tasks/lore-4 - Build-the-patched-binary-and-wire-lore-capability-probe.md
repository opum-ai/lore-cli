...
AC#2 VERIFIED. Shipped a minimal STANDALONE capability probe at src/adapters/backlog.ts (the design-spec §2.3/§8 'only backlog subprocess seam'; seeded with just the probe slice so LORE-21 extends the same file rather than a second spawning module). Implements contract §5 fail-loud over an injectable BacklogSpawn seam:
...
NOT wired into cli.ts dispatch: the coupling commands it gates (link/unlink/tasks/orphans/sync/check-coupling) are LORE-22+, so wiring is deferred to LORE-21 per the user's 'minimal standalone probe' decision.
