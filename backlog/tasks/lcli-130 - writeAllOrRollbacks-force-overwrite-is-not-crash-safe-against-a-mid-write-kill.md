...
2. Preserve the existing symlink refusal (LCLI-92): lstat the destination up front and throw the same conflict/ELOOP-style LoreError if it is a symlink, before any temp-file I/O -- document why this doesn't reopen the TOCTOU window (renameSync structurally never follows a symlink at its destination, so even a race there can only affect refuse-vs-silently-replace, never write-through).
3. Preserve mode/ownership across the swap (mirroring writeFileAtomic's LCLI-117 discipline) since the old in-place O_TRUNC write never re-created the inode and so never reset these -- a fresh temp file would otherwise silently apply the default umask.
