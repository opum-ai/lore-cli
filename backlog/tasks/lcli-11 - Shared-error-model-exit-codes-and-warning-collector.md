...
4. reportError(err,{json,color,stderr}) -> writes envelope JSON (json) or diagnostic text to stderr, returns exit code; non-LoreError -> EXIT_UNCAUGHT(1). Never touches stdout, never resolves mode/TTY (that is output.ts/LCLI-12).
