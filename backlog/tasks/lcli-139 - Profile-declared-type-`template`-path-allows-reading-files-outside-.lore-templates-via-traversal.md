...
3. Deliberately leave the LCLI-91 symlink-scope precedent alone: a SYMLINKED profile-declared template is documented (test/new.test.ts) as intentionally out of scope for that prior task; LCLI-139 is only about syntactic path-traversal/absolute-path confinement, not symlink-following.
