...
Mutation-check: reverted src/core/schema.ts via git diff + git apply -R (no stash, per campaign rules), reran bun test test/schema.test.ts -> new LCLI-167 test failed exactly as expected ('expected validateFrontmatter to throw a validation LoreError, but it returned'). Reapplied the patch (git apply) -> 27/27 pass.
