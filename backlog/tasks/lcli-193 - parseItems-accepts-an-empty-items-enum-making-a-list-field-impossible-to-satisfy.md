...
Same bug class as LCLI-140, one seam over (list fields instead of scalar fields). parseItems (src/core/profile.ts:546-565) reads a list field's `items = { enum = [] }` via asStringArray with no length check and returns { kind: "string", enum: [] }; itemToZod (src/core/profile.ts:734) feeds it straight to z.enum([...]), so a profile declaring e.g. `tags = { kind = "list", items = { enum = [] } }` compiles WITHOUT error into z.array(z.enum([])) — a list no element can ever satisfy, with no error pointing at the mistake. LCLI-140 added assertNonEmptyEnum but wired it only into parseFieldSpec (profile.ts:494); reuse it in parseItems for a one-call fix. Surfaced by the wave-12 LCLI-140 per-task review and re-confirmed by the wave-12 integration review (real + unfixed on dev).
