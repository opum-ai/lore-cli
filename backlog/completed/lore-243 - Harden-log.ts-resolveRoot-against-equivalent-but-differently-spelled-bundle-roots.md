...
2. Add new tests in test/log.test.ts: (a) generateLog byte-identical across docs/./docs/docs/./ (docs/) spellings, (b) internal redundant separators docs//adr and docs/./adr resolve like docs/adr, (c) extend the LORE-143 buildLog assertion so an equivalent-spelling root also arrives at adapter.history identically to its canonical form.
