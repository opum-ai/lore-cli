...
1. Live-CLI-verify (done): `lore rename reference/orders foo/../../pwned` already fails today with
   usage/exit 2, via LORE-79's assertDestinationConfined call in runRename (right after
...
3. Update the surrounding comments (module docstring lines ~28-33, assertDestinationConfined's own
   docstring) to reflect the new call site and credit LORE-78 alongside LORE-79/80 as the third
   (now-realized) layer.
4. Add one new test in test/rename.test.ts's "errors and arg parsing" describe block, labeled
   LORE-78, asserting a `..`-containing newId is rejected as usage — giving LORE-78 its own
   AC-to-test traceability distinct from LORE-79's existing coverage (no exported parse function
...
already failed with usage/exit 2 today, via LORE-79's assertDestinationConfined call in runRename's
body (right after parseRenameArgs returns) — AC#1 was already substantively satisfied before this
task's own change. LORE-78's genuine remaining value is AC#2's literal ask: the check named at the
"args.ts / parseRenameArgs" layer specifically, not the command-body layer LORE-79 used.
...
assertDestinationConfined's own docstring to credit LORE-78 alongside LORE-79/LORE-80.

Added one new test (test/rename.test.ts, "errors and arg parsing") explicitly labeled LORE-78: a
...
framing, not just a caller-side follow-up check (which is what LORE-79 had already shipped).
...
absent. Added one new LORE-78-labeled test (test/rename.test.ts) proving the check needs nothing
