...
1. src/adapters/backlog.ts — touch ONLY probeBacklog() and its immediate constants/types,
   ...
   LORE-54's rewrite. Reusing EXPECTED_SCHEMA_VERSION for the probe would flip it project-wide
   and break real reads + the golden fixture tests, which are explicitly LORE-54's scope.
...
6. Record notes on LORE-53: the probe-only-constant split rationale, and the confirmed
...
parseEnvelope (still this fork's string "1"/`data` shape) which LORE-54 owns -- confirmed by
...
`bun build --compile` silent-fails per prior LORE-4 notes) for real `--version` (from upstream's
...
later one (a real read, LORE-54 territory) -- verified this holds for every test in the file
