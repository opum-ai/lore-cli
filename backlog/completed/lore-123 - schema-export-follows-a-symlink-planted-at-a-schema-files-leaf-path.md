...
<!-- SECTION:PLAN:BEGIN -->
1. Swap schema.ts's per-file write loop from writeFileOverwriting to the existing leaf-symlink-safe writeFileNoFollow (fswrite.ts), updating the import; no change to fswrite.ts (LORE-117 owns it this wave).
2. writeFileNoFollow already maps ELOOP to a conflict LoreError (same shape as the existing LORE-93 ancestor-symlink guard), so no extra mapping in schema.ts is needed — verified against fswrite.ts's own implementation.
...
<!-- SECTION:NOTES:BEGIN -->
Swapped schema.ts's per-file write (schema.ts:110, import at :38) from writeFileOverwriting to the existing leaf-symlink-safe writeFileNoFollow (fswrite.ts:478, owned by sibling LORE-117 — used as-is, not modified). writeFileNoFollow already maps ELOOP to a conflict LoreError with the same message shape as the LORE-93 ancestor-symlink guard, so no additional error mapping was needed in schema.ts. Added a regression test (test/schema-export.test.ts, new describe block 'leaf-symlink guard on the written file itself (LORE-123)'): seeds a normal export, replaces .lore/schemas/story.schema.json with a symlink to an external tmp file, re-runs export, and asserts a conflict LoreError is thrown and the external file's bytes are unchanged (POSIX-only, matching the codebase's existing symlink-test skip convention).
<!-- SECTION:NOTES:END -->
