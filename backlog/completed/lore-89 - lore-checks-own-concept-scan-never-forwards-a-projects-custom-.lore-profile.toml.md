...
options.profile, entirely separate from core/bundle.ts's loadBundle (which LORE-84 fixed to thread
...
(LORE-27 rounds 9/10) is untouched.

AC#1/AC#2/AC#3: two new tests in test/check.test.ts's "runCheck -- status + managed-block drift
(LORE-27)" describe block -- a tasks:-linked Story missing a custom-profile-required "owner" field
...
loadBundle in isolation, reusing the same custom-Story-redefinition shape LORE-84's own
