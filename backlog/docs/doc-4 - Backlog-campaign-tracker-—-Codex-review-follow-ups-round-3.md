...
| 4 | LORE-201 | core-engine-b | — | Dispatched | 20 | docs. Fix the `validation` instructions topic's overstated colon quote-safety claim and add a regression test. [src/core/instructions.ts] |
...
| 6 | LORE-203 | core-index-context | — | Dispatched | 20 | docs. Clarify `lore context` --max-tokens docs: omitting it applies no token cap (bounded only by --depth). [src/core/context.ts] |
...
| 8 | LORE-205 | build-ci-config | — | Dispatched | 20 | chore. Test fakes dirtyGitSpawn/failingCommitGitSpawn: dispatch the dirty status on the git subcommand, not on call index. [test/helpers.ts] |
...
| 10 | LORE-207 | cmd-check | — | Dispatched | 20 | chore. Release the response body in check's --external liveness fetch. [src/commands/check.ts] |
...
| 14 | LORE-195 | cmd-crud-b/tooling | — | Dispatched | 20 | chore. Restore biome lint baseline to green (biome check . -> 0 errors/0 infos). Filed 2026-07-23. [test/context.test.ts, test/replace.test.ts, test/validate.test.ts, src/core/managed-block.ts] |
| 15 | LORE-211 | cmd-meta-a | — | Dispatched | 20 | task. Strengthen the tasks drift test: mix a dangling id with a failing read and assert empty streams. [test/tasks.test.ts] |
