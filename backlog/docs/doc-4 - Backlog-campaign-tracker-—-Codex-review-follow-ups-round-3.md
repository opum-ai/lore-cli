...
The "ready now" set is **always recomputed live** from `backlog/tasks/*.md` + this table at the start of every restore/wave — never trust a persisted "next wave" plan. Informational hint only: as of 2026-07-23 after **waves 19-20 merged**, **43 tasks are ready** (of the original 55; waves 19-20 resolved 12), **0 in-flight**, **12 resolved**. Every item has zero unmet formal deps; wave membership is bounded by the live file-conflict graph + the 6-worker cap. Recompute live at next restore — do NOT hardcode a next-wave list.
