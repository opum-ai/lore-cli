...
**Next issue: LORE-89** — `lore check`'s own concept scan never forwards a
project's custom profile. Item 5 of the 8-item queue confirmed by the user
...
| --- | --- | --- | --- |
| 1 | LORE-89 | bug | lore check's own concept scan never forwards a project's custom profile |
| 2 | LORE-88 | bug | rewriteInbound (rename/supersede --rewrite-links) never forwards a project's custom profile |
| 3 | LORE-91 | bug | lore new --template silently follows a symlink, reads outside the repo |
| 4 | LORE-93 | bug | ensureDir call sites in 5 commands follow symlinks, escaping docs/ |
