...
**Next issue: LORE-74** — queue order confirmed by the user on 2026-07-21
...
| --- | --- | --- | --- |
| 1 | LORE-74 | bug | lore orphans report has no output cap, contradicting the documented truncation contract |
| 2 | LORE-75 | bug | lore schema export --out can irreversibly delete unrelated files outside its own directory |
| 3 | LORE-80 | bug | rewriteInbound shared engine does not confine fromId/toId to docs/ bundle root |
| 4 | LORE-79 | bug | lore rename destination path is not confined to docs/ root at the command layer |
| 5 | LORE-78 | bug | lore rename destination id is not validated for `..` traversal at the argument-parsing layer |
| 6 | LORE-81 | bug | lore rename index <new> (renaming FROM the reserved root index) is not rejected |
