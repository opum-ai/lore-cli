...
**Next issue: LORE-73** — queue order confirmed by the user on 2026-07-21
...
| --- | --- | --- | --- |
| 1 | LORE-73 | bug | lore replace can corrupt lore:tasks managed blocks (MANAGED_MARKERS gap) |
| 2 | LORE-74 | bug | lore orphans report has no output cap, contradicting the documented truncation contract |
| 3 | LORE-75 | bug | lore schema export --out can irreversibly delete unrelated files outside its own directory |
| 4 | LORE-80 | bug | rewriteInbound shared engine does not confine fromId/toId to docs/ bundle root |
| 5 | LORE-79 | bug | lore rename destination path is not confined to docs/ root at the command layer |
| 6 | LORE-78 | bug | lore rename destination id is not validated for `..` traversal at the argument-parsing layer |
| 7 | LORE-81 | bug | lore rename index <new> (renaming FROM the reserved root index) is not rejected |
...
- 2026-07-21 — session 20: resolved LORE-77 (see Resolved table). Branch
  ...
  One new campaign convention recorded (when a handover names a specific
  reusable helper for the next session's fix, say so explicitly with the
  exact function/file — it paid off exactly as predicted here). Cursor
  advanced to LORE-73.
