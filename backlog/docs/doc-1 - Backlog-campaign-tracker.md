...
- 2026-07-21 — session 22: resolved LORE-74 (see Resolved table). Branch
  ...
  since LORE-73's three-round ordeal — no independent review round run (routine
  output-shaping fix, single command file, well-precedented against `query`'s
  existing `--limit` shape). Resolved the prior handover's open design
  question (independent per-section truncation, not one combined cap). One new
  campaign convention recorded (a multi-section bounded-output command needs a
  total/shown/truncated triple per section, flat-prefixed on the result type,
  not nested and not combined). Cursor advanced to LORE-75.
