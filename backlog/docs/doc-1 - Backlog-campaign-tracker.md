...
**Next issue: LORE-91** — `lore new --template` silently follows a symlink,
reading outside the repo. Item 7 of the 8-item queue confirmed by the user
on 2026-07-21 ("Risk-ascending, sweep last (Recommended)": small
...
5-command sweep) for last, mirroring how the just-finished campaign
sequenced its own interrelated LORE-80/79/78/81 cluster). Do not re-ask
...
| --- | --- | --- | --- |
| 1 | LORE-91 | bug | lore new --template silently follows a symlink, reads outside the repo |
| 2 | LORE-93 | bug | ensureDir call sites in 5 commands follow symlinks, escaping docs/ |
